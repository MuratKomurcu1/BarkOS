...
environment reference is served by the `barkos` binary itself — kept out of this file on
purpose so it can never drift from the binary that will actually run your commands.

Engage BarkOS whenever you set up, review, debug, or validate a per-workspace environment
...
`environmentRecipes` entry in `barkos.yaml`, scaffold provider lifecycle scripts, or resolve
an `barkos vm recipe doctor` failure. BarkOS is a thin wrapper: you guide, detect, and scaffold;
...
```text
ORCA skills get barkos-per-workspace-env
```

That prints the complete, version-matched guide for the exact binary that will handle your
next commands — provider setup, base and auth snapshots, `environmentRecipes` in
`barkos.yaml`, lifecycle scripts, and `barkos vm recipe doctor`. Read it first, then run the
...
`ORCA skills get barkos-per-workspace-env`. Beyond these commands, ask the user rather than
