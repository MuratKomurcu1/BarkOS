...
name for `barkos-linear`; both resolve to the same Linear CLI (`barkos linear ...`). The full,
version-matched reference is served by the `barkos` binary itself — kept out of this file on
purpose so it can never drift from the binary that will actually run your commands.

Engage BarkOS's Linear CLI whenever you work a Linear-linked task: read linked ticket context,
...
PR/MR links, and triaging issues. The `barkos-linear` topic serves the same content. Read it
