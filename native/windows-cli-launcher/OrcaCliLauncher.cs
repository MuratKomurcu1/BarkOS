...
internal static class OrcaCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            ...
            string electronPath = Path.Combine(appDirectory, "BarkOS.exe");
            ...
            {
                Console.Error.WriteLine("Unable to locate BarkOS.exe next to \"{0}\"", resourcesDirectory);
                return 1;
            }

            if (!File.Exists(cliPath))
            {
                Console.Error.WriteLine("Unable to locate the BarkOS CLI entrypoint at \"{0}\"", cliPath);
                return 1;
            }
            ...
            Environment.SetEnvironmentVariable("ORCA_CLI_COMMAND", "barkos");
        ...
        {
            Console.Error.WriteLine("Unable to start the BarkOS CLI: {0}", error.Message);
            return 1;
        }
    }
