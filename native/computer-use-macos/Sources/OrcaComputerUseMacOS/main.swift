    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrustedSettled() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for BarkOS Computer Use. Run `orca computer permissions` or open Settings > Computer Use, grant Accessibility to BarkOS Computer Use, then retry."
            )
        }
        ...
        let screenshotStatus: ScreenshotStatus = if screenshot != nil {
            ...
            .failed("Screen Recording permission is required for BarkOS Computer Use; grant permission or pass --no-screenshot to inspect accessibility state only.")
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    if visibleWindowCount > 0 {
        ...
        throw ProviderError.coded("permission_denied", "app '\(app.name)' has visible windows but no accessibility window (AX reads stayed blocked for \(outcome.waitedMs)ms after retries). macOS Accessibility may need BarkOS Computer Use toggled off and on again in System Settings.")
    }
...
private final class PermissionWindowController: NSWindowController {
    ...
    convenience init(initialPermission: PermissionKind? = nil, terminateWhenDragAssistantCloses: Bool = false) {
        ...
        window.title = "Enable BarkOS Computer Use"
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        switch self {
        case .accessibility:
            "Drag BarkOS Computer Use into the list above to allow Accessibility."
        case .screenshots:
            "Drag BarkOS Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let titleText = checking
            ...
            : (ready ? "Computer Use is Ready" : "Enable BarkOS Computer Use")
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag BarkOS Computer Use"
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "BarkOS Computer Use")
...
private func isTrustedOrcaApplication(_ pid: pid_t) -> Bool {
    ...
    return bundleId == "com.barkos.desktop" ||
        bundleId.hasPrefix("com.barkos.desktop.dev") ||
        bundleId == "com.github.Electron"
}
...
private func runStdio() {
    fputs("BarkOS Computer Use provider must be launched by BarkOS in app-agent mode.\n", stderr)
    exit(13)
}
...
if arguments.first == "--agent" {
    guard arguments.count >= 2 else {
        fputs("usage: barkos-computer-use-macos --agent <socket-path> --token-file <token-path>\n", stderr)
        exit(2)
    }
    ...
    guard let token, !token.isEmpty else {
        fputs("barkos-computer-use-macos --agent requires a non-empty --token-file\n", stderr)
        exit(2)
    }
    ...
    guard arguments.count >= 2 else {
        fputs("usage: barkos-computer-use-macos --permission-status-file <path>\n", stderr)
        exit(2)
    }
