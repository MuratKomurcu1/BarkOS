...
describe('contextual tour definitions', () => {
  ...
  it('defines the workspace agent sessions value tour as split then create-worktree', () => {
    ...
    expect(tour?.steps.map((step) => step.title)).toEqual([
      'Terminal bölmesi açın',
      'Başka bir görevi paralel başlatın'
    ])
  ...
  it('points the workspace board tour at the board center and done lane', () => {
    ...
    expect(tour?.steps.map((step) => step.title)).toEqual([
      'İşi panoda planlayın',
      'İşi aşamalar arasında taşıyın'
    ])
    expect(tour?.steps[0]).toMatchObject({
  ...
  it('orders the browser tour as grab, annotate, then import cookies', () => {
    ...
    expect(tour?.steps.map((step) => step.title)).toEqual([
      'Sayfa bağlamını ajanlara aktarın',
      'Tasarım geri bildirimini yerinde işaretleyin',
      'Oturumlarınızı açık tutun'
    ])
    expect(tour?.steps[0]).toMatchObject({
  ...
  it('orders the automations tour as create, then results', () => {
    ...
    expect(tour?.steps.map((step) => step.title)).toEqual(['Otomasyon nedir?', 'Sonuçları bulun'])
    expect(tour?.steps[0]).toMatchObject({
  ...
  it('defines the floating workspace tour on the action list with a surface fallback', () => {
    ...
    expect(tour?.steps.map((step) => step.title)).toEqual([
      'Bir ajanı tüm depolarda çalıştırın',
      'Veya geçici çalışma alanı olarak kullanın'
    ])
    expect(tour?.steps.map((step) => step.body)).toEqual([
      'Ajanlar seçtiğiniz klasörde çalışır. Tüm depolarda birlikte çalışması için servislerin üst klasörünü seçin.',
      'Odaklandığınız çalışma alanını dağıtmadan ajan, terminal, not ve tarayıcı sekmeleri açın.'
    ])
    expect(tour?.steps[0]).toMatchObject({
      requiredForStart: true,
