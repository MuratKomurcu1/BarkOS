...
export const AGENT_PROVIDER_FAILURE_KINDS = ['usage-limit-exceeded'] as const
export type AgentProviderFailure = {
  kind: (typeof AGENT_PROVIDER_FAILURE_KINDS)[number]
}
...
export type AgentStatusEntry = {
  ...
  /** Provider-owned structured terminal cause. Never inferred from prose. */
...
export type AgentStatusPayload = {
  ...
  sessionBoundary?: boolean
  /** Provider-owned structured terminal cause. Only meaningful on `done`. */
  providerFailure?: AgentProviderFailure
...
function normalizeAgentStatusObject(parsed: unknown): ParsedAgentStatusPayload | null {
  ...
  return {
    ...
    providerFailure:
      state === 'done' &&
      typeof obj.providerFailure === 'object' &&
      obj.providerFailure !== null &&
      (obj.providerFailure as Record<string, unknown>).kind === 'usage-limit-exceeded'
