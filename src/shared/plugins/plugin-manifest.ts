...
/**
 * Plugin manifest v1 (`barkos-plugin.json` at the plugin root). The
 * `contributes` key names deliberately mirror common Electron-ecosystem
 * manifest conventions so future adapters stay cheap.
 *
 * Lives in `shared` so the desktop app, the headless `barkos serve` runtime,
...
const barkosEngineRangeSchema = z
  .string()
  .max(64)
...
export const pluginManifestSchema = z
  .object({
    ...
    engines: z
      .object({
        barkos: barkosEngineRangeSchema.optional(),
        orca: barkosEngineRangeSchema.optional()
      })
      .refine((engines) => engines.barkos !== undefined || engines.orca !== undefined, {
        message: 'must declare a BarkOS engine range'
      }),
...
export const PLUGIN_MANIFEST_FILENAME = 'barkos-plugin.json'
export const LEGACY_PLUGIN_MANIFEST_FILENAME = 'orca-plugin.json'

export function pluginEngineRange(manifest: PluginManifest): string {
  return manifest.engines.barkos ?? manifest.engines.orca!
}
...
export function satisfiesBarkosEngineRange(hostVersion: string, range: string): boolean {
