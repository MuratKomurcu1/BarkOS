...
export type HookListenerState = {
  ...
  /** Exact root-turn rollout cursors used for structured terminal outcomes. */
  codexRootTurnTranscriptByPaneKey: Map<string, CodexRootTurnTranscriptState>
...
export function createHookListenerState(): HookListenerState {
  return {
    ...
    codexRootTurnTranscriptByPaneKey: new Map(),
...
export function clearPaneCacheState(state: HookListenerState, paneKey: string): void {
  ...
  state.codexRootTurnTranscriptByPaneKey.delete(paneKey)
...
export function movePaneCacheState(
  ...
  movePaneScopedMapEntries(state.codexRootTurnTranscriptByPaneKey, fromPaneKey, toPaneKey)
...
export function clearAllListenerCaches(state: HookListenerState): void {
  ...
  state.codexRootTurnTranscriptByPaneKey.clear()
...
function extractOpenCodeToolFields(
  ...
  if (eventName === 'PreToolUse') {
    // Why: enforced side-effect gate posts carry tool_name/tool_input like
    // Claude/Codex PreToolUse; surface them as the pane's active tool snapshot.
    const toolName = readString(hookPayload, 'tool_name')
    return toolUpdate(
...
function getOrCreateCodexRootTurnTranscriptState(
  state: HookListenerState,
  paneKey: string
): CodexRootTurnTranscriptState {
  let transcriptState = state.codexRootTurnTranscriptByPaneKey.get(paneKey)
  if (!transcriptState) {
    transcriptState = createCodexRootTurnTranscriptState()
    state.codexRootTurnTranscriptByPaneKey.set(paneKey, transcriptState)
  }
  return transcriptState
}

export function hasCodexTranscriptActivity(state: HookListenerState, paneKey: string): boolean {
  return (
    hasTrackedCodexTranscriptSubagents(state.codexSubagentTranscriptByPaneKey.get(paneKey)) ||
    hasActiveCodexRootTurnTranscript(state.codexRootTurnTranscriptByPaneKey.get(paneKey))
  )
}

function hasCodexTranscriptSubagents(state: HookListenerState, paneKey: string): boolean {
...
function normalizeCodexEvent(
  ...
  if (agentId) {
    upsertCodexSubagent(
      ...
      agentId,
      {
        agentType: readString(hookPayload, 'agent_type'),
        model: readString(hookPayload, 'model'),
        state: hookStateName === 'waiting' ? 'waiting' : 'working'
      },
      Date.now()
    )
    return buildCodexChildDrivenStatusPayload(state, eventName, paneKey, hookPayload)
  ...
  const rootTranscript = getOrCreateCodexRootTurnTranscriptState(state, paneKey)
  const rootTurnId = readFirstString(hookPayload, ['turn_id', 'turnId'])
  if (eventName !== 'SessionStart' && eventName !== 'Stop') {
    beginCodexRootTurnTranscript(rootTranscript, transcriptPath, rootTurnId)
  }
  const rootCompletion =
    rootTurnId && rootTranscript.trackedTurnId === rootTurnId
      ? reconcileCodexRootTurnTranscript(rootTranscript)
      : undefined
  if (eventName === 'Stop') {
    finishCodexRootTurnTranscript(rootTranscript)
  }
  const stateName = rootCompletion ? 'done' : hookStateName
  ...
  return buildCodexStatusPayload(state, eventName, promptText, paneKey, hookPayload, {
    stateName: effectiveState,
    updateLead: true,
    providerFailure: rootCompletion?.providerFailure
  })
}
    ...
    eventName === 'SessionBusy' || eventName === 'MessagePart'
      ...
      : eventName === 'SessionIdle'
        ...
        : eventName === 'PermissionRequest' || eventName === 'AskUserQuestion'
          ? 'waiting'
          : eventName === 'PreToolUse'
            ? // Why: BarkOS side-effect enforcement POSTs every tool call as a
              // PreToolUse before execution; mirror Codex so gated calls surface
              // as working instead of being dropped (a dropped enforced request
              // must fail closed as a transport denial). A blocked-on-human
              // question tool maps to waiting so it cannot flip an existing
              // question card back to working.
              isAskUserQuestionTool(readString(hookPayload, 'tool_name')) ||
              readString(hookPayload, 'tool_name') === 'question'
              ? 'waiting'
              : 'working'
            : null
