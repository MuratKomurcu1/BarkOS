...
import {
  BARKOS_LINK_PROTOCOL,
  isBarkosCompatibleLinkProtocol,
  startsWithBarkosCompatibleLink
...
export function encodePairingOffer(offer: PairingOffer): string {
  ...
  return `${BARKOS_LINK_PROTOCOL}//pair?code=${base64url}`
}
...
export function decodePairingOffer(url: string): PairingOffer {
  ...
  if (!code) {
    throw new Error('Invalid pairing URL: must start with barkos://pair and include a pairing code')
  }
...
function extractPairingCodeFromUrl(url: string): string | null {
  ...
  // Why: prefix checks accepted routes like `barkos://pairing?...`; only the
  // pairing deep-link host may carry runtime auth material.
  if (!isBarkosCompatibleLinkProtocol(parsed.protocol) || parsed.hostname !== 'pair') {
    return null
  }
...
// Why: accept a BarkOS URL, a legacy Orca URL, or the bare base64
...
export function parsePairingCode(input: string): PairingOffer | null {
  ...
  try {
    if (startsWithBarkosCompatibleLink(trimmed)) {
