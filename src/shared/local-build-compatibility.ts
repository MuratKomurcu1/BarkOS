...
export const LOCAL_BUILD_COMPATIBILITY_FILENAME = 'barkos-local-build.json'
export const LOCAL_BUILD_COMPATIBILITY_FORMAT_VERSION =
  LOCAL_BUILD_COMPATIBILITY_CONTRACT.formatVersion
export const BARKOS_APP_ID = LOCAL_BUILD_COMPATIBILITY_CONTRACT.appId
...
export function getLocalBuildCompatibilityError(
  ...
  if (!target.readableStateSchemaVersions.includes(currentStateSchemaVersion)) {
    return `This build cannot read BarkOS workspace state schema ${currentStateSchemaVersion}. Your workspace was not changed.`
  }
