...
export function describeRuntimeCompatBlock(verdict: RuntimeCompatVerdict): string {
  ...
  if (verdict.reason === 'client-too-old') {
    return `This BarkOS client is too old for the selected server. Update BarkOS on this machine. Client protocol ${verdict.clientProtocolVersion}, server requires client protocol ${verdict.requiredClientProtocolVersion}.`
  }
  return `The selected BarkOS server is too old for this client. Update BarkOS on the server. Server protocol ${verdict.serverProtocolVersion}, client requires server protocol ${verdict.requiredServerProtocolVersion}.`
}
