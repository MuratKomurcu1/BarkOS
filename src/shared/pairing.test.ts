...
describe('pairing offer', () => {
  ...
  it('encode then decode round-trips correctly', () => {
    ...
    expect(url).toMatch(/^barkos:\/\/pair\?code=/)
  ...
  it('rejects BarkOS and legacy Orca URLs outside the exact pairing route', () => {
    const url = encodePairingOffer(offer)
    const code = new URLSearchParams(url.slice(url.indexOf('?') + 1)).get('code')!
    ...
    expect(parsePairingCode(`barkos://pairing?code=${code}`)).toBeNull()
  })
  ...
  it('decodes legacy Orca hash URLs', () => {
    const url = encodePairingOffer(offer)
    const code = new URLSearchParams(url.slice(url.indexOf('?') + 1)).get('code')!
...
describe('parsePairingCode', () => {
  ...
  it('parses a full BarkOS pairing URL', () => {
    const url = encodePairingOffer(offer)
