...
describe('release channel', () => {
  ...
  it('keeps dev builds out of the main release repo, and apart from each other', () => {
    expect(getReleaseRepoForChannel('hourly')).toBe('MuratKomurcu1/BarkOS')
    expect(getReleaseRepoForChannel('daily')).toBe('MuratKomurcu1/BarkOS')
    ...
    expect(getReleaseRepoForChannel('adhoc')).toBe('MuratKomurcu1/BarkOS')
    expect(getReleaseRepoForChannel('stable')).toBe('MuratKomurcu1/BarkOS')
    expect(getReleaseRepoForChannel('rc')).toBe('MuratKomurcu1/BarkOS')
  })
  ...
  it('builds release-notes links against the repo that published the version', () => {
    expect(getReleaseNotesUrlForVersion('1.4.160-hourly.202607281400')).toBe(
      'https://github.com/MuratKomurcu1/BarkOS/releases/tag/v1.4.160-hourly.202607281400'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160-daily.202607281300')).toBe(
      'https://github.com/MuratKomurcu1/BarkOS/releases/tag/v1.4.160-daily.202607281300'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160')).toBe(
      'https://github.com/MuratKomurcu1/BarkOS/releases/tag/v1.4.160'
    )
    expect(getReleaseNotesUrlForVersion('v1.4.160-rc.3')).toBe(
      'https://github.com/MuratKomurcu1/BarkOS/releases/tag/v1.4.160-rc.3'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160-adhoc.20260728140533')).toBe(
      'https://github.com/MuratKomurcu1/BarkOS/releases/tag/v1.4.160-adhoc.20260728140533'
    )
    expect(getReleaseNotesUrlForVersion(null)).toBe(
      'https://github.com/MuratKomurcu1/BarkOS/releases'
    )
  })
  ...
  it('sorts consecutive hourly builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/MuratKomurcu1/BarkOS-hourly/releases/tag/v${version}`
    })
  ...
  it('sorts consecutive adhoc builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/MuratKomurcu1/BarkOS-adhoc/releases/tag/v${version}`
    })
    const sorted = sortReleaseBuildsNewestFirst([
