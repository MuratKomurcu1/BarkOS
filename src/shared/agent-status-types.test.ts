...
Fix dispatch fallback preview for normalized status prompts`
    ...
    expect(
      result!.prompt.startsWith('You are working inside BarkOS, a multi-agent workspace.')
    ).toBe(true)
  ...
  it('accepts only the known structured provider failure on done', () => {
    expect(
      parseAgentStatusPayload('{"state":"done","providerFailure":{"kind":"usage-limit-exceeded"}}')!
        .providerFailure
    ).toEqual({ kind: 'usage-limit-exceeded' })
    for (const state of ['working', 'blocked', 'waiting'] as const) {
      expect(
        parseAgentStatusPayload(
          `{"state":"${state}","providerFailure":{"kind":"usage-limit-exceeded"}}`
        )!.providerFailure
      ).toBeUndefined()
    }
    expect(
      normalizeAgentStatusPayload({
        state: 'done',
        providerFailure: { kind: 'server-overloaded' }
      })!.providerFailure
    ).toBeUndefined()
    expect(
      normalizeAgentStatusPayload({ state: 'done', providerFailure: 'usage-limit-exceeded' })!
        .providerFailure
    ).toBeUndefined()
  })

  it('carries the provider failure through the client-visible payload projection', () => {
    expect(
      pickParsedAgentStatusPayload({
        state: 'done',
        prompt: 'continue',
