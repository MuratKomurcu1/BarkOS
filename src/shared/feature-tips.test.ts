...
describe('feature tips', () => {
  it('orders new unseen tips before older unseen tips', () => {
    ...
    expect(tips.map((tip) => tip.id)).toEqual(['barkos-cli', 'cmd-j-palette', 'voice-dictation'])
  })

  it('skips tips the user has already seen', () => {
    const tips = getOrderedUnseenFeatureTips({
      seenTipIds: new Set<FeatureTipId>(['voice-dictation', 'barkos-cli', 'cmd-j-palette'])
    })
  ...
  it('skips tips for features the user has already interacted with', () => {
    ...
    expect(tips.map((tip) => tip.id)).toEqual(['barkos-cli', 'cmd-j-palette'])
  })
  ...
  it('normalizes persisted tip ids', () => {
    ...
    ).toEqual(['barkos-cli', 'cmd-j-palette', 'voice-dictation'])
  })
  ...
  it('describes the command palette tip as a passive acknowledgement', () => {
    ...
    expect(paletteTip?.description).toContain('iş ağaçlarında')
    expect(paletteTip?.description).toContain('yeni bir iş ağacı')
  })

  it('describes the CLI tip as an install action with concrete workflows', () => {
    const cliTip = FEATURE_TIPS.find((tip) => tip.id === 'barkos-cli')
    ...
    expect(cliTip?.description).toContain('alt iş ağaçlarını koordine')
    expect(cliTip?.description).toContain('birbirleriyle haberleşmesini')
  })

  it('does not label the voice dictation tip as new', () => {
    const voiceTip = FEATURE_TIPS.find((tip) => tip.id === 'voice-dictation')

    expect(voiceTip?.eyebrow).toBe('İpucu')
    expect(voiceTip?.priority).toBe('unseen')
    expect(voiceTip?.title).toBe('Herhangi bir bölmeye sesle yazdırın')
    expect(voiceTip?.ctaLabel).toBe('Sesle yazmayı ayarla')
  })
})
