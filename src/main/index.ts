...
} from './barkos-profiles/profile-index-store'
...
import { installLinuxBareBarkosDispatcher } from './cli/linux-bare-orca-dispatcher'
...
function requestDesktopActivation(argv: readonly string[] = []): void {
  ...
  // Why: a duplicate `barkos serve` must not drag a headless server into opening a desktop window (#11935).
...
if (hasSingleInstanceLock) {
  // Why: couple to dev-parent only for electron-vite desktop runs; `barkos serve`'s parent (CLI shim/background shell) isn't the intended server lifetime.
  ...
  const startupServices = startFirstWindowStartupServices({
    ...
    startAgentHookServer: async () => {
      ...
      await agentHookServer.start({
        ...
        // Why: hooks source this endpoint file at invocation time so old PTY env reaches the current process after restart; dev namespaces it (worktrees share `barkos-dev`).
...
function prepareCodexRuntimeHomeForLaunch(
  ...
  const isolatedE2EHome = process.env.ORCA_E2E_HOME_DIR?.trim()
  if (runtimeHomePath === null && target?.runtime !== 'wsl' && isolatedE2EHome) {
    // Why: macOS login shells restore the native user HOME; an explicit path keeps E2E Codex inside its disposable profile.
    runtimeHomePath = join(isolatedE2EHome, '.codex')
  }
  if (
    ...
    try {
      // Why: managed accounts may launch from a self-contained CODEX_HOME rather than the shared mirror.
      markCodexProjectTrustedInHome(launchContext.workspacePath, runtimeHomePath)
    } catch (error) {
      console.warn('[codex-project-trust] failed to trust resolved runtime home:', error)
    }
  }
...
async function presentRendererRecoveryPrompt(recentRecoveryCount: number): Promise<void> {
  ...
  const options = {
    ...
    defaultId: 0,
    cancelId: 1,
    title: 'BarkOS keeps failing to load',
    message: 'The app window crashed repeatedly and stopped reloading automatically.',
    detail: `BarkOS tried to recover ${recentRecoveryCount} times in a row without success. This is often a graphics-driver or installation problem. Reload to try again, or quit and relaunch BarkOS.`
  }
...
void app.whenReady().then(async () => {
  ...
  const runtimeService = new OrcaRuntimeService(store, stats, {
    ...
    barkosUsageStores: { claudeUsage, codexUsage },
    orchestrationEnvironmentTransport,
    skillTransactionRecovery
  })
  runtime = runtimeService
  agentHookServer.setPairedToolUseDecisionEvaluator(
    runtimeService.getBarkosPairedSideEffectApprovalBroker().evaluate
  )
  ...
  // Why: headless `barkos serve` clients reach plugins through the runtime RPC
  ...
  runtimeRpc = new OrcaRuntimeRpcServer({
    ...
    // `barkos serve` is an explicit remote opt-in, and E2E keeps the wide bind its harness connects over.
    ...
    ...(serveOptions?.wsPort !== undefined
      ? {
          ...
          // Why: only explicit `barkos serve --port` overrides a stale STA-1511 fallback (issue #8535); default/dev stay fallback-first for pairing stability.
  ...
  if (serveOptions) {
    ...
    if (process.platform === 'darwin' || process.platform === 'linux') {
      try {
        ...
        console.log(
          `[serve] BarkOS CLI install: ${cliStatus.state}${cliStatus.commandPath ? ` (${cliStatus.commandPath})` : ''}`
        )
      } catch (error) {
        console.warn(
          '[serve] BarkOS CLI install skipped:',
          error instanceof Error ? error.message : String(error)
        )
      }
    }
    // Why: headless serve still needs a non-privileged BarkOS CLI fallback when normal installation is unavailable.
    if (process.platform === 'linux' && app.isPackaged && process.resourcesPath) {
      try {
        const dispatcher = await installLinuxBareBarkosDispatcher({
          resourcesPath: process.resourcesPath
        })
        console.log(
          `[serve] bare BarkOS dispatcher ${dispatcher.state}: ${dispatcher.dispatcherPath}` +
            `${dispatcher.target ? ` -> ${dispatcher.target}` : ''}`
        )
      } catch (error) {
        console.warn(
          '[serve] bare BarkOS dispatcher install skipped:',
          error instanceof Error ? error.message : String(error)
        )
      }
    }
