...
export type WslRelayLinkOptions = {
  ...
  evaluateSideEffect: (params: unknown, connectionId: string) => unknown
...
export function wireWslRelayLink(options: WslRelayLinkOptions): void {
  const { mux, child, distro, ingest, evaluateSideEffect, warn, onDead } = options
  const connectionId = wslHookRelayConnectionId(distro)

  mux.onRequest(AGENT_HOOK_EVALUATE_SIDE_EFFECT_METHOD, (params) =>
