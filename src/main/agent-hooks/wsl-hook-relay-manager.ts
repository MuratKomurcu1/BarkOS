...
type DistroState = {
  ...
  sideEffectApprovalAgents?: Set<AgentHookTarget>
}
...
export class WslHookRelayManager {
  ...
  isSideEffectApprovalReady(
    distro: string | null,
    agent: 'claude' | 'codex' | 'droid' | 'gemini'
  ): boolean {
    const state = this.stateFor(distro)
    return state?.phase === 'running' && state.sideEffectApprovalAgents?.has(agent) === true
  }
  ...
  private async connect(
    ...
    wireWslRelayLink({
      ...
      evaluateSideEffect: this.deps.evaluateSideEffect,
      warn: this.deps.warn,
      onDead: (reason) => {
