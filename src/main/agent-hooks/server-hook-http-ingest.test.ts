...
describe('AgentHookServer listener replay', () => {
  it('returns an exact PreToolUse decision while keeping other Claude hooks neutral', async () => {
    ...
    try {
      ...
      const toolResponse = await fetch(`http://127.0.0.1:${env.ORCA_AGENT_HOOK_PORT}/hook/claude`, {
        method: 'POST',
        headers,
        body: JSON.stringify(
          buildBody(
      ...
      const lifecycleResponse = await fetch(
        `http://127.0.0.1:${env.ORCA_AGENT_HOOK_PORT}/hook/claude`,
        {
          method: 'POST',
          headers,
          body: JSON.stringify(buildBody({ hook_event_name: 'Stop' }))
        }
      )
      expect(lifecycleResponse.status).toBe(200)
      expect(await lifecycleResponse.json()).toEqual({})
  ...
  it('returns blocking and neutral JSON to an enforced local Codex hook', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production' })
    try {
        ...
        request.source === 'codex'
          ? {
              hookSpecificOutput: {
                hookEventName: 'PreToolUse' as const,
                permissionDecision: 'deny' as const,
                permissionDecisionReason: 'BarkOS approval required.'
              }
            }
          : null
      )
      ...
      const body = buildBody(
        ...
        {
          launchToken: 'codex-launch-1',
          barkosSideEffectEnforcement: '1'
        }
      )
      const toolResponse = await fetch(`http://127.0.0.1:${env.ORCA_AGENT_HOOK_PORT}/hook/codex`, {
        method: 'POST',
        headers,
      ...
      const lifecycleResponse = await fetch(
        `http://127.0.0.1:${env.ORCA_AGENT_HOOK_PORT}/hook/codex`,
        {
          ...
          headers,
          body: JSON.stringify(
            buildBody({ hook_event_name: 'Stop' }, { barkosSideEffectEnforcement: '1' })
          )
        }
      )
      expect(lifecycleResponse.status).toBe(200)
  ...
  it('returns Factory-compatible blocking and neutral JSON to an enforced local Droid hook', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production' })
    try {
      const evaluator = vi.fn((request) =>
        ...
        (request.toolInput as { command?: unknown }).command === 'git push origin main'
          ? {
              hookSpecificOutput: {
                hookEventName: 'PreToolUse' as const,
                permissionDecision: 'deny' as const,
      ...
      const post = (payload: Record<string, unknown>) =>
        fetch(`http://127.0.0.1:${env.ORCA_AGENT_HOOK_PORT}/hook/droid`, {
          method: 'POST',
          headers,
          body: JSON.stringify(
            buildBody(payload, {
              launchToken: 'droid-launch-1',
              barkosSideEffectEnforcement: '1'
            })
          )
        })
      ...
      const lifecycle = await post({ hook_event_name: 'Stop' })
      expect(lifecycle.status).toBe(204)
      ...
      expect(await evaluatorFailure.json()).toMatchObject({
        hookSpecificOutput: { permissionDecision: 'deny' }
      })
    } finally {
      server.stop()
  ...
  it('uses the paired evaluator first and fails closed when no owner handles a side effect', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production' })
    try {
      const paired = vi.fn(async () => ({ version: 1 as const, matched: true, decision: null }))
      const local = vi.fn(() => null)
      server.setPairedToolUseDecisionEvaluator(paired)
      server.setToolUseDecisionEvaluator(local)
      const env = server.buildPtyEnv()
      const post = () =>
        fetch(`http://127.0.0.1:${env.ORCA_AGENT_HOOK_PORT}/hook/codex`, {
          ...
          body: JSON.stringify(
            buildBody(
              ...
              { launchToken: 'launch-1', barkosSideEffectEnforcement: '1' }
            )
          )
        })
      ...
      paired.mockResolvedValue({ version: 1, matched: false, decision: null })
      server.setToolUseDecisionEvaluator(null)
      const denied = await post()
      expect(await denied.json()).toMatchObject({
        hookSpecificOutput: { permissionDecision: 'deny' }
      })
