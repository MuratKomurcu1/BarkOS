...
describe('Windows managed hook stdin structure', () => {
  it('exits immediately when Orca env is missing and keeps drain for other failures', () => {
    ...
    try {
      ...
      const claudeDevinGuard =
        'if not "%DEVIN_PROJECT_DIR%"=="" (echo {} & goto :orca_agent_hook_drain_stdin)'
      expect(claude, 'claude devin guard present').toContain(claudeDevinGuard)
      expect(claude.indexOf('if "%ORCA_PANE_KEY%"=="" (echo {} & exit /b 0)')).toBeLessThan(
        claude.indexOf(claudeDevinGuard)
      )
