...
export type AgentHookToolUseRequest = Readonly<{
...
export type AgentHookToolUseDecision = AgentHookSideEffectDecision
...
type ToolUseDecisionEvaluator = (
  request: AgentHookToolUseRequest
) => AgentHookToolUseDecision | null | Promise<AgentHookToolUseDecision | null>
type PairedToolUseDecisionEvaluator = (
  request: AgentHookToolUseRequest
) => Promise<AgentHookSideEffectRelayResponse>
type RemoteToolUseDecisionEvaluator = (
  request: AgentHookToolUseRequest
...
const CODEX_TRANSCRIPT_POLL_MS = 1_000
...
function equivalentParsedAgentStatusPayload(
  ...
  return (
    ...
    a.providerFailure?.kind === b.providerFailure?.kind &&
...
function hookPayloadRecord(body: unknown): Record<string, unknown> | null {
...
function hasBarkosSideEffectEnforcement(body: unknown): boolean {
  if (typeof body !== 'object' || body === null || Array.isArray(body)) {
    return false
  }
  const value = (body as Record<string, unknown>).barkosSideEffectEnforcement
  return value === '1' || value === true
}

function extractToolUseRequest(
  source: AgentHookSource,
  body: unknown
): AgentHookToolUseRequest | null {
  if (typeof body !== 'object' || body === null || Array.isArray(body)) {
    return null
  }
  const envelope = body as Record<string, unknown>
  const payload = hookPayloadRecord(envelope)
  const paneKey = typeof envelope.paneKey === 'string' ? envelope.paneKey.trim() : ''
  const toolName = typeof payload?.tool_name === 'string' ? payload.tool_name.trim() : ''
  const expectedEventName = source === 'gemini' ? 'BeforeTool' : 'PreToolUse'
  if (
    !payload ||
    payload.hook_event_name !== expectedEventName ||
    !isValidPaneKey(paneKey) ||
    toolName.length === 0 ||
    toolName.length > 256
  ) {
    return null
  ...
  const launchToken =
    typeof envelope.launchToken === 'string' && envelope.launchToken.trim()
      ? envelope.launchToken.trim()
      : undefined
  const toolUseId =
  ...
  const providerSessionId =
    typeof payload.session_id === 'string' && payload.session_id.trim()
      ...
      : undefined
  const sideEffectEnforcement = hasBarkosSideEffectEnforcement(envelope)
  return {
    source,
    paneKey,
    ...(launchToken ? { launchToken } : {}),
    ...(sideEffectEnforcement ? { sideEffectEnforcement: true as const } : {}),
    toolName,
...
export class AgentHookServer {
  ...
  private toolUseDecisionEvaluator: ToolUseDecisionEvaluator | null = null
  private pairedToolUseDecisionEvaluator: PairedToolUseDecisionEvaluator | null = null
  private remoteToolUseDecisionEvaluator: RemoteToolUseDecisionEvaluator | null = null
  ...
  private codexTranscriptPollTimers = new Map<string, ReturnType<typeof setTimeout>>()
  ...
  setToolUseDecisionEvaluator(evaluator: ToolUseDecisionEvaluator | null): void {
    this.toolUseDecisionEvaluator = evaluator
  }

  setPairedToolUseDecisionEvaluator(evaluator: PairedToolUseDecisionEvaluator | null): void {
    this.pairedToolUseDecisionEvaluator = evaluator
  }

  setRemoteToolUseDecisionEvaluator(evaluator: RemoteToolUseDecisionEvaluator | null): void {
    this.remoteToolUseDecisionEvaluator = evaluator
  }
  ...
  private clearCodexTranscriptPoll(paneKey: string): void {
    const timer = this.codexTranscriptPollTimers.get(paneKey)
    if (!timer) {
      return
    }
    clearTimeout(timer)
    this.codexTranscriptPollTimers.delete(paneKey)
  }

  private scheduleCodexTranscriptPoll(
    source: AgentHookSource,
    body: unknown,
    ...
    this.clearCodexTranscriptPoll(original.paneKey)
    if (!hasCodexTranscriptActivity(this.state, original.paneKey)) {
      return
    }
    const timer = setTimeout(() => {
      this.codexTranscriptPollTimers.delete(original.paneKey)
      ...
      const transcriptStatusChanged =
        normalized.payload.state !== original.payload.state ||
        normalized.payload.providerFailure?.kind !== original.payload.providerFailure?.kind ||
        JSON.stringify(normalized.payload.subagents) !== JSON.stringify(original.payload.subagents)
      const next = transcriptStatusChanged ? this.applyNormalizedStatus(normalized) : original
      this.scheduleCodexTranscriptPoll(source, body, next)
    }, CODEX_TRANSCRIPT_POLL_MS)
    this.codexTranscriptPollTimers.set(original.paneKey, timer)
  ...
  transferPaneAuthority(
    ...
    this.clearCodexTranscriptPoll(previousOwnerPaneKey)
  ...
  retirePaneAuthority(paneKey: string): void {
    ...
    for (const key of paneKeys) {
      ...
      this.clearCodexTranscriptPoll(key)
  ...
  ingestTerminalStatus(event: {
    ...
    // That erased it from persisted rows (lost across restart) and from headless `barkos serve`, which
  ...
  async evaluateRemoteToolUse(
    params: unknown,
    connectionId: string
  ): Promise<AgentHookSideEffectRelayResponse> {
    const request = parseAgentHookSideEffectRelayRequest(params)
    const normalizedConnectionId = connectionId.trim()
    if (
      !request ||
      normalizedConnectionId.length === 0 ||
      (!this.remoteToolUseDecisionEvaluator && !this.pairedToolUseDecisionEvaluator)
    ) {
      return createAgentHookSideEffectRelayResponse(false, null)
    }

    this.ingestRemote(request.envelope, normalizedConnectionId)
    const toolUseRequest = extractToolUseRequest(request.envelope.source, {
      paneKey: request.envelope.paneKey,
      launchToken: request.envelope.launchToken,
      barkosSideEffectEnforcement: true,
      payload: request.toolPayload
    })
    if (!toolUseRequest) {
      return createAgentHookSideEffectRelayResponse(false, null)
    }
    ...
    if (this.pairedToolUseDecisionEvaluator) {
      try {
        const paired = await this.pairedToolUseDecisionEvaluator(normalizedRequest)
        if (paired.matched) {
          return paired
        }
      } catch {
        return createAgentHookSideEffectRelayResponse(
          true,
          createAgentHookSideEffectTransportDenial(undefined, request.envelope.source)
        )
      }
    }
    if (this.remoteToolUseDecisionEvaluator) {
      return this.remoteToolUseDecisionEvaluator(normalizedRequest)
    }
    return createAgentHookSideEffectRelayResponse(
      true,
      createAgentHookSideEffectTransportDenial(undefined, request.envelope.source)
  ...
  async start(options?: {
    ...
    const handleRequest = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      let enforcedToolUseSource: AgentHookSource | null = null
      if (req.method !== 'POST') {
        res.writeHead(404)
        res.end()
        return
      ...
      try {
        ...
        const hookEventName = hookPayloadRecord(aliasedBody)?.hook_event_name
        if (
          hasBarkosSideEffectEnforcement(aliasedBody) &&
          hookEventName === (source === 'gemini' ? 'BeforeTool' : 'PreToolUse')
        ) {
          enforcedToolUseSource = source
        }
        ...
        const statusDisposition = normalized.event
          ...
          : 'suppress'
        let toolUseDecision: AgentHookToolUseDecision | null = null
        let hasToolUseRequest = false
        if (normalized.event && statusDisposition !== 'suppress') {
          ...
          this.scheduleCodexTranscriptPoll(source, aliasedBody, enriched)
          const toolUseRequest = extractToolUseRequest(source, aliasedBody)
          if (toolUseRequest) {
            hasToolUseRequest = true
            let pairedHandled = false
            let pairedResponse: AgentHookSideEffectRelayResponse | null = null
            if (this.pairedToolUseDecisionEvaluator) {
              try {
                pairedResponse = await this.pairedToolUseDecisionEvaluator(toolUseRequest)
              } catch {
                pairedResponse = createAgentHookSideEffectRelayResponse(
                  true,
                  createAgentHookSideEffectTransportDenial(undefined, toolUseRequest.source)
                )
              }
              if (pairedResponse.matched) {
                pairedHandled = true
                toolUseDecision = pairedResponse.decision
              }
            }
            if (!pairedHandled && this.toolUseDecisionEvaluator) {
              toolUseDecision = await this.toolUseDecisionEvaluator(toolUseRequest)
            } else if (
              !pairedHandled &&
              pairedResponse?.matched === false &&
              toolUseRequest.sideEffectEnforcement === true
            ) {
              toolUseDecision = createAgentHookSideEffectTransportDenial(
                undefined,
                toolUseRequest.source
              )
            }
          }
        }

        const needsNeutralDecisionBody =
          (this.toolUseDecisionEvaluator || this.pairedToolUseDecisionEvaluator) &&
          (source === 'claude' ||
            source === 'gemini' ||
            ((source === 'codex' ||
              source === 'opencode' ||
              (source === 'droid' && hasToolUseRequest)) &&
              hasBarkosSideEffectEnforcement(aliasedBody)))
        if (
          (source === 'droid' || source === 'gemini' || source === 'opencode') &&
          enforcedToolUseSource === source &&
          (!hasToolUseRequest ||
            (!this.toolUseDecisionEvaluator && !this.pairedToolUseDecisionEvaluator))
        ) {
          toolUseDecision = createAgentHookSideEffectTransportDenial(undefined, source)
        }
        if (toolUseDecision && !agentHookSideEffectDecisionMatchesSource(source, toolUseDecision)) {
          toolUseDecision = createAgentHookSideEffectTransportDenial(undefined, source)
        }
        if (toolUseDecision || needsNeutralDecisionBody) {
          res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' })
          res.end(JSON.stringify(toolUseDecision ?? {}))
          return
        }
        res.writeHead(204)
        res.end()
      } catch {
        if (enforcedToolUseSource) {
          res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' })
          res.end(
            JSON.stringify(
              createAgentHookSideEffectTransportDenial(undefined, enforcedToolUseSource)
            )
          )
        } else {
          // Why: ordinary status hooks stay fail-open; only an authenticated enforced PreToolUse blocks.
          res.writeHead(204)
          res.end()
        }
      }
  ...
  stop(): void {
    ...
    for (const timer of this.codexTranscriptPollTimers.values()) {
      clearTimeout(timer)
    }
    this.codexTranscriptPollTimers.clear()
  ...
  private deleteStatusEntry(
    ...
    this.clearCodexTranscriptPoll(resolvedPaneKey)
  ...
  dropStatusEntriesByTabPrefix(tabId: string): void {
    ...
    for (const paneKey of paneKeysToClear) {
      ...
      this.clearCodexTranscriptPoll(paneKey)
