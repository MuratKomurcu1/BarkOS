...
export function createHookStatusSessionTabsInvalidator(): {
  ...
  const invalidator = (event: AgentHookEventPayload): boolean => {
    ...
    return (
      ...
      (previous.providerFailure?.kind ?? null) !== (next.providerFailure?.kind ?? null) ||
      (previous.turnCompletedAt ?? null) !== (next.turnCompletedAt ?? null) ||
      (previous.lastAssistantMessage ?? null) !== (next.lastAssistantMessage ?? null)
    )
  }
