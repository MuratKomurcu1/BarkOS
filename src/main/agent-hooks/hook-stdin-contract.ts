...
export function buildWindowsHookEnvironmentGuardLines(options?: {
  neutralJson?: boolean
}): string[] {
  const exit = options?.neutralJson ? '(echo {} & exit /b 0)' : 'exit /b 0'
  return [
    `if "%ORCA_AGENT_HOOK_PORT%"=="" ${exit}`,
    `if "%ORCA_AGENT_HOOK_TOKEN%"=="" ${exit}`,
    `if "%ORCA_PANE_KEY%"=="" ${exit}`
  ]
}
