...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    ...
    it('delivers hook posts through the live relay and installs guest approval hooks', async () => {
      ...
      server.setRemoteToolUseDecisionEvaluator(evaluateSideEffect)
      ...
      manager = new WslHookRelayManager({
        ...
        evaluateSideEffect: (params, connectionId) =>
          server.evaluateRemoteToolUse(params, connectionId),
        managedHookSettings: () => ({
          agentCmdOverrides: {
            claude: '/bin/sh',
            codex: '/bin/sh',
      ...
      await vi.waitFor(
        () => expect(manager?.isSideEffectApprovalReady('LiveDistro', 'droid')).toBe(true),
        { timeout: 15_000 }
      )
      expect(existsSync(join(fakeHome, '.claude', 'settings.json')), warns.join('\n')).toBe(true)
      expect(existsSync(join(fakeHome, '.factory', 'settings.json')), warns.join('\n')).toBe(true)
      ...
      const droidRes = await fetch(`http://127.0.0.1:${port}/hook/droid`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Orca-Agent-Hook-Token': token ?? ''
      ...
      expect(droidRes.status).toBe(200)
      expect(await droidRes.json()).toMatchObject({
