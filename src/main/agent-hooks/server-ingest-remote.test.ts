...
describe('AgentHookServer ingestRemote', () => {
  it('evaluates a versioned remote PreToolUse request with stamped transport identity', async () => {
    const server = new AgentHookServer()
    const evaluator = vi.fn(() => ({ version: 1 as const, matched: true, decision: null }))
    server.setRemoteToolUseDecisionEvaluator(evaluator)

    await expect(
      server.evaluateRemoteToolUse(
        {
          version: 1,
    ...
    ).resolves.toEqual({ version: 1, matched: true, decision: null })
  ...
  it('evaluates a versioned remote Gemini BeforeTool request', async () => {
    const server = new AgentHookServer()
    const evaluator = vi.fn(() => ({
      version: 1 as const,
      matched: true,
      decision: { decision: 'deny' as const, reason: 'Approval required.' }
    }))
    server.setRemoteToolUseDecisionEvaluator(evaluator)
      ...
      server.evaluateRemoteToolUse(
        {
          ...
          envelope: {
            ...
            launchToken: 'launch-token',
            connectionId: null,
            payload: { state: 'working', prompt: 'task', agentType: 'gemini' }
          },
          toolPayload: {
  ...
  it('returns unmatched for malformed or unsupported remote approval requests', async () => {
    const server = new AgentHookServer()
    const evaluator = vi.fn(() => ({ version: 1 as const, matched: true, decision: null }))
    server.setRemoteToolUseDecisionEvaluator(evaluator)

    await expect(server.evaluateRemoteToolUse({ version: 2 }, 'remote-1')).resolves.toEqual({
      version: 1,
      matched: false,
      decision: null
    })
  ...
  it('routes nested remote approvals through the paired owner before the SSH evaluator', async () => {
    const server = new AgentHookServer()
    const pairedEvaluator = vi.fn(async () => ({
      version: 1 as const,
      matched: true,
      decision: null
    }))
    const remoteEvaluator = vi.fn(() => ({
      version: 1 as const,
      matched: true,
      decision: null
    }))
    server.setPairedToolUseDecisionEvaluator(pairedEvaluator)
    server.setRemoteToolUseDecisionEvaluator(remoteEvaluator)
    ...
    expect(pairedEvaluator).toHaveBeenCalledWith(
      expect.objectContaining({ connectionId: 'remote-1', sideEffectEnforcement: true })
    )
    expect(remoteEvaluator).not.toHaveBeenCalled()
  })

  it('preserves the optional structured provider failure and remains compatible when absent', () => {
    const server = new AgentHookServer()
    server.ingestRemote(
      {
        paneKey: PANE,
    ...
    expect(server.getStatusSnapshot()[0]?.providerFailure).toEqual({
      kind: 'usage-limit-exceeded'
    })
    ...
    server.ingestRemote(
      {
        ...
        tabId: 'tab-1',
        worktreeId: 'wt-1',
        payload: { state: 'working', prompt: 'Next turn', agentType: 'codex' }
      },
      'conn-1'
    )
    expect(server.getStatusSnapshot()[0]?.providerFailure).toBeUndefined()
  })
