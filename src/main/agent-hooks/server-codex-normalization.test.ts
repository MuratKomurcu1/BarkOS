...
describe('Codex hook normalization', () => {
  ...
  it('derives a typed limit failure only from the exact rollout turn completion', () => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-hook-limit-'))
    dirs.push(dir)
    ...
    expect(_internals.normalizeHookPayload('codex', body, 'production')?.payload).toMatchObject({
      state: 'working',
      prompt: 'Continue the task'
    })
    appendFileSync(
    ...
    expect(nextTurn?.payload.state).toBe('working')
    expect(nextTurn?.payload.providerFailure).toBeUndefined()
  })

  it('does not classify assistant or error prose as a provider limit', () => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-hook-limit-'))
    dirs.push(dir)
    const transcriptPath = join(dir, 'rollout.jsonl')
    writeFileSync(
      ...
      `${JSON.stringify({
        ...
        payload: {
          type: 'task_complete',
          turn_id: 'turn-one',
          error: { message: 'usage_limit_exceeded', codex_error_info: 'server_overloaded' }
        }
      })}\n`
    )
    const result = _internals.normalizeHookPayload(
      ...
      buildBody({
        ...
        turn_id: 'turn-one',
        transcript_path: transcriptPath,
        prompt: 'The words usage_limit_exceeded are not evidence'
      }),
      'production'
    )

    expect(result?.payload.state).toBe('done')
    expect(result?.payload.providerFailure).toBeUndefined()
  })
