...
import { AGENT_HOOK_EVALUATE_SIDE_EFFECT_METHOD } from '../../shared/agent-hook-side-effect-relay'
...
describe('WslHookRelayManager', () => {
  ...
  function guestTransport(
    ...
    // Why: the manager creates the sole host mux for this transport; the harness mux is only for standalone fs tests.
    harness.mux.dispose()
  ...
  function createManager(overrides: Partial<WslHookRelayManagerDeps>): {
    ...
    const deps: WslHookRelayManagerDeps = {
      ...
      ingest: vi.fn(),
      evaluateSideEffect: vi.fn(() => ({ version: 1, matched: false, decision: null })),
  ...
  it('advertises exact approval-hook readiness and serves guest decision requests', async () => {
    const decision = { version: 1, matched: true, decision: null }
    const installHooks = vi.fn(async () => [
      {
        agent: 'codex' as const,
        state: 'installed' as const,
        configPath: '/home/wsl-test-user/.codex/config.toml',
      ...
      },
      {
        ...
        state: 'installed' as const,
        configPath: '/home/wsl-test-user/.factory/settings.json',
        managedHooksPresent: true,
        detail: null
      },
      {
        ...
        state: 'installed' as const,
        configPath: '/home/wsl-test-user/.gemini/settings.json',
        managedHooksPresent: true,
        detail: null
      }
    ])
    const evaluateSideEffect = vi.fn(() => decision)
    const { manager } = createManager({ installHooks, evaluateSideEffect })
    manager.ensureForDistro('Ubuntu')

    await vi.waitFor(() => expect(manager.isSideEffectApprovalReady('ubuntu', 'codex')).toBe(true))
    expect(manager.isSideEffectApprovalReady('Ubuntu', 'droid')).toBe(true)
    expect(manager.isSideEffectApprovalReady('Ubuntu', 'gemini')).toBe(true)
    expect(manager.isSideEffectApprovalReady('Ubuntu', 'claude')).toBe(false)
    await expect(
      harnesses[0]!.guestDispatcher.requestPrimary(AGENT_HOOK_EVALUATE_SIDE_EFFECT_METHOD, {
        version: 1
      })
    ).resolves.toEqual(decision)
    expect(evaluateSideEffect).toHaveBeenCalledWith({ version: 1 }, 'wsl:Ubuntu')
    manager.disposeAll()
  })
