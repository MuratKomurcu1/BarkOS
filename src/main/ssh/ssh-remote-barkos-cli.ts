...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'barkos serve starts a foreground headless BarkOS server and cannot run through the SSH relay bridge. Run it directly on the machine that should host BarkOS.',
  'claude-teams':
    'barkos claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the BarkOS host machine.',
  'agent-teams-tmux':
    'orca agent-teams-tmux is a tmux pane shim for the BarkOS host machine and cannot run through the SSH relay bridge.',
  'account add':
    'barkos account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the BarkOS host machine.'
}
...
async function dispatchRemoteCli(
  ...
  switch (command) {
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH BarkOS CLI command: ${command} (full BarkOS CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
