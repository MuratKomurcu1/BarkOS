...
describe('SshRelaySession agent hooks over a fake relay transport', () => {
  ...
  afterEach(() => {
    ...
    agentHookServer.setRemoteToolUseDecisionEvaluator(null)
    agentHookInternals.resetCachesForTests()
    warnSpy.mockRestore()
  ...
  it('serves relay-to-client BarkOS side-effect decisions with the SSH target identity', async () => {
    relay = createFakeRelay()
    vi.mocked(deployAndLaunchRelay).mockResolvedValue({
      transport: relay.transport,
      serverBuildId: 'test-relay-build',
    ...
    const evaluate = vi.fn(() => ({ version: 1 as const, matched: true, decision: null }))
    agentHookServer.setRemoteToolUseDecisionEvaluator(evaluate)
    session = createSession('conn-fake')
    await session.establish({} as SshConnection)

    await expect(
      relay.dispatcher.requestPrimary(AGENT_HOOK_EVALUATE_SIDE_EFFECT_METHOD, {
        version: 1,
        barkosSideEffectEnforcement: true,
        envelope: makeEnvelope({
          source: 'droid',
          launchToken: 'launch-token',
          payload: { state: 'working', prompt: 'remote prompt', agentType: 'droid' }
        }),
        toolPayload: {
    ...
    await expect(
      relay.dispatcher.requestPrimary(AGENT_HOOK_EVALUATE_SIDE_EFFECT_METHOD, {
        ...
        envelope: makeEnvelope({
          source: 'gemini',
          launchToken: 'gemini-launch-token',
          payload: { state: 'working', prompt: 'remote prompt', agentType: 'gemini' }
        }),
        toolPayload: {
