...
async function deployAndLaunchRelayAttempt(
  ...
  if (!hostPlatform) {
    throw new Error(
      'Unsupported remote platform. BarkOS relay supports: linux-x64, linux-arm64, darwin-x64, darwin-arm64, win32-x64, win32-arm64.'
    )
  }
  ...
  if (!localRelayDir) {
    throw new Error(
      `Relay package for ${platform} not found locally. ` +
        `This may be a packaging issue — try reinstalling BarkOS.`
    )
  }
