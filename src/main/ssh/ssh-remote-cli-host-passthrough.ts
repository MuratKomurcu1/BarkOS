...
export async function runHostOrcaCliPassthrough(
  ...
  if (!entryExists(cliEntryPath)) {
    throw new HostCliUnavailableError(`BarkOS CLI entry not found at ${cliEntryPath}`)
  }
  ...
  return await new Promise<RemoteOrcaCliResult>((resolve, reject) => {
    ...
    const killTimer = setTimeout(() => {
      ...
      resolve({
        ...
        stderr: `${stderr.toString()}BarkOS CLI bridge timed out after ${killTimeoutMs}ms on the host.\n`,
        exitCode: 1
      })
    }, killTimeoutMs)
    killTimer.unref?.()
    ...
    child.on('error', (err) => {
      ...
      reject(
        new HostCliUnavailableError(`Failed to launch the BarkOS CLI on the host: ${err.message}`)
      )
    })
