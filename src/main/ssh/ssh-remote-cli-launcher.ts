...
internal static class OrcaRemoteCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            ...
            {
                Console.Error.WriteLine("BarkOS SSH CLI bridge cannot find Node.js at \"{0}\"", nodePath);
                return 1;
            }
            if (!File.Exists(relayPath))
            {
                Console.Error.WriteLine("BarkOS SSH CLI bridge cannot find the relay at \"{0}\"", relayPath);
                return 1;
            }
        ...
        {
            Console.Error.WriteLine("Unable to start the BarkOS SSH CLI bridge: {0}", error.Message);
            return 1;
        }
    }
    ...
    private static string BuildArguments(string relayPath, string socketPath, string credentialFile, string[] args)
    {
        ...
        AppendArgument(commandLine, "--barkos-cli");
...
function createWindowsLauncherCompileCommand(
  ...
  return powerShellCommand(
    [
      ...
      "if (-not $compiler) { Write-Error 'Unable to find the .NET Framework C# compiler required for the BarkOS SSH CLI launcher.'; exit 1 }",
      `& $compiler ${compilerArgs}`,
      'if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }',
      `if (-not (Test-Path -LiteralPath ${powerShellLiteral(launcherPath)} -PathType Leaf)) { Write-Error 'The BarkOS SSH CLI launcher compiler produced no executable.'; exit 1 }`,
    ...
    files: [
      {
        ...
        contents: [
          ...
          '  echo "BarkOS SSH CLI bridge cannot find the relay socket: $ORCA_RELAY_SOCKET_PATH" >&2',
          '  exit 1',
          'fi',
          'exec "$ORCA_RELAY_NODE_PATH" "$ORCA_RELAY_DIR/relay.js" --sock-path "$ORCA_RELAY_SOCKET_PATH" --credential-file "$ORCA_RELAY_CREDENTIAL_FILE" --barkos-cli "$@"',
          ''
        ].join('\n')
      }
    ],
