...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'com.barkos.desktop.computer-use'
...
async function openComputerUsePermissionsAsync(
  ...
  if (!helperAppPath) {
    throw new RuntimeClientError('accessibility_error', 'BarkOS Computer Use.app was not found')
  }
...
function closeExistingPermissionHelpers(): void {
  ...
  const setupHelperPatterns = [
    'barkos-computer-use-macos[[:space:]]+--permission([[:space:]]|$)',
    'barkos-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'
  ]
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to BarkOS Computer Use, then retry get-app-state.`
}
