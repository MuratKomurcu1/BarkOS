...
async function getComputerUsePermissionStatusAsync(): Promise<ComputerUsePermissionStatusResult> {
  ...
  if (!helperAppPath) {
    return createUnavailablePermissionStatus('BarkOS Computer Use.app was not found', null)
  }

  const executablePath = resolveMacOSComputerUseExecutablePath()
  if (!executablePath) {
    return createUnavailablePermissionStatus(
      `${helperAppPath}/Contents/MacOS/barkos-computer-use-macos was not found`,
      helperAppPath
    )
  }
