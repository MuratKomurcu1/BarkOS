...
export function resolveMacOSComputerUseAppPath(): string | null {
  ...
  const packaged = [join(process.resourcesPath ?? '', 'BarkOS Computer Use.app')]
  const dev = [
    join(process.cwd(), 'native/computer-use-macos/.build/release/BarkOS Computer Use.app'),
    resolve(__dirname, '../../native/computer-use-macos/.build/release/BarkOS Computer Use.app')
  ]
...
export function resolveMacOSComputerUseExecutablePath(): string | null {
  ...
  const executablePath = join(appPath, 'Contents', 'MacOS', 'barkos-computer-use-macos')
