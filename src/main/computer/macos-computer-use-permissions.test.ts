...
const helperAppPath = '/Applications/BarkOS Computer Use.app'
...
describe('openComputerUsePermissions', () => {
  ...
  beforeEach(() => {
    ...
    resolveHelperExecutablePathMock.mockReturnValue(
      '/Applications/BarkOS Computer Use.app/Contents/MacOS/barkos-computer-use-macos'
    )
  ...
  it('does not launch the setup helper when all permissions are granted', async () => {
    resolveHelperAppPathMock.mockReturnValue('/Applications/BarkOS Computer Use.app')

    await expect(openComputerUsePermissions()).resolves.toEqual({
      platform: 'darwin',
      helperAppPath: '/Applications/BarkOS Computer Use.app',
    ...
    expect(spawn).not.toHaveBeenCalledWith(
      '/usr/bin/open',
      ['-n', '/Applications/BarkOS Computer Use.app', '--args', '--permissions'],
  ...
  it('launches the helper app in permissions mode', async () => {
    ...
    await expect(openComputerUsePermissions()).resolves.toEqual({
      ...
      nextStep: 'Grant Screen Recording to BarkOS Computer Use, then retry get-app-state.'
    })
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/pkill',
      ['-f', 'barkos-computer-use-macos[[:space:]]+--permission([[:space:]]|$)'],
      { stdio: 'ignore' }
    )
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/pkill',
      ['-f', 'barkos-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'],
      { stdio: 'ignore' }
    )
  ...
  it('launches a targeted permission helper flow', async () => {
    ...
    await expect(openComputerUsePermissions('accessibility')).resolves.toEqual({
      ...
      nextStep: 'Grant Accessibility to BarkOS Computer Use, then retry get-app-state.'
    })
    expect(spawn).toHaveBeenCalledWith(
      '/usr/bin/open',
      ['-n', '/Applications/BarkOS Computer Use.app', '--args', '--permission', 'accessibility'],
      { detached: true, stdio: 'ignore' }
    )
  })
  ...
  it('throws when the helper app is missing on macOS', async () => {
    ...
    await expect(openComputerUsePermissions()).rejects.toThrow(
      'BarkOS Computer Use.app was not found'
    )
  })
  ...
  it('throws when the helper executable is missing during setup', async () => {
    ...
    await expect(openComputerUsePermissions('accessibility')).rejects.toThrow(
      '/Applications/BarkOS Computer Use.app/Contents/MacOS/barkos-computer-use-macos was not found'
    )
  })
