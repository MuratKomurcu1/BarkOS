  ...
  try {
    ...
    if (eligibility.reviewLookupOutcome === 'unavailable') {
      return {
        ok: false,
        code: 'validation',
        error: `Create ${copy.shortLabel} failed: BarkOS could not confirm whether this branch already has a ${copy.reviewLabel}. Retry once the ${copy.providerName} lookup succeeds.`
      }
    }
