...
describe('markCodexProjectTrusted', () => {
  it('trusts a project in the exact Codex runtime home selected for launch', () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), 'barkos-codex-selected-home-'))
    const workspace = join(fixtureRoot, 'workspace')
    const selectedHome = join(fixtureRoot, 'selected-account-home')
    try {
      mkdirSync(workspace, { recursive: true })

      markCodexProjectTrustedInHome(workspace, selectedHome)

      const written = readFileSync(join(selectedHome, 'config.toml'), 'utf-8')
