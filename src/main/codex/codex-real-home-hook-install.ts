...
function assertHooksJsonGeneration(
  ...
  if (currentRaw !== expectedRaw || resolveHooksJsonWritePath(hooksJsonPath) !== hooksWritePath) {
    ...
    throw new Error('Codex hooks.json changed while BarkOS prepared its trust repair')
  }
}
...
function sweepRealHomeCodexHook(): RealHomeCodexHookLane {
  ...
  if (removedAny) {
    ...
    try {
      ...
      console.warn('[codex-real-home-hooks] failed to drop BarkOS trust entries:', error)
    }
  }
  return 'removed'
}
