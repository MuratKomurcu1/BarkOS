...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    "// OpenCode's Node process (not BarkOS's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on BarkOS restart',
    ...
    '      // writeEndpointFile on the BarkOS side) allocates a fresh inode on',
    ...
    '  // frozen when OpenCode was fork()ed — stale after a BarkOS restart. The',
    '  // file is rewritten on every BarkOS start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / BarkOS',
    ...
    '// per turn through BarkOS (loopback HTTP -> main JSON parse -> status compare',
    ...
    '// flip the root BarkOS pane to done or overwrite the parent turn preview.',
    ...
    '  // rewritten by a newer BarkOS since the last call. Pane/tab/worktree IDs',
    '  // stay on process.env because they are per-PTY (stable for the life of',
    '  // the OpenCode process), not per-BarkOS-instance.',
    '  const coords = resolveHookCoords();',
    ...
    '        "X-BarkOS-Agent-Hook-Token": coords.token,',
    '      },',
    '      body,',
    ...
    '    // because BarkOS is unavailable or the local loopback request failed.',
    '    return false;',
    '  } finally {',
    '    clearTimeout(timeout);',
    '  }',
    '}',
    '',
    'const SIDE_EFFECT_GATE_TIMEOUT_MS = 8000;',
    'const SIDE_EFFECT_TRANSPORT_DENIAL =',
    '  "BarkOS approval service is unavailable; the side effect was blocked.";',
    'async function evaluateSideEffectGate(toolName, toolArgs) {',
    '  // Why: mirrors the Claude/Codex PreToolUse contract for BarkOS side-effect',
    '  // enforcement: POST the tool call to /hook/opencode, block by throwing on a',
    '  // structured deny decision, allow on a neutral body, and fail CLOSED on any',
    '  // transport ambiguity so an unreachable/stale approval service can never',
    '  // silently wave a side effect through. Enforcement env is frozen when',
    '  // OpenCode is fork()ed — toggling it requires restarting the pane.',
    '  if (process.env.ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT !== "1") return;',
    '  const coords = resolveHookCoords();',
    '  const paneKey = process.env.ORCA_PANE_KEY;',
    '  if (!coords.port || !coords.token || !paneKey) {',
    '    throw new Error(SIDE_EFFECT_TRANSPORT_DENIAL);',
    '  }',
    '  let body;',
    '  try {',
    '    body = JSON.stringify({',
    ...
    '      env: coords.env,',
    '      version: coords.version,',
    '      barkosSideEffectEnforcement:',
    '        process.env.ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT || "",',
    '      payload: {',
    '        hook_event_name: "PreToolUse",',
    '        tool_name: typeof toolName === "string" ? toolName : "",',
    '        tool_input: toolArgs && typeof toolArgs === "object" ? toolArgs : {},',
    '      },',
    '    });',
    '  } catch {',
    '    throw new Error(SIDE_EFFECT_TRANSPORT_DENIAL);',
    ...
    `  const url = \`http://127.0.0.1:\${coords.port}${hookPathname}\`;`,
    '  const controller = new AbortController();',
    '  const timeout = setTimeout(() => controller.abort(), SIDE_EFFECT_GATE_TIMEOUT_MS);',
    '  if (timeout.unref) timeout.unref();',
    '  try {',
    '    let text;',
    '    try {',
    ...
    '      if (!response.ok) throw new Error(SIDE_EFFECT_TRANSPORT_DENIAL);',
    '    } catch (err) {',
    '      // Why: any transport ambiguity (connection refused, gate-timeout abort, non-2xx)',
    '      // denies with the stable transport message; the marked denial passes through.',
    '      throw err instanceof Error && err.message === SIDE_EFFECT_TRANSPORT_DENIAL ? err : new Error(SIDE_EFFECT_TRANSPORT_DENIAL);',
    '    }',
    '    // Why: enforced requests always get a decision body from BarkOS; an empty',
    '    // body means the peer cannot evaluate opencode (too-old BarkOS/relay),',
    '    // which must deny instead of silently allowing.',
    '    if (!text) throw new Error(SIDE_EFFECT_TRANSPORT_DENIAL);',
    '    let decision;',
    '    try {',
    ...
    '    }',
    '    const output =',
    '      decision && typeof decision === "object" ? decision.hookSpecificOutput : null;',
    '    if (output && output.permissionDecision === "deny") {',
    '      throw new Error(',
    '        (typeof output.permissionDecisionReason === "string" &&',
    '          output.permissionDecisionReason) ||',
    '          SIDE_EFFECT_TRANSPORT_DENIAL',
    '      );',
    '    }',
    '  } finally {',
    '    clearTimeout(timeout);',
    ...
    '    await evaluateSideEffectGate(input && input.tool, output && output.args);',
    '  },',
    '  dispose: async () => {',
    '    if (disposed) return;',
    '    disposed = true;',
    ...
    '          // publish a final idle so BarkOS does not retain a dead owner.',
...
export class OpenCodeHookService {
  ...
  /** Why: BarkOS side-effect enforcement must fail closed — true only when
   *  Orca's gate/status plugin file is present in the config root a PTY will
   *  actually load, so an overlay write failure cannot launch an enforced
   *  opencode with no approval path at all. */
  isPluginInstalledIn(configDir: string): boolean {
    try {
      return existsSync(join(configDir, 'plugins', ORCA_OPENCODE_PLUGIN_FILE))
    } catch {
      return false
    }
  }
