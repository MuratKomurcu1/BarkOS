...
export function buildCommandCodeManagedScript(
  ...
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      'if exist "%APPDATA%\\barkos-dev\\agent-hooks" for /r "%APPDATA%\\barkos-dev\\agent-hooks" %%F in (endpoint.cmd) do call :maybeSourceEndpoint "%%~fF"',
      'if "%ORCA_AGENT_HOOK_TOKEN%"=="" if exist "%APPDATA%\\barkos\\agent-hooks" for /r "%APPDATA%\\barkos\\agent-hooks" %%F in (endpoint.cmd) do call :maybeSourceEndpoint "%%~fF"',
      'exit /b 0',
      ':maybeSourceEndpoint',
  ...
  return [
    ...
    '# still has BarkOS pane/hook metadata, so recover it before posting.',
    ...
    '  for endpoint in \\',
    '    "$HOME/Library/Application Support/barkos-dev/agent-hooks"/*/endpoint.env \\',
    '    "$HOME/Library/Application Support/barkos-dev/agent-hooks/endpoint.env" \\',
    '    "${XDG_CONFIG_HOME:-$HOME/.config}/barkos-dev/agent-hooks"/*/endpoint.env \\',
    '    "${XDG_CONFIG_HOME:-$HOME/.config}/barkos-dev/agent-hooks/endpoint.env" \\',
    '    "$HOME/Library/Application Support/barkos/agent-hooks/endpoint.env" \\',
    '    "${XDG_CONFIG_HOME:-$HOME/.config}/barkos/agent-hooks/endpoint.env"; do',
    '    [ -r "$endpoint" ] || continue',
