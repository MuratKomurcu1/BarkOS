...
function buildAndApplyMenu(options: RegisterAppMenuOptions): void {
  ...
  const featureTourItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.exploreOrca', 'Explore BarkOS'),
    click: (_menuItem, window) => onOpenFeatureTour(window)
  }

  const setupGuideItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.gettingStarted', 'Getting Started with BarkOS'),
  ...
  // redundant "BarkOS" entry with roles that don't apply, so we omit it there
  // and distribute its items across File / Help instead.
  const macAppMenu: Electron.MenuItemConstructorOptions = {
    label: options.appMenuLabel ?? app.name,
    submenu: [
      { role: 'about', label: translateMain('menu.aboutBarkos', 'About BarkOS') },
      checkForUpdatesItem,
      settingsItem,
      { type: 'separator' },
      { role: 'services', label: translateMain('menu.services', 'Services') },
      { type: 'separator' },
      { role: 'hide', label: translateMain('menu.hideBarkos', 'Hide BarkOS') },
      { role: 'hideOthers', label: translateMain('menu.hideOthers', 'Hide Others') },
      { role: 'unhide', label: translateMain('menu.showAll', 'Show All') },
      { type: 'separator' },
      { role: 'quit', label: translateMain('menu.quitBarkos', 'Quit BarkOS') }
    ]
  }
  ...
  const appearanceSubmenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        label: translateMain('menu.showMobileButton', 'Show BarkOS Mobile Button'),
        type: 'checkbox',
  ...
  const viewMenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        role: 'toggleDevTools',
        label: translateMain('menu.toggleDeveloperTools', 'Toggle Developer Tools')
      },
      { type: 'separator' },
      {
  ...
  const windowMenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      { role: 'zoom', label: translateMain('menu.zoomWindow', 'Zoom') }
    ]
  }
