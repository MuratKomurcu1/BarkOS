...
let releaseUpdatesEnabled = true
...
export function getRemoteServerUpdateSupport(): RemoteServerUpdateSupport {
  ...
  if (!autoUpdaterInitialized || !releaseUpdatesEnabled) {
    return {
      installMode: updateInstallMode,
      automatic: false,
...
export function setupAutoUpdater(
  ...
  releaseUpdatesEnabled = opts?.releaseUpdatesEnabled ?? true
  ...
  if (activeUpdateSource === 'release' && releaseUpdatesEnabled) {
    autoUpdater.allowDowngrade = false
    autoUpdater.disableDifferentialDownload = false
  }
