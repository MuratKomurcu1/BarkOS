...
const STAGING_ROOT = join(TEMP_ROOT, `barkos-clipboard-files${UID_SUFFIX}`)
...
const LEGACY_EXPIRED = `barkos-clipboard-file-${EXPIRED_TRANSFER}`
const LEGACY_FRESH = `barkos-clipboard-file-${FRESH_TRANSFER}`
...
describe('legacy remote clipboard staging compatibility', () => {
  ...
  it('removes only exact expired legacy children encountered inside the bound', async () => {
    opendirMock.mockResolvedValue(
      openedDirectory([
        directoryEntry('barkos-clipboard-file-lookalike'),
