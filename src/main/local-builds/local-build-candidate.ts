...
function extractCompatibility(zipFile: FileHandle): Promise<string> {
  return new Promise((resolve, reject) => {
    const child = spawn(
      '/usr/bin/unzip',
      ['-p', '/dev/fd/3', `BarkOS.app/Contents/Resources/${LOCAL_BUILD_COMPATIBILITY_FILENAME}`],
...
async function validateArtifact(
  ...
  try {
    ...
    if (compatibility.appId !== BARKOS_APP_ID || compatibility.version !== manifestVersion) {
...
export async function loadLocalBuildCandidate(
  ...
  if (matching.length !== 1) {
    await Promise.all(validated.map((entry) => entry.file.close()))
    throw new Error(`The manifest must contain exactly one ${architecture} BarkOS ZIP.`)
  }
