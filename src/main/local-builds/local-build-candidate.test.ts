...
async function fixture(options: { sha512?: string; url?: string } = {}) {
  const directory = await mkdtemp(join(tmpdir(), 'barkos-local-build-'))
  tempDirectories.push(directory)
  const artifactName = 'barkos-macos-arm64.zip'
...
describe('loadLocalBuildCandidate', () => {
  it('returns a sanitized, architecture-specific feed after hash validation', async () => {
    ...
    expect([...candidate.artifacts.keys()]).toEqual(['barkos-macos-arm64.zip'])
    expect(candidate.manifestContent).toContain('barkos-macos-arm64.zip')
  ...
  it('rejects mismatched hashes and traversal paths', async () => {
    ...
    const traversal = await fixture({ url: '../barkos-macos-arm64.zip' })
    await expect(
  ...
  it('serves the same artifact descriptor that passed validation', async () => {
    ...
    try {
      await expect(
        fetch(`${feed.url}barkos-macos-arm64.zip`).then((response) => response.text())
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const directory = await mkdtemp(join(tmpdir(), 'barkos-local-build-zip-'))
      tempDirectories.push(directory)
      const zipRoot = join(directory, 'zip-root')
      const resources = join(zipRoot, 'BarkOS.app', 'Contents', 'Resources')
      await mkdir(resources, { recursive: true })
      await writeFile(join(resources, 'barkos-local-build.json'), JSON.stringify(compatibility()))
      const artifactName = 'barkos-macos-arm64.zip'
      const artifactPath = join(directory, artifactName)
      await execFileAsync('/usr/bin/zip', ['-qry', artifactPath, 'BarkOS.app'], { cwd: zipRoot })
  ...
  it('requires exactly one ZIP for the running architecture', async () => {
    ...
    ).rejects.toThrow('exactly one x64 BarkOS ZIP')
  })
})
