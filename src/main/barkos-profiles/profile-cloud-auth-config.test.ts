...
describe('Orca cloud auth config', () => {
  it('reports unconfigured without both API URL and client ID', () => {
    expect(getOrcaCloudAuthConfig({})).toEqual({
      configured: false,
      setupMessage: 'BarkOS Cloud sign-in is not configured for this build.'
    })
  })
  ...
  it('stays unconfigured in packaged builds without explicit endpoints', () => {
    // Why: BarkOS has no first-party cloud, so packaged builds must not fall
    // back to the upstream product's auth server (handoff #4).
  ...
  it('configures packaged builds from explicit env endpoints', () => {
    const state = getOrcaCloudAuthConfig(
      {
        ORCA_CLOUD_API_URL: 'https://cloud.barkos.example',
        ORCA_CLOUD_CLIENT_ID: 'barkos-desktop'
      },
      true
    )
    ...
    expect(state.config.apiBaseUrl).toBe('https://cloud.barkos.example')
    expect(state.config.clientId).toBe('barkos-desktop')
    expect(state.config.relayDirectorUrl).toBe('https://cloud.barkos.example')
  })
