...
describe('convergableSkillNames', () => {
  ...
  it('drops a skill whose lock records a revision the disk does not have', () => {
    const result = convergableSkillNames(
      [placement('barkos-linear', 1)],
      new Map([['barkos-linear', '091d9bcc']]),
      { 'barkos-linear': [PRE_STUB, STUB] }
    )
  ...
  it('keeps a skill whose lock matches disk even when it is outdated', () => {
    const result = convergableSkillNames(
      [placement('barkos-cli', 1)],
      new Map([['barkos-cli', 'aaaa1111']]),
      { 'barkos-cli': [revision(1, 'aaaa1111')] }
    )
    expect([...result]).toEqual(['barkos-cli'])
  })

  it('keeps a skill whose disk content matches no known revision', () => {
    const result = convergableSkillNames(
      [placement('barkos-cli', null)],
      new Map([['barkos-cli', 'aaaa1111']]),
      { 'barkos-cli': [revision(1, 'bbbb2222')] }
    )
    expect([...result]).toEqual(['barkos-cli'])
  })
  ...
  it('keeps a skill with no observable placement', () => {
    const result = convergableSkillNames(
      [placement('barkos-cli', null, 'canonical-copy', null)],
      new Map([['barkos-cli', 'aaaa1111']]),
      { 'barkos-cli': [revision(1, 'aaaa1111')] }
    )
    expect([...result]).toEqual(['barkos-cli'])
  ...
  it('drops a stale skill whose folder holds files no revision lists', () => {
    const result = convergableSkillNames(
      [placement('barkos-linear', 1, 'canonical-copy', 'digest-with-sidecar')],
      new Map([['barkos-linear', '091d9bcc']]),
      { 'barkos-linear': [PRE_STUB, STUB] }
    )
    expect([...result]).toEqual([])
  ...
  it('keeps a skill when any placement still matches the lock', () => {
    const result = convergableSkillNames(
      [placement('barkos-cli', 1), placement('barkos-cli', 2)],
      new Map([['barkos-cli', 'aaaa1111']]),
      { 'barkos-cli': [revision(1, 'aaaa1111'), revision(2, 'f3727995')] }
    )
    expect([...result]).toEqual(['barkos-cli'])
  })
  ...
  it('keeps a skill whose lock names no revision we know', () => {
    const result = convergableSkillNames(
      [placement('barkos-cli', 1)],
      new Map([['barkos-cli', 'not-a-known-tree']]),
      { 'barkos-cli': [revision(1, 'f3727995')] }
    )
    expect([...result]).toEqual(['barkos-cli'])
  })
  ...
  it('keeps a skill when one placement is stale but another is unidentifiable', () => {
    const result = convergableSkillNames(
      [placement('barkos-cli', 1), placement('barkos-cli', null)],
      new Map([['barkos-cli', '091d9bcc']]),
      { 'barkos-cli': [PRE_STUB, STUB] }
    )
    expect([...result]).toEqual(['barkos-cli'])
  })
  ...
  it('ignores an unidentifiable plugin-cache copy when judging the canonical', () => {
    const result = convergableSkillNames(
      [placement('barkos-linear', 1), placement('barkos-linear', null, 'plugin-cache')],
      new Map([['barkos-linear', '091d9bcc']]),
      { 'barkos-linear': [PRE_STUB, STUB] }
    )
    expect([...result]).toEqual([])
  ...
  it('ignores a plugin-cache copy that matches the lock', () => {
    const result = convergableSkillNames(
      [placement('barkos-linear', 1), placement('barkos-linear', 2, 'plugin-cache')],
      new Map([['barkos-linear', '091d9bcc']]),
      { 'barkos-linear': [PRE_STUB, STUB] }
    )
    expect([...result]).toEqual([])
  ...
  it('judges each locked skill independently', () => {
    const result = convergableSkillNames(
      [placement('barkos-linear', 1), placement('barkos-cli', 1)],
      new Map([
        ['barkos-linear', '091d9bcc'],
        ['barkos-cli', 'aaaa1111']
      ]),
      {
        'barkos-linear': [PRE_STUB, STUB],
