...
describe('runtime RPC startup failure reporting', () => {
  ...
  it('shows the CLI impact and local cause', async () => {
    ...
    expect(showMessageBoxMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        type: 'error',
        title: 'BarkOS CLI unavailable',
        message: "BarkOS couldn't start its local command transport.",
        detail: expect.stringMatching(
          /barkos status.*barkos terminal.*orchestration.*Cause: metadata write failed/s
        )
      })
    )
  })
  ...
  it.each([
    ['EACCES', "Check permissions on BarkOS's data folder"],
    ['EPERM', "Check permissions on BarkOS's data folder"],
    ...
    ['ENOENT', "BarkOS's data folder may be missing"],
  ...
  it('falls back to a plain restart when the cause is unrecognised', async () => {
    ...
    const detail = showMessageBoxMock.mock.calls[0]?.[1]?.detail as string
    expect(detail).toContain('Restart BarkOS to try again.')
    expect(detail).not.toContain("Check permissions on BarkOS's data folder")
  })
