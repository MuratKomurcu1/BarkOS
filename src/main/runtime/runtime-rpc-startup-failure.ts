...
> = {
  permission_denied: {
    ...
    fallback:
      "BarkOS couldn't write its runtime file. Check permissions on BarkOS's data folder, then restart."
  },
  storage_unavailable: {
    key: 'runtimeRpc.startupFailure.guidance.storageUnavailable',
    fallback: 'Your disk may be full or read-only. Free up space, then restart BarkOS.'
  },
  invalid_path: {
    key: 'runtimeRpc.startupFailure.guidance.invalidPath',
    fallback:
      "BarkOS's data folder may be missing, moved, or at a path that is too long. Restore it or use a shorter path, then restart BarkOS."
  },
  address_in_use: {
    key: 'runtimeRpc.startupFailure.guidance.addressInUse',
    fallback: 'Another process may be holding the port. Restart BarkOS to try again.'
  },
  unknown: {
    key: 'runtimeRpc.startupFailure.guidance.unknown',
    fallback: 'Restart BarkOS to try again.'
  }
}
...
function createRuntimeRpcStartupFailureDialogOptions(error: unknown): MessageBoxOptions {
  ...
  return {
    ...
    cancelId: 0,
    noLink: true,
    title: translateMain('runtimeRpc.startupFailure.title', 'BarkOS CLI unavailable'),
    message: translateMain(
      'runtimeRpc.startupFailure.message',
      "BarkOS couldn't start its local command transport."
    ),
    detail: translateMain(
      'runtimeRpc.startupFailure.detail',
      'BarkOS will continue to work, but commands such as barkos status, barkos terminal, and orchestration are unavailable for this session.\n\n{{guidance}}\n\nCause: {{cause}}',
