...
export const CreateAgentSessionParams: z.ZodType<RuntimeCreateAgentSessionRequest> = z
  .object({
    ...
    sideEffectApproval: z
      .object({ version: z.union([z.literal(1), z.literal(2), z.literal(3), z.literal(4)]) })
      .strict()
      .optional()
  })
  .strict()
  .superRefine((value, context) => {
    ...
    if (value.agent === 'droid' && value.sideEffectApproval?.version === 1) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['sideEffectApproval'],
        message: 'BarkOS paired Droid approval requires protocol version 2'
      })
    } else if (
      value.sideEffectApproval !== undefined &&
      value.agent === 'gemini' &&
      value.sideEffectApproval.version !== 3
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['sideEffectApproval'],
        message: 'BarkOS paired Gemini approval requires protocol version 3'
      })
    } else if (
      value.agent === 'opencode' &&
      value.sideEffectApproval !== undefined &&
      value.sideEffectApproval.version !== 4
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['sideEffectApproval'],
        message: 'BarkOS paired OpenCode approval requires protocol version 4'
      })
    } else if (
      value.sideEffectApproval !== undefined &&
      value.agent !== 'claude' &&
      ...
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['sideEffectApproval'],
        message: 'BarkOS side-effect approval requires Claude, Codex, Droid, Gemini, or OpenCode'
      })
    }
  })
...
function callerContext(
  ...
  barkosSideEffectApprovalOwnerDeviceId?: string
): RuntimeAgentSessionRpcCaller {
  return {
    ...(clientId !== undefined ? { clientId } : {}),
    ...(clientKind !== undefined ? { clientKind } : {}),
    ...(signal ? { signal } : {}),
    ...(barkosSideEffectApprovalOwnerDeviceId ? { barkosSideEffectApprovalOwnerDeviceId } : {})
  }
}
...
export const AGENT_SESSION_METHODS: RpcAnyMethod[] = [
  ...
  defineMethod({
    ...
    handler: (params, { runtime, pairedDeviceId, clientId, clientKind, signal }) => {
      ...
      if (params.sideEffectApproval && (!pairedDeviceId || clientKind !== 'runtime')) {
        throw new Error('barkos_side_effect_approval_unauthorized')
      }
      return (runtime as AgentSessionRuntime).createAgentSession(
        withExecutionHostAgentPresentation(params, clientKind),
        callerContext(
          pairedDeviceId ?? clientId,
          clientKind,
          signal,
          params.sideEffectApproval ? pairedDeviceId : undefined
        )
      )
    }
  })
