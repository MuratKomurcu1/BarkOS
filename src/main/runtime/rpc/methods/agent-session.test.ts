...
describe('agent session RPC methods', () => {
  ...
  it('binds paired BarkOS approval ownership to the authenticated runtime device', async () => {
    const runtime = runtimeStub()
    const dispatcher = new RpcDispatcher({
      runtime: runtime as unknown as OrcaRuntimeService,
      methods: AGENT_SESSION_METHODS
    ...
    const params = {
      ...
      worktree: 'id:worktree-1',
      agent: 'codex',
      sideEffectApproval: { version: 1 }
    }

    const unauthorized = await dispatcher.dispatch(request('terminal.createAgentSession', params))
    expect(unauthorized).toMatchObject({ ok: false })

    const replies: RpcResponse[] = []
    await dispatcher.dispatchStreaming(
      request('terminal.createAgentSession', params),
      (response) => replies.push(JSON.parse(response) as RpcResponse),
      { pairedDeviceId: 'paired-runtime', clientKind: 'runtime' }
    )

    expect(replies[0]).toMatchObject({ ok: true })
    expect(runtime.createAgentSession).toHaveBeenCalledWith(params, {
      clientId: 'paired-runtime',
      clientKind: 'runtime',
      barkosSideEffectApprovalOwnerDeviceId: 'paired-runtime'
    })
  })

  it('requires the v2 contract for paired Droid sessions', async () => {
    const runtime = runtimeStub()
    const dispatcher = new RpcDispatcher({
      runtime: runtime as unknown as OrcaRuntimeService,
      methods: AGENT_SESSION_METHODS
    ...
    const replies: RpcResponse[] = []
    const accepted = { ...base, sideEffectApproval: { version: 2 } }
    await dispatcher.dispatchStreaming(
      request('terminal.createAgentSession', accepted),
      (response) => replies.push(JSON.parse(response) as RpcResponse),
      { pairedDeviceId: 'paired-runtime', clientKind: 'runtime' }
    )
    expect(replies[0]).toMatchObject({ ok: true })
    expect(runtime.createAgentSession).toHaveBeenCalledWith(accepted, {
      clientId: 'paired-runtime',
      clientKind: 'runtime',
      barkosSideEffectApprovalOwnerDeviceId: 'paired-runtime'
    })
  ...
  it('requires the v3 contract for paired Gemini sessions', async () => {
    const runtime = runtimeStub()
    const dispatcher = new RpcDispatcher({
      runtime: runtime as unknown as OrcaRuntimeService,
      methods: AGENT_SESSION_METHODS
    ...
    const replies: RpcResponse[] = []
    const accepted = { ...base, sideEffectApproval: { version: 3 } }
    await dispatcher.dispatchStreaming(
      request('terminal.createAgentSession', accepted),
      (response) => replies.push(JSON.parse(response) as RpcResponse),
      { pairedDeviceId: 'paired-runtime', clientKind: 'runtime' }
  ...
  it('requires the v4 contract for paired OpenCode sessions', async () => {
    const runtime = runtimeStub()
    const dispatcher = new RpcDispatcher({
      runtime: runtime as unknown as OrcaRuntimeService,
      methods: AGENT_SESSION_METHODS
    ...
    const replies: RpcResponse[] = []
    const accepted = { ...base, sideEffectApproval: { version: 4 } }
    await dispatcher.dispatchStreaming(
      request('terminal.createAgentSession', accepted),
      (response) => replies.push(JSON.parse(response) as RpcResponse),
      { pairedDeviceId: 'paired-runtime', clientKind: 'runtime' }
