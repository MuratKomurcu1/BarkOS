  ...
  describe('orchestration.dispatch', () => {
    ...
    it('uses caller-provided dev mode for injected preamble', async () => {
      ...
      expect(send).toHaveBeenCalledWith(
        'term_a',
        expect.stringContaining('barkos-dev orchestration send')
      )
    })
    ...
    it('injects bounded supplemental context and returns its integrity receipt', async () => {
      setup()
      provideInjectIdentity()
      const task = db.createTask({ spec: 'work' })
      vi.spyOn(runtime, 'isTerminalRunningAgent').mockResolvedValue(true)
      ...
      const result = (await call('orchestration.dispatch', {
        ...
        to: 'term_a',
        inject: true,
        supplementalContext: 'Approved project memory.',
        contextReceiptId: 'memory-assignment-1'
      })) as {
        contextReceipt: { id: string; sha256: string; characterCount: number }
      }

      expect(agentPrompt).toHaveBeenCalledWith(
        'term_a',
        expect.stringContaining('=== SUPPLEMENTAL CONTEXT ===\nReference only.')
      )
      expect(result.contextReceipt).toEqual({
        id: 'memory-assignment-1',
        sha256: '233b9471a32ab640589574423e190fb90d3267ecb333aecbc5045c2c5c277c09',
        characterCount: 24
      })
    })
