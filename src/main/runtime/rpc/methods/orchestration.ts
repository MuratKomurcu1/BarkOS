...
const DispatchParams = z
  .object({
    ...
    devMode: OptionalBoolean,
    run: OptionalString,
    supplementalContext: z.string().trim().min(1).max(8_000).optional(),
    contextReceiptId: z
      .string()
      .min(1)
      .max(64)
  ...
  .superRefine((params, context) => {
    if ((params.supplementalContext === undefined) !== (params.contextReceiptId === undefined)) {
      context.addIssue({
        code: 'custom',
        message: 'Supplemental context and receipt id must be supplied together'
      })
    }
  })
  ...
  defineMethod({
    ...
    handler: async (
      ...
      const contextReceipt =
        injected && params.supplementalContext && params.contextReceiptId
          ? {
              id: params.contextReceiptId,
              sha256: createHash('sha256').update(params.supplementalContext).digest('hex'),
              characterCount: params.supplementalContext.length
            }
          : undefined
      ...
      if (params.returnPreamble) {
        return { dispatch: ctx, injected, preamble, contextReceipt }
      }
      return { dispatch: ctx, injected, contextReceipt }
    }
  }),
