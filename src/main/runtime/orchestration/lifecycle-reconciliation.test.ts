...
describe('lifecycle reconciliation', () => {
  ...
  it('preserves a validated BarkOS staffing proposal in the settled task result', () => {
    db = new OrchestrationDb(':memory:')
    const task = db.createTask({ spec: 'plan work' })
    const dispatch = db.createDispatchContext(task.id, 'term_worker', `tab_w:${LEAF_A}`)
    const staffingProposal = {
      version: 1,
      summary: 'Arayüz çalışanı gerekli.',
      roles: [
        {
          key: 'frontend',
          name: 'Arayüz Mühendisi',
          mission: 'Arayüzü uygula.',
          capabilities: ['react'],
          definitionOfDone: ['Testler geçti.'],
          instructions: null
        }
      ],
      workers: [{ name: 'Lina', roleKey: 'frontend' }],
      tasks: [
        {
          key: 'ui',
          title: 'Arayüzü uygula',
          spec: 'Ekranı tamamla.',
          roleKey: 'frontend',
          dependencyKeys: [],
    ...
    const message = db.insertMessage({
      ...
      to: 'term_coordinator',
      subject: 'Ekip hazır',
      body: 'Gereksinimler incelendi. Ekip belirlendi. Plan hazırlandı.',
      type: 'worker_done',
      payload: JSON.stringify({
        taskId: task.id,
        dispatchId: dispatch.id,
    ...
    expect(reconcileLifecycleMessage(db, message).action).toBe('completed')
    expect(JSON.parse(db.getTask(task.id)?.result ?? '{}')).toMatchObject({ staffingProposal })
  })
