...
describe('buildDispatchPreamble', () => {
  ...
  it('places supplemental context after the task with an authority boundary', () => {
    const result = buildDispatchPreamble(
      baseParams({ supplementalContext: 'Approved project memory.' })
    )

    expect(result).toContain(
      '=== TASK ===\nImplement the login form\n\n=== SUPPLEMENTAL CONTEXT ==='
    )
    expect(result).toContain('The TASK and explicit coordinator or user authority take precedence.')
    expect(result).toContain('Approved project memory.')
  })
