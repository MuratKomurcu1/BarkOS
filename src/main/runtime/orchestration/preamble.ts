...
export function buildDispatchPreamble(params: PreambleParams): string {
  ...
  const header = `You are working inside BarkOS, a multi-agent workspace. You are a dispatched worker.
...
${postDoneInstructions}`
  ...
  const supplementalContext = params.supplementalContext
    ? `

=== SUPPLEMENTAL CONTEXT ===
Reference only. The TASK and explicit coordinator or user authority take precedence.
...
=== TASK ===
${params.taskSpec}${supplementalContext}`
}
...
prompt for BarkOS to reuse; if the coordinator has more for you it will
