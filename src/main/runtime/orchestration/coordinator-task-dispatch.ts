...
export async function dispatchTaskToWorker(params: {
  ...
  // Why: dispatched agents use barkos-dev in dev mode to reach the dev runtime's socket, not production (Section 6.4).
  const preamble = buildDispatchPreamble({
    ...
    devMode: process.env.ORCA_USER_DATA_PATH?.includes('barkos-dev'),
