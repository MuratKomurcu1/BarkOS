...
import { shouldUseOneShotRuntimeRequest } from './runtime-environment-one-shot-method'
import {
...
import {
  ...
  sendRemoteRuntimeRequestWithStatusPreflightAbortable,
  sendRemoteRuntimeSharedControlRequestAbortable,
  type RuntimeEnvironmentRequestOptions
...
export async function callRuntimeEnvironment(
  ...
  options?: RuntimeEnvironmentRequestOptions
    ...
    return await enqueueRuntimeCall(
      ...
      async () => {
        ...
        if (options?.validateStatus) {
          const response = await sendRemoteRuntimeRequestWithStatusPreflightAbortable(
            pairing,
            method,
            params,
            effectiveTimeoutMs,
        ...
        if (
          method !== 'status.get' &&
          !shouldUseOneShotRuntimeRequest(method) &&
