...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    it('blocks an enforced native Droid worker until its managed approval hook is ready', () => {
      const status = vi.spyOn(droidHookService, 'getStatus').mockReturnValue({
        agent: 'droid',
        state: 'not_installed',
      ...
      const options = {
        ...
        launchAgent: 'droid' as const
      }

      try {
        expect(() =>
          buildPtyHostEnv('pty-barkos-droid', { ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' }, options)
        ).toThrow('managed approval hook is installed')
    ...
    it('blocks an enforced native Gemini worker until its BeforeTool hook is ready', () => {
      const status = vi.spyOn(geminiHookService, 'getStatus').mockReturnValue({
        agent: 'gemini',
        state: 'not_installed',
    ...
    it('blocks an enforced native OpenCode worker until its gate plugin is installed', () => {
      const options = {
        isPackaged: true,
        userDataPath: '/tmp/orca-user-data',
        selectedCodexHomePath: null,
      ...
      try {
        ...
        ).toThrow('only with managed hooks enabled')
        openCodeIsPluginInstalledMock.mockReturnValue(false)
        expect(() =>
          buildPtyHostEnv(
            'pty-barkos-opencode',
            { ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' },
        ...
        ).toThrow('approval plugin is installed')
        expect(openCodeIsPluginInstalledMock).toHaveBeenCalledWith('/tmp/orca-opencode-config')
        openCodeIsPluginInstalledMock.mockReturnValue(true)
        const env = buildPtyHostEnv(
          'pty-barkos-opencode-ready',
          { ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' },
          options
        )
        expect(env.ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT).toBe('1')
      } finally {
        openCodeIsPluginInstalledMock.mockReset()
        openCodeIsPluginInstalledMock.mockReturnValue(true)
      }
    })

    it('blocks enforced Droid inside WSL until its guest approval hook is ready', () => {
      const ensure = vi.spyOn(wslHookRelayManager, 'ensureForDistro').mockImplementation(() => {})
      const readiness = vi
        .spyOn(wslHookRelayManager, 'isSideEffectApprovalReady')
        .mockReturnValue(false)
      const localStatus = vi.spyOn(droidHookService, 'getStatus')
      const options = {
        isPackaged: true,
        userDataPath: '/tmp/orca-user-data',
        selectedCodexHomePath: null,
      ...
      try {
        expect(() =>
          buildPtyHostEnv(
            'pty-barkos-droid-wsl',
            { ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' },
            options
          )
        ).toThrow('WSL approval hook is installed')
        expect(readiness).toHaveBeenCalledWith('Ubuntu', 'droid')
        expect(localStatus).not.toHaveBeenCalled()

        readiness.mockReturnValue(true)
        expect(() =>
          buildPtyHostEnv(
            'pty-barkos-droid-wsl-ready',
            { ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' },
            options
          )
        ).not.toThrow()
      } finally {
        localStatus.mockRestore()
        readiness.mockRestore()
        ensure.mockRestore()
      }
    })

    it('blocks enforced OpenCode inside WSL until its guest overlay is ready', () => {
      const ensure = vi.spyOn(wslHookRelayManager, 'ensureForDistro').mockImplementation(() => {})
      const overlay = vi.spyOn(wslHookRelayManager, 'getOpenCodeOverlayDir').mockReturnValue(null)
      const options = {
        isPackaged: true,
        userDataPath: '/tmp/orca-user-data',
        selectedCodexHomePath: null,
      ...
      try {
        expect(() =>
          buildPtyHostEnv(
            'pty-barkos-opencode-wsl',
            { ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' },
            options
          )
        ).toThrow('WSL approval plugin is installed')
        expect(overlay).toHaveBeenCalledWith('Ubuntu')
        expect(openCodeIsPluginInstalledMock).not.toHaveBeenCalled()
        overlay.mockReturnValue('/tmp/wsl-opencode-overlay')
        expect(() =>
          buildPtyHostEnv(
            'pty-barkos-opencode-wsl-ready',
            { ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' },
            options
          )
        ).not.toThrow()
    ...
    it.each(['codex', 'gemini'] as const)(
      'blocks an enforced WSL %s worker until its exact approval hook is ready',
      (launchAgent) => {
        const ensure = vi.spyOn(wslHookRelayManager, 'ensureForDistro').mockImplementation(() => {})
        const readiness = vi
          .spyOn(wslHookRelayManager, 'isSideEffectApprovalReady')
        ...
        try {
          expect(() =>
            buildPtyHostEnv('pty-barkos-wsl', { ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' }, options)
          ).toThrow('WSL approval hook is installed')
          expect(readiness).toHaveBeenCalledWith('Ubuntu', launchAgent)
          readiness.mockReturnValue(true)
          expect(() =>
            buildPtyHostEnv(
              'pty-barkos-wsl-ready',
              { ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' },
              options
            )
          ).not.toThrow()
