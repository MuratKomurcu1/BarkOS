...
export function buildPtyHostEnv(
  ...
  if (
    baseEnv.ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT === '1' &&
    (opts.launchAgent === 'droid' || opts.launchAgent === 'gemini')
  ) {
    if (!opts.agentStatusHooksEnabled) {
      throw new Error(`BarkOS can start ${opts.launchAgent} only with managed hooks enabled.`)
    }
    if (opts.isWsl !== true) {
      const status =
        opts.launchAgent === 'droid' ? droidHookService.getStatus() : geminiHookService.getStatus()
      if (status.state !== 'installed' || !status.managedHooksPresent) {
        throw new Error(
          `BarkOS cannot start ${opts.launchAgent} until its managed approval hook is installed.`
        )
      }
    }
  }
  if (
    baseEnv.ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT === '1' &&
    opts.launchAgent === 'opencode' &&
    opts.isWsl !== true
  ) {
    // Why: opencode's gate is the Orca-shipped plugin, so an enforced launch is
    // only safe when the config root this PTY loads actually contains it — a
    // failed overlay write must fail closed, not silently drop approvals.
    if (!opts.agentStatusHooksEnabled) {
      throw new Error('BarkOS can start opencode only with managed hooks enabled.')
    }
    const configDir = baseEnv.ORCA_OPENCODE_CONFIG_DIR || baseEnv.OPENCODE_CONFIG_DIR
    if (!configDir || !openCodeHookService.isPluginInstalledIn(configDir)) {
      throw new Error('BarkOS cannot start opencode until its approval plugin is installed.')
    }
  }
  if (opts.agentStatusHooksEnabled) {
    ...
    if (opts.isWsl === true) {
      ...
      if (
        baseEnv.ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT === '1' &&
        (opts.launchAgent === 'claude' ||
          opts.launchAgent === 'codex' ||
          opts.launchAgent === 'droid' ||
          opts.launchAgent === 'gemini') &&
        !wslHookRelayManager.isSideEffectApprovalReady(distro, opts.launchAgent)
      ) {
        throw new Error(
          `BarkOS cannot start ${opts.launchAgent} until its WSL approval hook is installed. Retry after WSL setup completes.`
        )
      }
      if (baseEnv.ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT === '1' && opts.launchAgent === 'opencode') {
        // Why: the guest relay materializes opencode's gate plugin overlay, so
        // until it reports a dir the enforced launch would have no approval path.
        const wslOpencodeOverlay = wslHookRelayManager.getOpenCodeOverlayDir(distro)
        if (!wslOpencodeOverlay) {
          throw new Error(
            'BarkOS cannot start opencode until its WSL approval plugin is installed. Retry after WSL setup completes.'
          )
        }
      }
  ...
  if (opts.isWsl) {
    ...
    if (opts.isPackaged) {
      baseEnv.ORCA_CLI_COMMAND = 'barkos'
    } else {
  ...
  if (!opts.isPackaged) {
    ...
    // Why: expose the bundled BarkOS CLI inside managed PTYs without touching the user's `orca` command.
    const shimDir = ensureLinuxTerminalBarkosCliShimDir({ userDataPath: opts.userDataPath })
    if (shimDir) {
