...
function assertTrustedBarkosFailoverSender(event: IpcMainInvokeEvent): void {
  if (!isTrustedUIRenderer(event.sender)) {
    throw new Error('unauthorized_barkos_codex_failover_sender')
  }
}
...
export function registerCodexAccountHandlers(
  ...
  ipcMain.handle(
    ...
    (event, args: { accountId?: unknown; providerSession?: unknown }) => {
      assertTrustedBarkosFailoverSender(event)
      if (typeof args?.accountId !== 'string' || !args.accountId.trim()) {
        throw new Error('invalid_barkos_codex_failover_account')
      }
      return codexAccounts.prepareSelectedHostAccountFailoverResume(
        args.accountId,
