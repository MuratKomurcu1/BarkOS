...
const {
  ...
  sendRemoteRuntimeRequestWithStatusPreflightMock,
  ...
  sendRemoteRuntimeRequestWithStatusPreflightMock: vi.fn(),
...
vi.mock('../../shared/remote-runtime-client', () => ({
  ...
  sendRemoteRuntimeRequestWithStatusPreflight: sendRemoteRuntimeRequestWithStatusPreflightMock,
...
describe('registerRuntimeEnvironmentHandlers', () => {
  ...
  beforeEach(() => {
    ...
    sendRemoteRuntimeRequestWithStatusPreflightMock.mockReset()
  ...
  it('marks paired orchestration mutations with the current contract', async () => {
    registerRuntimeEnvironmentHandlers(store as never)
    sendRemoteRuntimeRequestMock.mockResolvedValue({
      id: 'run-create',
      ok: true,
    ...
    await call(null, {
      ...
      params: { objective: 'İncele', from: 'term-lead' }
    })

    expect(sendRemoteRuntimeRequestMock).toHaveBeenCalledWith(
      expect.any(Object),
      'orchestration.runCreate',
      { objective: 'İncele', from: 'term-lead' },
      15_000,
  ...
  it('uses one-shot transport so paired BarkOS test cancellation reaches the host', async () => {
    registerRuntimeEnvironmentHandlers(store as never)
    sendRemoteRuntimeRequestMock.mockResolvedValue({
      id: 'test-run',
      ok: true,
