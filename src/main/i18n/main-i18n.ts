...
import { brandBarkosVisibleCopy } from '../../shared/barkos-visible-brand'
...
> = {
  ...
  tr: () => import('../../renderer/src/i18n/locales/tr.json'),
...
export function translateMain(key: string, fallback: string, options?: TOptions): string {
  ...
  const value = brandBarkosVisibleCopy(typeof raw === 'string' && raw.length > 0 ? raw : fallback)
