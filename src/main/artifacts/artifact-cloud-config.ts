...
const PRODUCTION_ARTIFACTS_API_URL = 'https://muratkomurcu.com'
...
export function resolveArtifactCloudApiUrl(
  ...
  const firstParty =
    url.hostname === 'muratkomurcu.com' || url.hostname.endsWith('.muratkomurcu.com')
  ...
  if (!firstParty && !loopback) {
    throw new Error('Artifact API URLs must use a muratkomurcu.com or loopback host.')
  }
