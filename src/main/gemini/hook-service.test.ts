...
describe('GeminiHookService', () => {
  ...
  it.each([
    [{ enabled: false }, 'Gemini hooks are disabled in settings'],
    [
      { enabled: true, disabled: ['orca-managed-gemini'] },
      'BarkOS managed Gemini hook is disabled in settings'
    ]
  ])(
    'does not report approval readiness when hooksConfig disables the managed hook',
    (hooksConfig, detail) => {
      const configDir = join(homeDir, '.gemini')
      mkdirSync(configDir, { recursive: true })
      writeFileSync(
        join(configDir, 'settings.json'),
        `${JSON.stringify({ hooksConfig, hooks: {} }, null, 2)}\n`
      )
      ...
      const config = JSON.parse(readFileSync(join(configDir, 'settings.json'), 'utf8')) as {
        hooks: Record<string, { hooks: { name?: string }[] }[]>
      }
      expect(config.hooks.BeforeTool[0]?.hooks[0]?.name).toBe('orca-managed-gemini')
    }
  )
})
