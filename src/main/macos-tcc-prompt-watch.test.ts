...
describe('parseTccPromptEvent', () => {
  ...
  it('separates the accessing binary from the responsible app', () => {
    const event = parseTccPromptEvent(BARKOS_APPDATA_LINE)
    // The whole point of #9756: the dialog says BarkOS, but node did the access.
    expect(event?.responsibleIdentifier).toBe('com.barkos.desktop')
...
describe('isBarkosAttributedPrompt', () => {
  it('accepts the app and detached terminal helper across BarkOS build identities', () => {
    for (const id of [
      'com.barkos.desktop',
      'com.barkos.desktop.helper',
      'com.barkos.desktop.dev',
      'com.barkos.desktop.dev.helper',
      'com.barkos.desktop.local',
      'com.barkos.desktop.local.helper'
    ]) {
      expect(
        isBarkosAttributedPrompt({
  ...
  it('rejects unrelated services even when BarkOS is responsible', () => {
    expect(
      isBarkosAttributedPrompt({
        service: 'kTCCServiceMicrophone',
        accessingIdentifier: 'orca',
        responsibleIdentifier: 'com.barkos.desktop'
      })
    ).toBe(false)
  })
})
...
describe('MacosTccPromptWatch', () => {
  ...
  it('reports only BarkOS-attributed dialogs from a live stream', async () => {
    ...
    stdout.write(`${BARKOS_APPDATA_LINE}\n`)
