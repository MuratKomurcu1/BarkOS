...
const linuxLauncherAsset = new URL('../../../resources/linux/bin/barkos', import.meta.url)
const darwinLauncherAsset = new URL('../../../resources/darwin/bin/barkos', import.meta.url)
const unixLauncherFixtures = [
  {
    name: 'Linux',
    asset: linuxLauncherAsset,
    appDir: ['BarkOS'],
    launcher: ['resources', 'bin', 'barkos'],
    executable: ['barkos'],
  ...
  },
  {
    ...
    appDir: ['BarkOS.app'],
    launcher: ['Contents', 'Resources', 'bin', 'barkos'],
    executable: ['Contents', 'MacOS', 'BarkOS'],
...
`,
  ...
  itRunsUnixShell(
    ...
    async () => {
      ...
      try {
        const appDir = join(root, 'BarkOS')
        const resourcesDir = join(appDir, 'resources')
        const launcherDir = join(resourcesDir, 'bin')
        const cliDir = join(resourcesDir, 'app.asar.unpacked', 'out', 'cli')
        const launcherPath = join(launcherDir, 'barkos')
        const electronPath = join(appDir, 'barkos')
...
`,
        ...
        const commandPath = join(commandDir, 'barkos')
        await mkdir(commandDir, { recursive: true })
        await mkdir(join(homeDir, 'barkos'), { recursive: true })
  ...
  itRunsUnixShell('runs the AppImage CLI wrapper through APPDIR at runtime', async () => {
    ...
    try {
      const appDir = join(root, 'BarkOS.AppDir')
      const cliDir = join(appDir, 'resources', 'app.asar.unpacked', 'out', 'cli')
      const cliPath = join(cliDir, 'index.js')
      const appImagePath = join(root, "BarkOS's AppImage.AppImage")
      const commandPath = join(root, 'barkos')
      await mkdir(cliDir, { recursive: true })
      await writeFile(
        cliPath,
        `exports.main = (argv) => {
