...
describe('CliInstaller', () => {
  ...
  // Why: packaged app moves can leave a symlink to an older BarkOS-owned launcher;
  // those are safe to refresh, unlike arbitrary user symlinks.
  it.skipIf(process.platform === 'win32')(
    'replaces stale packaged BarkOS launcher symlinks',
    async () => {
      const fixture = await makeFixture()
  ...
  // Why: old dev/package experiments wrote a generated BarkOS launcher file
  // directly into /usr/local/bin/barkos. That broke profiling because Settings
  ...
  // machines; packaged BarkOS should treat that as stale, not a hard conflict.
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      for (const devLauncherName of ['barkos', 'barkos-dev']) {
        ...
        const installPath = join(commandDir, 'barkos')
        const userDataPath = join(caseRoot, 'barkos')
