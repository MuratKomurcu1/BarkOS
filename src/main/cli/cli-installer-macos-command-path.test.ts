...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    'falls back to ~/.local/bin/barkos on macOS when /usr/local/bin does not exist',
    async () => {
      ...
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'barkos')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
        userDataPath: fixture.userDataPath,
        execPath: '/Applications/BarkOS.app/Contents/MacOS/BarkOS',
      ...
      expect(status.commandPath).toBe(join(homePath, '.local', 'bin', 'barkos'))
      expect(status.state).toBe('not_installed')
      expect(status.supported).toBe(true)

      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandPath).toBe(join(homePath, '.local', 'bin', 'barkos'))
  ...
  it.skipIf(process.platform === 'win32')(
    'uses /usr/local/bin/barkos on macOS when /usr/local/bin exists',
    async () => {
      const fixture = await makeFixture()
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const installPath = join(usrLocalBin, 'barkos')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
  ...
  // Why: users can have a managed BarkOS command in ~/.local/bin even when
  // /usr/local/bin exists; Settings must follow the shell-visible command.
  it.skipIf(process.platform === 'win32')(
    'uses an existing managed macOS BarkOS command from the shell PATH before /usr/local/bin',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const defaultInstallPath = join(usrLocalBin, 'barkos')
      const userInstallPath = join(userLocalBin, 'barkos')
      const launcherPath = join(resourcesPath, 'bin', 'barkos')
      await mkdir(usrLocalBin, { recursive: true })
  ...
  it.skipIf(process.platform === 'win32')(
    'skips a broken managed macOS BarkOS symlink before /usr/local/bin',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // ~/.local/bin/barkos must not steal status from /usr/local/bin/barkos.
  it.skipIf(process.platform === 'win32')(
    'keeps the default macOS command when a managed BarkOS command appears later on PATH',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // is still the shell-visible BarkOS command until the default path is installed.
  it.skipIf(process.platform === 'win32')(
    'uses a later managed macOS BarkOS command when the default command is missing',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // Why: bash/zsh skip non-executable PATH entries even at BarkOS's configured
  // install slot, then keep looking for a runnable command later in PATH.
  it.skipIf(process.platform === 'win32')(
    'uses a later managed macOS BarkOS command when the default command is not executable',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(userInstallPath)
      await expect(readFile(defaultInstallPath, 'utf8')).resolves.toContain('other-barkos')
    }
  )

  // Why: a non-BarkOS command after an empty default install slot can be shadowed
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(installed.state).toBe('installed')
      await expect(readlink(defaultInstallPath)).resolves.toBe(launcherPath)
      await expect(readFile(userInstallPath, 'utf8')).resolves.toContain('other-barkos')
    }
  )

  // Why: an off-PATH ~/.local/bin/barkos must not hijack CLI registration and
  // leave the shell-visible /usr/local/bin command missing.
  it.skipIf(process.platform === 'win32')(
    'ignores managed macOS BarkOS commands that are not visible on the shell PATH',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  it.skipIf(process.platform === 'win32')(
    'reports a conflict for an unmanaged macOS BarkOS command that shadows the install path',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  it.skipIf(process.platform === 'win32')(
    'skips a non-executable unmanaged macOS BarkOS command before the install path',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // Why: macOS and Linux expose the same BarkOS command name.
  it.skipIf(process.platform === 'win32')(
    'reports commandName as barkos when falling back to ~/.local/bin on macOS',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const status = await installer.getStatus()
      expect(status.commandName).toBe('barkos')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(s1.commandPath).toBe(join(homePath, '.local', 'bin', 'barkos'))
    }
  )

  // Why: the arm64 fallback must apply for packaged builds, not just dev launchers.
  it.skipIf(process.platform === 'win32')(
    'resolves to ~/.local/bin/barkos on arm64 even when isPackaged is true',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'barkos')
      const resourcesPath = join(fixture.root, 'resources')
      const bundledLauncher = join(resourcesPath, 'bin', 'barkos')
