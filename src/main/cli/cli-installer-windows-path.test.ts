...
describe('CliInstaller', () => {
  ...
  it('creates a windows wrapper and updates the user PATH', async () => {
    ...
    expect(wrapperContent).toContain('barkos.cmd')
  ...
  it('resolves custom-install packaged Windows command path from resourcesPath', async () => {
    ...
    const resourcesPath = join(fixture.root, 'D Custom BarkOS', 'resources')
    await mkdir(join(resourcesPath, 'bin'), { recursive: true })
    await writeFile(join(resourcesPath, 'bin', 'barkos.exe'), 'native launcher', 'utf8')
    ...
    const installer = new CliInstaller({
      ...
      localAppDataPath,
      userDataPath: fixture.userDataPath,
      execPath: join(fixture.root, 'D Custom BarkOS', 'BarkOS.exe'),
      appPath: fixture.appPath,
    ...
    expect(status.commandPath).toBe(join(resourcesPath, 'bin', 'barkos.exe'))
  })
  ...
  it('keeps a bundled Windows launcher installed when the user PATH read is unknown', async () => {
    ...
    const bundledLauncher = join(resourcesPath, 'bin', 'barkos.exe')
    ...
    const installer = new CliInstaller({
      ...
      resourcesPath,
      userDataPath: fixture.userDataPath,
      execPath: join(fixture.root, 'BarkOS.exe'),
      appPath: fixture.appPath,
      userPathReader: async () => ({
        state: 'unknown',
        detail: 'Orca could not read the Windows user PATH registry value.'
