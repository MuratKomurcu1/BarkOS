...
describe('packaged Windows CLI launcher asset', () => {
  it('keeps the batch compatibility shim behind the newline-safe native launcher', () => {
    const launcherPath = join(process.cwd(), 'resources', 'win32', 'bin', 'barkos.cmd')
    const launcher = readFileSync(launcherPath, 'utf8')

    expect(launcher).toContain('set "LAUNCHER=%SCRIPT_DIR%barkos.exe"')
    expect(launcher).toContain('barkos.cmd cannot safely forward orchestration message bodies')
  ...
  it('marks the packaged child and propagates its exact exit status', () => {
    ...
    expect(source).toContain('Environment.SetEnvironmentVariable("ORCA_CLI_COMMAND", "barkos");')
    expect(source).toContain('Path.Combine(appDirectory, "BarkOS.exe")')
