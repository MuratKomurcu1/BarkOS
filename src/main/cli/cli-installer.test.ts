...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(fixture.root, 'bin', 'barkos')
      ...
      expect(initial.launcherPath).toContain(join('userData', 'cli', 'bin', 'barkos'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const fixture = await makeFixture()
      const installPath = join(fixture.root, '.local', 'bin', 'barkos')
      const installer = new CliInstaller({
        platform: 'linux',
        isPackaged: false,
        userDataPath: fixture.userDataPath,
        execPath: '/opt/BarkOS/barkos',
        appPath: fixture.appPath,
        commandPathOverride: installPath,
      ...
      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandName).toBe('barkos')
  ...
  // packaged `barkos` command developers rely on day to day.
  it.skipIf(process.platform === 'win32')(
    'uses a separate barkos-dev command for default development installs',
    async () => {
      const fixture = await makeFixture()
      ...
      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandName).toBe('barkos-dev')
      expect(installed.commandPath).toBe(join(commandDir, 'barkos-dev'))
      expect(installed.launcherPath).toBe(join(fixture.userDataPath, 'cli', 'bin', 'barkos-dev'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const appImagePath = join(fixture.root, 'BarkOS.AppImage')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const commandDir = join(fixture.root, '.local', 'bin')
      const installPath = join(commandDir, 'barkos')
      const oldAppImagePath = join(fixture.root, 'Old-BarkOS.AppImage')
      const newAppImagePath = join(fixture.root, 'BarkOS.AppImage')
  ...
  // BarkOS must not mutate an Orca command even when it points at an old Orca launcher.
  it.skipIf(process.platform === 'win32')(
    'leaves an installed Orca CLI untouched when installing BarkOS',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const commandDir = join(homePath, '.local', 'bin')
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(join(commandDir, 'barkos'))
      await expect(readlink(legacyCommandPath)).resolves.toBe(oldLauncherPath)
    }
  )

  it.skipIf(process.platform === 'win32')(
    'leaves an Orca CLI untouched when installing a BarkOS AppImage wrapper',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const commandDir = join(homePath, '.local', 'bin')
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(join(commandDir, 'barkos'))
      await expect(readlink(legacyCommandPath)).resolves.toBe(
        join('/tmp', '.mount_Orca1234', 'resources', 'bin', 'orca')
      )
    }
  )
  ...
  it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)(
    ...
    async () => {
      ...
      const installPath = join(protectedDir, 'bin', 'barkos')
