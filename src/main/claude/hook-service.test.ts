...
describe('ClaudeHookService.install', () => {
  it('installs managed hooks into Claude settings and preserves user Bedrock settings', () => {
    ...
    try {
      ...
      if (process.platform === 'win32') {
        expect(managedScript).toContain('if "%ORCA_AGENT_HOOK_PORT%"=="" (echo {} & exit /b 0)')
      } else {
        expect(managedScript).toMatch(/^#!\/bin\/sh\npayload=/)
        expect(managedScript).toContain('if [ -n "$DEVIN_PROJECT_DIR" ]; then\n  printf "{}\\n"')
      }
      expect(managedScript).toContain('BarkOS approval service is unavailable')
    } finally {
      vi.unstubAllEnvs()
  ...
  it.skipIf(process.platform === 'win32')(
    'fails closed on transport loss only for BarkOS-enforced sessions',
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), 'orca-claude-barkos-hook-'))
      vi.stubEnv('HOME', tmpHome)
      vi.stubEnv('USERPROFILE', tmpHome)
      try {
        expect(new ClaudeHookService().install().state).toBe('installed')
        ...
        const normal = spawnSync('/bin/sh', [scriptPath], {
          env: { ...baseEnv, ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '0' },
          input,
          encoding: 'utf8'
        })
        const enforced = spawnSync('/bin/sh', [scriptPath], {
          env: { ...baseEnv, ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' },
          input,
          encoding: 'utf8'
        })
        ...
        expect(enforced.status).toBe(2)
        expect(enforced.stderr).toContain('BarkOS approval service is unavailable')
        expect(enforcedStop.status).toBe(0)
        expect(JSON.parse(enforcedStop.stdout.trim())).toEqual({})
      } finally {
        vi.unstubAllEnvs()
        rmSync(tmpHome, { recursive: true, force: true })
      }
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(script).toContain('if "%ORCA_AGENT_HOOK_PORT%"=="" (echo {} & exit /b 0)')
        expect(script).toContain('BarkOS approval service is unavailable')
      } finally {
        vi.unstubAllEnvs()
        rmSync(tmpHome, { recursive: true, force: true })
      }
...
describe('ClaudeHookService.installRemote', () => {
  it('writes Claude settings + managed script under the remote $HOME', async () => {
    ...
    expect(script).toContain('if [ -n "$DEVIN_PROJECT_DIR" ]; then\n  printf "{}\\n"')
    expect(script).toContain('BarkOS approval service is unavailable')
