...
export async function prepareGitHubStackedPullRequest(
  ...
  try {
    ...
    if (parentPullRequests.length !== 1) {
      return creationError(
        `BarkOS found multiple open pull requests for the parent branch ${base}.`
      )
    }
    if (currentPullRequests.length > 1) {
      return creationError(
        `BarkOS found multiple open pull requests for the current branch ${head}.`
      )
    }
    ...
    return {
      ...
      error: isStacksUnavailableError(error)
        ...
        : 'BarkOS could not verify the parent pull request. Retry in a moment.'
    }
  } finally {
    release()
  }
