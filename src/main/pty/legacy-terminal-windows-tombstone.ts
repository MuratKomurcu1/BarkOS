...
if not defined orca_real (
  echo BarkOS compatibility wrapper could not locate __ORCA_COMMAND__ on PATH. 1>&2
  exit /b 127
)
:run
"%orca_real%" %*
...
if (-not $realCommand) {
  [Console]::Error.WriteLine("BarkOS compatibility wrapper could not locate $commandName on PATH.")
  exit 127
}
& $realCommand @args
exit $LASTEXITCODE
