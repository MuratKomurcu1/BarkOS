...
export function getOrcaCloudAuthConfig(
  ...
  // Why: BarkOS has no first-party cloud yet, so there is no built-in default
  // endpoint — packaged builds stay honestly unconfigured unless an explicit
  // env override points at a real deployment. Silently falling back to the
  // upstream product's auth server would present a sign-in that does not
  // belong to BarkOS (handoff #4).
  const apiBaseUrl = configuredApiBaseUrl ? cleanEndpointUrl(configuredApiBaseUrl) : null
  const clientId = env.ORCA_CLOUD_CLIENT_ID?.trim() || undefined
  ...
  return {
    ...
    config: {
      ...
      relayDirectorUrl: cleanOrigin(env.ORCA_RELAY_URL, allowLoopbackHttp) ?? apiBaseUrl,
      clientId,
