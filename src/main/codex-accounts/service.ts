...
import { prepareCodexAccountFailoverResume } from '../codex/codex-account-failover-resume'
...
export class CodexAccountService {
   ...
   * instead of driving `codex login` here. Lets the `barkos account add --agent codex`
  ...
  prepareSelectedHostAccountFailoverResume(
    accountId: string,
    ...
    return this.serializeMutation(async () => {
      ...
      if (accountTarget.runtime !== 'host') {
        throw new Error('Codex account failover resume supports only host-managed accounts')
      }
      if (normalizeCodexRuntimeSelection(this.store.getSettings()).host !== account.id) {
        throw new Error('Codex account failover target is no longer selected')
      }
      const targetCodexHomePath =
        this.runtimeHome.resolveSelectedHostAccountCodexHomePathForResume()
      if (
        !targetCodexHomePath ||
        normalizeRuntimePathForComparison(targetCodexHomePath) !==
          normalizeRuntimePathForComparison(account.managedHomePath)
      ) {
        throw new Error('Codex account failover target home could not be verified')
      }
      return prepareCodexAccountFailoverResume({
        providerSession,
        targetCodexHomePath,
        trustedCodexHomes: [
          getSystemCodexHomePath(),
          ...this.runtimeHome.getHostCodexHomePathsForSessionDiscovery()
        ]
      })
    })
  }
  ...
  private async persistCapturedCodexAccount(
    ...
    if (!identity.email) {
      throw new Error('Codex login completed, but BarkOS could not resolve the account email.')
    }
  ...
  private assertOAuthAccountAddAllowed(canonicalConfig: CanonicalCodexConfig | null): void {
    ...
    throw new Error(
      `BarkOS cannot add a Codex OAuth account while ~/.codex/config.toml pins the custom provider ${JSON.stringify(modelProvider)}. Keep using the system-default account for this provider, or remove model_provider (or set it to "openai") before adding an OAuth account. BarkOS left your config unchanged.`
    )
  }
  ...
  private assertManagedHomePath(candidatePath: string, expectedAccountId?: string): string {
    ...
    if (wslInfo) {
      if (
        ...
        throw new Error('Managed WSL Codex home is outside BarkOS account storage.')
      }
      if (
      ...
      if (!existsSync(join(candidatePath, '.orca-managed-home'))) {
        throw new Error('Managed Codex home is missing BarkOS ownership marker.')
      }
      if (
        expectedAccountId !== undefined &&
