...
import { getDroidManagedHookScript } from './managed-hook-script'
...
function writeScript(): string {
  const root = mkdtempSync(join(tmpdir(), 'orca-droid-managed-hook-'))
  roots.push(root)
  const scriptPath = join(root, 'droid-hook.sh')
  writeFileSync(scriptPath, getDroidManagedHookScript('posix'), { mode: 0o700 })
  return scriptPath
}

describe.skipIf(process.platform === 'win32')('Droid managed hook script', () => {
  it('fails closed on transport loss only for an enforced PreToolUse event', () => {
  ...
  it('forwards the exact decision and enforcement marker', async () => {
    const scriptPath = writeScript()
    let postedBody = ''
