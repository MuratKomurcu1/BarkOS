...
import menuBarIconPath from '../../../resources/tray/barkos-menu-barTemplate.png?asset&asarUnpack'
import menuBarIconRetinaPath from '../../../resources/tray/barkos-menu-barTemplate@2x.png?asset&asarUnpack'
...
export type SystemTrayOptions = {
  ...
  /** Quit BarkOS for real (caller must set the quitting latch before quitting). */
  onQuit: () => void
}
...
let activeTrayOptions: SystemTrayOptions | null = null
...
function baseTooltip(): string {
  ...
  return devIndicator.label ? `BarkOS DEV (${devIndicator.label})` : 'BarkOS DEV'
}
      ...
      try {
        ...
        tray.setToolTip(
          devIndicator
            ...
            : translateMain('tray.activityWaiting', 'BarkOS - activity waiting')
        )
        return
      } catch (error) {
...
function buildTrayMenu(opts: SystemTrayOptions): Electron.Menu {
  return Menu.buildFromTemplate([
    ...(devIndicator
      ...
      : []),
    {
      label: translateMain('tray.openOrca', 'Open BarkOS'),
...
/** Rebuilds native labels after a live language change without replacing the status item. */
export function refreshSystemTrayMenu(): Tray | null {
  if (!tray || tray.isDestroyed() || !activeTrayOptions) {
    return null
  }
  tray.setContextMenu(buildTrayMenu(activeTrayOptions))
  applyTrayImage()
  return tray
}
