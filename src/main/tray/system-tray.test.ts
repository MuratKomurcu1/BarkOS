...
vi.mock('../../../resources/tray/barkos-menu-barTemplate.png?asset&asarUnpack', () => ({
  default: '/assets/barkos-menu-barTemplate.png'
}))

vi.mock('../../../resources/tray/barkos-menu-barTemplate@2x.png?asset&asarUnpack', () => ({
  default: '/assets/barkos-menu-barTemplate@2x.png'
}))
...
describe('createSystemTray', () => {
  it('keeps the Windows icon, Open/Quit menu, and left-click behavior', async () => {
    ...
    expect(trayInstances[0].setToolTip).toHaveBeenCalledWith('BarkOS')
    expect(builtMenuItems().map((item) => item.label)).toEqual(['Open BarkOS', undefined, 'Quit'])
  ...
  it('refreshes the macOS tray menu in Turkish after a live language change', async () => {
    setPlatform('darwin')
    const { createSystemTray, refreshSystemTrayMenu } = await loadModule()
    const { setMainUiLanguage } = await import('../i18n/main-i18n')

    createSystemTray(createOptions())
    await setMainUiLanguage('tr')
...
describe('dev instance indicator', () => {
  it('stamps the DEV badge into the macOS template with a rebuilt Retina rep', async () => {
    ...
    expect(trayInstances[0].setToolTip).toHaveBeenCalledWith('BarkOS DEV (my-branch)')
    expect(builtMenuItems()[0]).toMatchObject({
      label: 'BarkOS DEV (my-branch)',
      enabled: false
    })
  })
  ...
  it('omits the label suffix when the dev instance has none', async () => {
    ...
    expect(trayInstances[0].setToolTip).toHaveBeenCalledWith('BarkOS DEV')
    expect(builtMenuItems()[0]).toMatchObject({ label: 'BarkOS DEV', enabled: false })
  })
  ...
  it('keeps the DEV marker in the tooltip across the attention toggle', async () => {
    ...
    setTrayAttention(true)
    flushTraySceneMutation()
    expect(created.setToolTip).toHaveBeenCalledWith('BarkOS DEV (my-branch) - activity waiting')
    setTrayAttention(false)
    flushTraySceneMutation()
    expect(created.setToolTip).toHaveBeenLastCalledWith('BarkOS DEV (my-branch)')
  })
  ...
  it('marks the Windows tooltip without badging the icon', async () => {
    ...
    expect(builtMenuItems()[0]).toMatchObject({ label: 'BarkOS DEV (my-branch)', enabled: false })
  })
  ...
  it('adds no DEV marker for production instances', async () => {
    ...
    expect(stampDevBadgeMock).not.toHaveBeenCalled()
    expect(trayInstances[0].image).toBe(baseMacImage)
    expect(builtMenuItems()[0].label).toBe('Open BarkOS')
  })
})
...
describe('setTrayAttention', () => {
  ...
  it('uses a literal theme-aware glyph for macOS attention, then restores template mode', async () => {
    ...
    expect(created.setImage).toHaveBeenCalledWith(attentionImage)
    expect(created.setToolTip).toHaveBeenCalledWith('BarkOS - activity waiting')
    ...
    expect(created.setToolTip).toHaveBeenLastCalledWith('BarkOS')
  })
