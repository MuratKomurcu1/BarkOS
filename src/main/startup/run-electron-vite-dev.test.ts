...
describe('run-electron-vite-dev', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'barkos-dev-wrapper-'))
  ...
  it('forwards dev instance identity to electron-vite', async () => {
    ...
    expect(envSnapshot.dockTitle).toBe('BarkOS: feature/billing-shell')
  ...
  it.skipIf(process.platform === 'win32')(
    'prepares userData orca and barkos-dev wrappers for dev terminals',
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'barkos-dev-wrapper-'))
      ...
      const trackedPids = trackPidFile(pidFile)
      const devWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'barkos-dev'), 'utf8')
