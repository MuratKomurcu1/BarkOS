...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another BarkOS instance is already running for this userData profile; exiting this launch after requesting the existing window. If no BarkOS process is running, this may be an Electron/macOS single-instance lock failure.'
...
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] ORCA_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another BarkOS instance running for the same profile.'
...
// Why: a duplicate `barkos serve` is a supervisor artifact, not a user asking for a window; fail open when argv is unavailable.
 ...
 * instance quits, metadata points at a dead pid and `barkos status` reports
 ...
 * way dev (`barkos-dev` userData) and packaged (`orca` userData) runs lock in
