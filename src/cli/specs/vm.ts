...
export const VM_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage:
      'barkos vm recipe doctor <recipe-id> [--repo-path <path>] [--provision|--connect] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'recipe-id', 'repo-path', 'provision', 'connect'],
    positionalArgs: ['recipe-id'],
    notes: [
      'Reads environmentRecipes from barkos.yaml in the repo path, validates the selected recipe, and reports agent-friendly checks.',
    ...
    examples: [
      'barkos vm recipe doctor cloud-sandbox',
      'barkos vm recipe doctor cloud-sandbox --repo-path /path/to/repo --json',
      'barkos vm recipe doctor cloud-sandbox --provision --json'
    ]
  }
]
