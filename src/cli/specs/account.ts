...
export const ACCOUNT_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['account', 'add'],
    summary: 'Add a managed Claude or Codex account by signing in on this BarkOS host',
    usage: 'barkos account add [--agent claude|codex] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'agent'],
    notes: [
      'Runs the agent login (`claude login` / `codex login`) in this terminal, then registers the account with the local BarkOS runtime.',
      ...
      '--agent defaults to claude. Requires the BarkOS runtime to be running on this machine.'
    ],
    examples: ['barkos account add', 'barkos account add --agent codex']
  },
  {
    path: ['account', 'list'],
    summary: 'List managed Claude and Codex accounts on this BarkOS host',
    usage: 'barkos account list [--json]',
