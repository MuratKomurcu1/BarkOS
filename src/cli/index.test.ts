...
describe('unknown command surfaces a suggestion', () => {
  ...
  it('prints did-you-mean for a near-miss command and exits non-zero', async () => {
    ...
    expect(stderr).toContain('barkos worktree')
  })
...
describe('unknown help command surfaces a suggestion', () => {
  it.each([
    ...
    expect(process.exitCode).toBe(1)
    expect(logSpy.mock.calls.flat().join('\n')).toContain('Did you mean: barkos worktree')
    logSpy.mockRestore()
    process.exitCode = 0
  })
})
...
describe('orca root help', () => {
  ...
  it('progressively discloses Linear commands', async () => {
    ...
    expect(groupHelp).toContain('barkos linear')
    ...
    expect(issueHelp).toContain('barkos linear issue [<id>]')
    ...
    expect(searchHelp).toContain('barkos linear search <query>')
  ...
  it('hides removed parent-workspace help and scopes create parent selectors', async () => {
    ...
    expect(createHelp).toContain(
      '--no-parent only affects BarkOS lineage; omit --base-branch to use the repo default base'
    )
