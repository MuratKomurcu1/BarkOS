...
async function assertAccountImportSupported({ client }: HandlerContext): Promise<void> {
  ...
  if (!status.result.capabilities?.includes(ACCOUNT_IMPORT_RUNTIME_CAPABILITY)) {
    throw new RuntimeClientError(
      ...
      'The running BarkOS runtime is too old to add accounts from the CLI. Update or restart BarkOS and try again.'
    )
  }
}

/** CLI handlers for `barkos account add [--agent claude|codex]` and `barkos account list`. */
export const ACCOUNT_HANDLERS: Record<string, CommandHandler> = {
  'account add': async (ctx) => {
    ...
    rejectRemoteSelectionFlags(ctx, 'barkos account add')
  ...
  'account list': async (ctx) => {
    rejectRemoteSelectionFlags(ctx, 'barkos account list')
