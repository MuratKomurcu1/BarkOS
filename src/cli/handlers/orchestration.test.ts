...
describe('orchestration send structured payload flags', () => {
  ...
  it('validates and carries a BarkOS staffing proposal in worker_done', async () => {
    const staffingProposal = {
      version: 1,
      summary: 'Tek bir arayüz çalışanı yeterli.',
      roles: [
        {
          key: 'frontend',
      ...
      tasks: [
        {
          ...
          spec: 'Ekranı ve testlerini tamamla.',
          roleKey: 'frontend',
          dependencyKeys: [],
    ...
    await invokeSend(
      new Map<string, string | boolean>([
        ...
        ['dispatch-id', 'ctx_1'],
        ['outcome', 'succeeded'],
        ['staffing-proposal-json', JSON.stringify(staffingProposal)]
      ])
    )
