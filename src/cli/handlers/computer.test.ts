...
describe('orca computer observation CLI handlers', () => {
  ...
  it('prints targeted permission setup command help', async () => {
    ...
    expect(output).toContain(
      'barkos computer permissions [--id <accessibility|screenshots>] [--json]'
    )
  ...
  it('opens computer permission setup without resolving a worktree', async () => {
    ...
    const output = vi.mocked(console.log).mock.calls[0][0]
    expect(output).toContain('Opened BarkOS Computer Use permission setup')
    expect(output).toContain('/Applications/BarkOS Computer Use.app')
  })
