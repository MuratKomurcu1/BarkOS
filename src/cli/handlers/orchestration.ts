...
import { parseBarkosStaffingProposal } from '../../shared/barkos/staffing-proposal'
...
function resolvePackagedWindowsCompatibilityCommand(): 'barkos' | 'orca' | 'orca-ide' | undefined {
  if (process.env.ORCA_WINDOWS_PACKAGED_CLI_LAUNCHER !== '1') {
    return undefined
  }
  const command = process.env.ORCA_CLI_COMMAND
  if (command === 'barkos' || command === 'orca' || command === 'orca-ide') {
    return command
  }
  throw new RuntimeClientError(
    'invalid_argument',
    'The packaged BarkOS launcher did not provide a valid resume command. No question was created.'
  )
}
...
async function getOptionalStructuredMessagePayload(
  ...
  const staffingProposalJson = getOptionalStringFlag(flags, 'staffing-proposal-json')
  const staffingProposalFile = getOptionalStringFlag(flags, 'staffing-proposal-file')
  if (staffingProposalJson && staffingProposalFile) {
    throw new RuntimeClientError(
      'invalid_argument',
      'Use either --staffing-proposal-json or --staffing-proposal-file, not both.'
    )
  }
  const hasStructuredPayload =
    ...
    phase !== undefined ||
    staffingProposalJson !== undefined ||
    staffingProposalFile !== undefined
  ...
  const staffingProposalSource = staffingProposalFile
    ? await readFile(staffingProposalFile, 'utf8')
    : staffingProposalJson
  if (staffingProposalSource) {
    if (staffingProposalSource.length > 64_000) {
      throw new RuntimeClientError(
        'invalid_argument',
        '--staffing-proposal-json cannot exceed 64,000 characters.'
      )
    }
    try {
      payload.staffingProposal = parseBarkosStaffingProposal(JSON.parse(staffingProposalSource))
    } catch {
      throw new RuntimeClientError(
        'invalid_argument',
        'The BarkOS staffing proposal must contain valid JSON matching the staffing contract.'
      )
    }
  }
...
export const ORCHESTRATION_HANDLERS: Record<string, CommandHandler> = {
  ...
  'orchestration send': async ({ flags, client, cwd, json }) => {
    ...
    if (
      type !== 'worker_done' &&
      (staffingProposalJson !== undefined || staffingProposalFile !== undefined)
    ) {
      throw new RuntimeClientError(
        'invalid_argument',
        'Staffing proposal flags are only valid with --type worker_done.'
      )
    }
    ...
    const sendParams = {
      ...
      payload: await getOptionalStructuredMessagePayload(flags),
