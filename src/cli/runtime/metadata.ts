  ...
  try {
    ...
    if (!metadata || !findTransport(metadata, 'unix', 'named-pipe') || !metadata.authToken) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        `BarkOS runtime metadata is incomplete at ${metadataPath}`
      )
    }
    return metadata
  } catch (error) {
    ...
    throw new RuntimeClientError(
      'runtime_unavailable',
      `Could not read BarkOS runtime metadata at ${metadataPath}. Start the BarkOS app first.`
    )
  }
}
...
export function getDefaultUserDataPath(
  ...
  // runtime metadata to a separate userData directory (e.g. `barkos-dev`) to avoid
  ...
  if (platform === 'darwin') {
    return join(homeDir, 'Library', 'Application Support', 'barkos')
  }
  if (platform === 'win32') {
    ...
    if (!appData) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        'APPDATA is not set, so the BarkOS runtime metadata path cannot be resolved.'
      )
    }
    return join(appData, 'barkos')
  }
  ...
  return join(process.env.XDG_CONFIG_HOME || join(homeDir, '.config'), 'barkos')
}
