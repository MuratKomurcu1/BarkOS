...
export function launchOrcaApp(): void {
  ...
  throw new RuntimeClientError(
    'runtime_open_failed',
    'Could not determine how to launch BarkOS. Start BarkOS manually and try again.'
  )
}
    ...
    const onClose = (code: number | null, signal: NodeJS.Signals | null): void => {
      ...
      finish(
        new RuntimeClientError(
          ...
          typeof code === 'number'
            ? `BarkOS serve exited before printing valid recipe JSON with code ${code}.`
            : `BarkOS serve exited before printing valid recipe JSON via ${signal}.`
        )
      )
    }
...
function resolveAppRoot(): string {
  ...
  // process.cwd(). Pin it to the app root so `barkos serve` behaves the same
...
function resolveForegroundOrcaExecutable(): string {
  ...
  throw new RuntimeClientError(
    'runtime_serve_failed',
    'Could not determine how to start BarkOS server. Set ORCA_APP_EXECUTABLE to the BarkOS executable.'
  )
}
