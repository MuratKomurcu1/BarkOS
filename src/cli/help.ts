...
const ROOT_HELP_TEXT = `barkos

Usage: barkos <command> [options]

Startup:
  open                      Launch BarkOS and wait for the runtime to be reachable
  serve                     Start a headless BarkOS runtime server
  status                    Show app/runtime/graph readiness

Diagnostics:
  diagnostics memory        Collect a memory snapshot for BarkOS and managed terminals

Agent Discovery:
  agent-context             Print the machine-readable command schema for agents

Accounts:
  account add               Add a managed Claude or Codex account on this BarkOS host
  account list              List managed Claude and Codex accounts on this BarkOS host

Skills:
  skills installed          List installed skill selectors
  skills share              Publish selected skills behind one unlisted link
  skills list               List version-matched skill guides bundled with this BarkOS CLI
  skills get                Print a version-matched skill guide as Markdown
  skills install            Install bundled BarkOS skills globally via the community skills CLI
  skills update             Update already-installed BarkOS skills via the community skills CLI

Environments:
  environment add           Save a remote BarkOS runtime from a pairing code
  environment list          List saved remote BarkOS runtimes
  environment show          Show one saved remote BarkOS runtime
  environment rm            Remove a saved remote BarkOS runtime

Environment Recipes:
  vm recipe doctor          Validate a per-workspace environment recipe

Automations:
  automations list          List scheduled BarkOS automations
  automations show          Show one BarkOS automation
  automations create        Create a scheduled BarkOS automation
  automations edit          Edit a BarkOS automation
  automations remove        Remove a BarkOS automation and its run history
  automations run           Run a BarkOS automation now
  automations runs          List automation run history

Projects:
  project list              List durable projects known to BarkOS
...
Repos:
  repo list                 List repos registered in BarkOS
  repo add                  Add a project to BarkOS by filesystem path
...
Worktrees:
  worktree list             List BarkOS-managed worktrees
  worktree show             Show one worktree
  worktree current          Show the BarkOS-managed worktree for the current directory
  worktree create           Create a new BarkOS-managed worktree
  worktree set              Update BarkOS metadata for a worktree
  worktree rm               Remove a worktree from BarkOS and git
  worktree ps               Show a compact orchestration summary across worktrees

Files:
  file open                 Open a workspace file in the BarkOS editor
  file diff                 Open a workspace file diff in the BarkOS editor
  file open-changed         Open all git-changed files for a workspace

Terminals:
  terminal list             List live BarkOS-managed terminals
...
Orchestration:
  ...
  orchestration worker-start Start a supervised worker locally or on a connected BarkOS server
...
Mobile Emulator (iOS Simulator):
  emulator list             List available/running emulators (BarkOS-managed + raw serve-sim)
...
Common Commands:
  ...
  barkos status [--json]
  barkos diagnostics memory [--json]
  barkos agent-context [--json]
  barkos account add [--agent claude|codex] [--json]
  barkos account list [--json]
  barkos environment add --name <name> --pairing-code <code> [--json]
  barkos environment list [--json]
  barkos environment show --environment <selector> [--json]
  barkos environment rm --environment <selector> [--json]
  barkos worktree list [--repo <selector>] [--limit <n>] [--json]
  ...
  barkos worktree show --worktree <selector> [--json]
  barkos worktree current [--json]
  ...
  barkos worktree rm --worktree <selector> [--force] [--run-hooks] [--json]
  barkos worktree ps [--limit <n>] [--json]
  barkos file open <path> [--worktree <selector>] [--json]
  barkos file diff <path> [--staged] [--worktree <selector>] [--json]
  barkos file open-changed [--mode edit|diff|both] [--worktree <selector>] [--json]
  barkos terminal list [--worktree <selector>] [--limit <n>] [--include-visual-layouts] [--json]
  barkos terminal show [--terminal <handle>] [--json]
  barkos terminal read [--terminal <handle>] [--cursor <n>] [--limit <n>] [--json]
  ...
  barkos terminal stop --worktree <selector> [--json]
  barkos terminal create [--worktree <selector>] [--title <name>] [--command <text>] [--focus] [--json]
  barkos terminal split [--terminal <handle>] [--direction horizontal|vertical] [--json]
  barkos terminal switch [--terminal <handle>] [--json]
  barkos terminal close [--terminal <handle>] [--tab] [--json]
  barkos project list [--json]
  barkos project setups [--project <id>] [--host <host-id>] [--json]
  ...
  barkos project setup-delete --setup <setup-id> [--json]
  barkos repo list [--json]
  barkos repo add --path <path> [--json]
  barkos repo show --repo <selector> [--json]
  barkos repo set-base-ref --repo <selector> --ref <ref> [--json]
  barkos repo search-refs --repo <selector> --query <text> [--limit <n>] [--json]
...
Selectors:
  ...
  --terminal <handle>       Runtime-issued terminal handle returned by \`barkos terminal list --json\`
...
Output Options:
  ...
  --pairing-code <code>      Connect to a remote BarkOS runtime using a barkos://pair?... code
  --environment <selector>   Connect using a saved environment id or name
  --help                    Show this help message

Behavior:
  Most commands require a running BarkOS runtime. If BarkOS is not open yet, run \`barkos open\` first.
...
Agent Sessions And Worktrees:
  ...
  To start a fresh agent in the current worktree, use:
    barkos terminal create --worktree active --command "codex"

Browser Workflow:
  1. Create or navigate:  barkos tab create --url https://example.com
                          barkos goto --url https://example.com
  2. Inspect the page:    barkos snapshot
     (Returns an accessibility tree with element refs like e1, e2, e3)
     For concurrent workflows, prefer: barkos tab list --json
     then reuse tabs[].browserPageId with --page <id> on later commands.
  3. Interact:            barkos click --element e2
                          barkos fill --element e5 --value "search query"
                          barkos keypress --key Enter
  4. Re-inspect:          barkos snapshot
...
Examples:
  ...
  $ barkos diagnostics memory --json
  $ barkos repo list
  $ barkos worktree create --name agent-task --agent codex --prompt "hi"
  $ barkos worktree create --repo name:barkos --name cli-test-1 --issue 273
  $ barkos worktree create --repo name:barkos --name linear-task --linear-issue https://linear.app/stably/issue/STA-335/test-issue
  $ barkos worktree create --name linear-task --linear-issue STA-335
  $ barkos worktree show --worktree branch:Jinwoo-H/cli
  $ barkos worktree current
  $ barkos worktree set --worktree active --comment "waiting on review"
  $ barkos worktree set --worktree active --linear-issue null
  $ barkos worktree ps --limit 10
  $ barkos file open-changed --mode diff
  $ barkos file open src/App.tsx
  $ barkos terminal create --worktree active --command "codex"
  $ barkos terminal list --worktree path:/Users/me/barkos/workspaces/barkos/cli-test-1 --json
  $ barkos terminal send --terminal term_123 --text "hi" --enter
  $ barkos terminal wait --terminal term_123 --for exit --timeout-ms 60000 --json
  $ barkos tab current --json
  $ barkos tab show --page page_123 --json
  $ barkos tab create --url https://example.com --profile work
  $ barkos tab profile clone --page page_123 --profile work --json
  $ barkos snapshot
  $ barkos click --element e3
  $ barkos fill --element e5 --value "hello"
  $ barkos goto --url https://example.com/login
  $ barkos keypress --key Enter
  $ barkos eval --expression "document.title"
  $ barkos tab list --json`

function brandCliHelp(text: string): string {
  return text.replaceAll('BarkOS', 'BarkOS').replaceAll('barkos', 'barkos')
}
...
export function printHelp(specs: CommandSpec[], commandPath: string[] = []): void {
  ...
  if (commandPath.length > 0) {
    ...
    console.log(
      brandCliHelp(`Unknown command: ${commandPath.join(' ')}${recovery ? `\n${recovery}` : ''}\n`)
    )
  }
...
export function formatCommandHelp(spec: CommandSpec): string {
  const lines = [`barkos ${spec.path.join(' ')}`, '', `Usage: ${spec.usage}`, '', spec.summary]
  ...
  return brandCliHelp(lines.join('\n'))
}
...
export function formatGroupHelp(specs: CommandSpec[], group: string): string {
  ...
  const lines = [
    `barkos ${group}`,
    '',
    `Usage: barkos ${group} <command> [options]`,
    '',
    'Commands:'
  ]
  ...
  lines.push('', `Run \`barkos ${group} <command> --help\` for command-specific usage.`)
  return brandCliHelp(lines.join('\n'))
}
...
export function formatFlagHelp(flag: string): string {
  const helpByFlag: Record<string, string> = {
    ...
    comment: '--comment <text>       Comment stored in BarkOS metadata',
    ...
    activate: '--activate             Reveal the new worktree in the BarkOS app',
    ...
    'display-name': '--display-name <name>  Override the BarkOS display name',
    ...
    focus: '--focus                Reveal the created terminal session in BarkOS',
    ...
    page: '--page <id>            Stable browser page id from `barkos tab list --json`',
  ...
  if (flag === 'current') {
    return '--current              Use the current BarkOS worktree linked Linear issue'
  }
  if (flag === 'comments') {
