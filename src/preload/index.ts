...
const api = {
  ...
  codexAccounts: {
    ...
    prepareFailoverResume: (args: {
      accountId: string
      providerSession: AgentProviderSessionMetadata
    }): Promise<unknown> => ipcRenderer.invoke('codexAccounts:prepareFailoverResume', args),
    listStalePanes: (args: {
      ptyIds: string[]
    }): Promise<
      {
  ...
  barkosCompany: {
    load: (): Promise<BarkosCompany | null> => ipcRenderer.invoke('barkosCompany:load'),
    save: (company: BarkosCompany): Promise<BarkosCompany> =>
      ipcRenderer.invoke('barkosCompany:save', company),
    archive: () => ipcRenderer.invoke('barkosCompany:archive'),
    exportCurrent: () => ipcRenderer.invoke('barkosCompany:exportCurrent'),
    pickImport: () => ipcRenderer.invoke('barkosCompany:pickImport'),
    applyImport: (backup) => ipcRenderer.invoke('barkosCompany:applyImport', backup)
  },

  barkosControlPolicy: {
    load: (): Promise<BarkosControlPolicy | null> => ipcRenderer.invoke('barkosControlPolicy:load'),
    save: (policy: BarkosControlPolicy): Promise<BarkosControlPolicy> =>
      ipcRenderer.invoke('barkosControlPolicy:save', policy)
  },

  barkosBundledSkills: {
    status: (): Promise<BundledSkillStatus> => ipcRenderer.invoke('barkosBundledSkills:status'),
    install: (request: {
      skills: readonly string[]
      agents?: readonly string[]
    }): Promise<BundledSkillInstallResult> =>
      ipcRenderer.invoke('barkosBundledSkills:install', request)
  } satisfies PreloadApi['barkosBundledSkills'],

  barkosDecisionInbox: {
    load: (): Promise<BarkosDecisionInbox | null> => ipcRenderer.invoke('barkosDecisionInbox:load'),
    save: (inbox: BarkosDecisionInbox): Promise<BarkosDecisionInbox> =>
      ipcRenderer.invoke('barkosDecisionInbox:save', inbox),
    resolveSideEffect: (
      requestId: string,
      decision: 'approved' | 'rejected'
    ): Promise<BarkosDecisionInbox> =>
      ipcRenderer.invoke('barkosDecisionInbox:resolveSideEffect', { requestId, decision }),
    preparePairedSideEffectApproval: (
      environmentId: string,
      agent: 'claude' | 'codex' | 'droid' | 'gemini'
    ): Promise<boolean> =>
      ipcRenderer.invoke('barkosDecisionInbox:preparePairedSideEffectApproval', {
        environmentId,
        agent
      })
  },
  barkosMemoryVault: {
    load: (): Promise<BarkosMemoryVault | null> => ipcRenderer.invoke('barkosMemoryVault:load'),
    save: (vault: BarkosMemoryVault): Promise<BarkosMemoryVault> =>
      ipcRenderer.invoke('barkosMemoryVault:save', vault)
  },

  barkosProviderCapacity: {
    load: (): Promise<BarkosProviderCapacityLedger | null> =>
      ipcRenderer.invoke('barkosProviderCapacity:load'),
    save: (ledger: BarkosProviderCapacityLedger): Promise<BarkosProviderCapacityLedger> =>
      ipcRenderer.invoke('barkosProviderCapacity:save', ledger)
  },

  barkosUsageCost: {
    load: (): Promise<BarkosUsageCostLedger | null> => ipcRenderer.invoke('barkosUsageCost:load'),
    sync: (request: BarkosUsageCostSyncRequest): Promise<BarkosUsageCostLedger> =>
      ipcRenderer.invoke('barkosUsageCost:sync', request)
  },

  barkosWorkerSessions: {
    load: (): Promise<BarkosWorkerSessionSnapshot | null> =>
      ipcRenderer.invoke('barkosWorkerSessions:load'),
    record: (binding: BarkosWorkerSessionBinding): Promise<BarkosWorkerSessionSnapshot> =>
      ipcRenderer.invoke('barkosWorkerSessions:record', binding)
  },

  barkosWorkLedger: {
    load: (): Promise<BarkosWorkLedger | null> => ipcRenderer.invoke('barkosWorkLedger:load'),
    save: (ledger: BarkosWorkLedger): Promise<BarkosWorkLedger> =>
      ipcRenderer.invoke('barkosWorkLedger:save', ledger),
    pickScreenshot: (): Promise<BarkosEvidenceScreenshotSelection | null> =>
      ipcRenderer.invoke('barkosWorkLedger:pickScreenshot'),
    runTest: (request: BarkosTestEvidenceRunRequest): Promise<BarkosTestEvidenceRunResult> =>
      ipcRenderer.invoke('barkosWorkLedger:runTest', request),
    cancelTest: (dispatchId: string): Promise<boolean> =>
      ipcRenderer.invoke('barkosWorkLedger:cancelTest', dispatchId)
  },
