...
describe('PtyHandler', () => {
  ...
  it('runs spawn guards after env assembly and before creating the physical PTY', async () => {
    handler.addEnvAugmenter(() => ({ ORCA_AGENT_HOOK_PORT: '43117' }))
    handler.addSpawnGuard((ctx) => {
      if (ctx.env.ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT === '1' && ctx.launchAgent === 'codex') {
        throw new Error('approval hook unavailable')
      }
    })

    await expect(
      dispatcher.callRequest('pty.spawn', {
        env: { ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT: '1' },
        launchAgent: 'codex'
      })
    ).rejects.toThrow('approval hook unavailable')
