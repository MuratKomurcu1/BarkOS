...
export class PtyHandler {
  ...
  private spawnGuards: PtySpawnGuard[] = []
  ...
  addSpawnGuard(guard: PtySpawnGuard): () => void {
    this.spawnGuards.push(guard)
    return () => {
      const idx = this.spawnGuards.indexOf(guard)
      if (idx !== -1) {
        this.spawnGuards.splice(idx, 1)
      }
    }
  }
  ...
  private buildSpawnEnv(
    ...
    for (const guard of this.spawnGuards) {
      guard({ ...ctx, env: result })
    }
    return result
  }
