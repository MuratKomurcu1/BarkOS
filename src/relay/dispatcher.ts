...
export class RelayDispatcher {
  ...
  requestAnyClient(
    ...
    if (!target) {
      return Promise.reject(new Error('No owning BarkOS client is connected to the relay'))
    }
    return this.requestClient(target.id, method, params, options)
  }

  async requestActiveClients(
    method: string,
    params?: Record<string, unknown>,
    options?: { timeoutMs?: number }
  ): Promise<readonly { clientId: number; result: unknown }[]> {
    const requests = this.activeClients().map(async (client) => ({
      clientId: client.id,
      result: await this.requestClient(client.id, method, params, options)
    }))
