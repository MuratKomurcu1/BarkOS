...
export class AgentHookResultRetryScheduler {
  ...
  clearCodexTranscriptPoll(paneKey: string): void {
  ...
  scheduleCodexTranscriptPoll(
    ...
    if (!hasCodexTranscriptActivity(this.host.state, original.paneKey)) {
      return
    }
    const timer = setTimeout(() => {
      ...
      const transcriptStatusChanged =
        event.payload.state !== original.payload.state ||
        event.payload.providerFailure?.kind !== original.payload.providerFailure?.kind ||
        JSON.stringify(event.payload.subagents) !== JSON.stringify(original.payload.subagents)
      const next = transcriptStatusChanged ? event : original
      if (transcriptStatusChanged) {
        this.host.applyEvent(event, source, env, version)
      }
      this.scheduleCodexTranscriptPoll(source, body, next, env, version)
