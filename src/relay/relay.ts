...
async function main(): Promise<void> {
  ...
  const barkosSideEffectAgents = new Set<AgentHookTarget>()
  ...
  const hookServer = new RelayAgentHookServer({
    ...
    evaluateSideEffect: (request) =>
      evaluateAgentHookSideEffectThroughDispatcher(dispatcher, request),
  ...
  ptyHandler.addSpawnGuard((ctx) => {
    if (
      ctx.env.ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT === '1' &&
      (ctx.launchAgent === 'claude' ||
        ctx.launchAgent === 'codex' ||
        ctx.launchAgent === 'droid' ||
        ctx.launchAgent === 'gemini') &&
      !barkosSideEffectAgents.has(ctx.launchAgent)
    ) {
      throw new Error(
        `BarkOS cannot start ${ctx.launchAgent} until its remote approval hook is installed. Retry after remote setup completes.`
      )
    }
    // Why: OpenCode's gate lives in the Orca-shipped plugin, so an enforced
    // remote launch without a materialized plugin overlay would run tools
    // with no approval path at all — fail closed like the hook-script agents.
    // The augmenter sets ORCA_OPENCODE_CONFIG_DIR only when the overlay
    // materialized, so a missing value here means the plugin will not load.
    if (
      ctx.env.ORCA_BARKOS_SIDE_EFFECT_ENFORCEMENT === '1' &&
      ctx.launchAgent === 'opencode' &&
      (!pluginOverlay.hasOpenCodeSource() || !ctx.env.ORCA_OPENCODE_CONFIG_DIR)
    ) {
      throw new Error(
        'BarkOS cannot start opencode until its remote approval plugin is installed. Retry after remote setup completes.'
      )
    }
  })
  ...
  registerManagedHookInstaller(dispatcher, undefined, (summary, requestedAgents) => {
    const installed = new Set(summary.installedAgents)
    for (const agent of requestedAgents) {
      if (agent !== 'claude' && agent !== 'codex' && agent !== 'droid' && agent !== 'gemini') {
        continue
      }
      if (installed.has(agent)) {
        barkosSideEffectAgents.add(agent)
      } else {
        barkosSideEffectAgents.delete(agent)
      }
    }
  })
