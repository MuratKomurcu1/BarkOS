...
export type ManagedHookInstallSummary = {
  installers: number
  errors: number
  installedAgents: AgentHookTarget[]
}
...
export function registerManagedHookInstaller(
  ...
  onInstalled?: (summary: ManagedHookInstallSummary, requestedAgents: AgentHookTarget[]) => void
): void {
  dispatcher.onRequest(
    ...
    async (params, context: RequestContext): Promise<ManagedHookInstallSummary> => {
      ...
      const summary = await loadRuntime().installManagedHooks({
        signal: context.signal,
        ...(hostKeyFingerprint ? { hostKeyFingerprint } : {}),
        agents
      })
      onInstalled?.(summary, agents)
      return summary
    }
  )
}
