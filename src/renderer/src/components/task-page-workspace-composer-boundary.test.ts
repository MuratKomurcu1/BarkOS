...
describe('TaskPage workspace creation source boundaries', () => {
  ...
  it('starts TaskPage GitHub work immediately and falls back to the composer', () => {
    const section = sourceBetween(
      TASK_PAGE_SOURCE,
      'const handleUseWorkItem = useCallback(',
    ...
    expect(section).toContain('repoId: item.repoId')
    expect(section).toContain("promptDelivery: 'submit-after-ready'")
