...
function createDeps(
  ...
  return {
    ...
    installBundledSkills: vi.fn(async (skills: readonly string[]) => {
      installedSkills.push([...skills])
      return { outcomes: [] }
    }),
    getBundledSkillsStatus: vi.fn(async () => ({
      skills: [
        { skill: ORCA_CLI_SKILL_NAME, installedDirectories: [], state: 'current' },
        { skill: COMPUTER_USE_SKILL_NAME, installedDirectories: [], state: 'current' },
        { skill: ORCHESTRATION_SKILL_NAME, installedDirectories: [], state: 'current' },
        { skill: ORCA_LINEAR_SKILL_NAME, installedDirectories: [], state: 'current' }
      ] as const
    })),
...
describe('onboarding feature setup runner', () => {
  ...
  it('keeps the runner on the host when the selected WSL runtime needs repair', async () => {
    ...
    // Why: bundled skills install on the host running the app regardless of the
    // agent runtime, so no clipboard command is needed.
    expect(deps.installedSkills).toEqual([[ORCHESTRATION_SKILL_NAME]])
  ...
  it('runs selected feature setup through injected deps only', async () => {
    ...
    // Why: skills install straight from the bundled registry, so nothing is
    // handed to the clipboard and no external command runs.
    expect(deps.installedSkills).toEqual([
      [
        ORCA_CLI_SKILL_NAME,
        COMPUTER_USE_SKILL_NAME,
        ORCHESTRATION_SKILL_NAME,
  ...
  it('falls back to the clipboard command when the bundled install channel fails', async () => {
    const deps = createDeps({
      installBundledSkills: vi.fn(async () => {
        throw new Error('install channel unavailable')
      })
    })
  ...
  it('warns when a selected skill cannot be verified as installed', async () => {
    const deps = createDeps({
      getBundledSkillsStatus: vi.fn(async () => ({
        skills: [
          {
            skill: ORCHESTRATION_SKILL_NAME,
    ...
    expect(result.warnings).toEqual([
      {
        ...
        message: `"${ORCHESTRATION_SKILL_NAME}" could not be verified as installed on this machine.`
      }
    ])
  })

  it('warns when the clipboard fallback also fails', async () => {
    const deps = createDeps({
      installBundledSkills: vi.fn(async () => {
        throw new Error('install channel unavailable')
      }),
