...
function humanizeUnreattachableSession(error: string): string {
  const explanation = translate(
    ...
    "BarkOS couldn't reattach to this pane's terminal session on the host. Open a new terminal to continue."
  )
  ...
  if (humanized.includes(PANE_OWNER_UNVERIFIED_MARKER)) {
    humanized = humanized.replace(
      ...
      translate(
        ...
        "BarkOS couldn't confirm whether this terminal's previous session is still running, so it left the session untouched. Reopen this pane to retry."
      )
    )
  }
...
export function TerminalErrorToast({
  ...
  const showSupportHint = !ssh && !showDaemonRestart && !isExplainedTerminalError(error)
          ...
          {showDaemonRestart ? (
            ...
            <>
              ...
              {translate(
                ...
                'If this persists, review the local logs and diagnostics.'
              )}
            </>
          ) : null}
          {!ssh && footer ? `\n\n${footer}` : null}
