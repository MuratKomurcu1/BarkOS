...
describe('terminal link action routing', () => {
  ...
  it('offers system browser first and Orca second when system browser is the default', () => {
    ...
    expect(request.mock.calls[0][0].alternate.label).toBe('BarkOS Browser')
  ...
  it('offers Orca first and system browser second when Orca is the default', () => {
    ...
    expect(request.mock.calls[0][0].primary.label).toBe('BarkOS Browser')
  ...
  it('routes an explicit BarkOS Browser action to the owning runtime', () => {
    const request = vi.fn()
