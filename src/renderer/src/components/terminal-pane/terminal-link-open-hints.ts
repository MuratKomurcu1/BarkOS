...
export function getTerminalOrcaFileOpenHint(showActions = true): string {
  ...
  return isMacPlatform()
    ? `${prefix}⌘+click to open in BarkOS`
    : `${prefix}Ctrl+click to open in BarkOS`
}
...
export function getTerminalUrlOpenHint(options: TerminalUrlOpenHintOptions = {}): string {
  ...
  if (invertsToOrca) {
    return isMacPlatform()
      ? `${prefix}⌘+click to open, or ⇧⌘+click to open in BarkOS`
      : `${prefix}Ctrl+click to open, or Shift+Ctrl+click to open in BarkOS`
  }
  return isMacPlatform()
...
export function getTerminalUrlOrcaBrowserHint(): string {
  return isMacPlatform() ? '⇧⌘+click to open in BarkOS' : 'Shift+Ctrl+click to open in BarkOS'
}
