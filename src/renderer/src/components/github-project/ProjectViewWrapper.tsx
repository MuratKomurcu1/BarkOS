      ...
      {views.map((v) => {
        ...
        const layoutLabel =
          v.layout === 'BOARD_LAYOUT'
            ? translate('barkos.github.projectView.board', 'Board')
            : v.layout === 'ROADMAP_LAYOUT'
              ? translate('barkos.github.projectView.roadmap', 'Roadmap')
              : translate('barkos.github.projectView.table', 'Table')
        const unsupportedMessage = translate(
          'barkos.github.projectView.unsupported',
          'BarkOS does not support {{value0}} project views yet.',
          { value0: layoutLabel }
        )
        const Icon =
          v.layout === 'BOARD_LAYOUT'
        ...
        const tab = (
          <button
            ...
            disabled={!supported}
            onClick={() => onPick(v.id)}
            title={supported ? v.name : `${v.name} — ${unsupportedMessage}`}
            className={cn(
          ...
          <HoverCard key={v.id} openDelay={200} closeDelay={100}>
            <HoverCardTrigger asChild>
              <span
                tabIndex={0}
                aria-label={translate(
                  'barkos.github.projectView.accessibleUnsupported',
                  '{{value0}}. {{value1}}',
            ...
            <HoverCardContent side="bottom" align="start" sideOffset={8} className="w-72 p-3">
              <div className="space-y-2">
                <p className="text-xs leading-5 text-muted-foreground">
                  {unsupportedMessage}{' '}
                  {translate(
                    'barkos.github.projectView.useTable',
                    'Switch to a Table view to work with this project in BarkOS.'
                  )}
                </p>
              </div>
            </HoverCardContent>
