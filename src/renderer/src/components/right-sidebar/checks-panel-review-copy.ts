...
export function concurrentLookupDetail(input: ChecksPanelReviewStateInput): string | undefined {
  ...
  if (input.reviewLookup === 'positive_unresolved') {
    return translate(
      ...
      'BarkOS also has saved {{reviewLabel}} information that it could not verify.',
      { reviewLabel }
    )
  }
  if (isRateLimitRefresh(refresh)) {
    return translate(
      ...
      'BarkOS also could not check {{reviewLabel}} status because {{provider}} is temporarily limiting requests.',
  ...
  if (refresh?.errorType === 'network') {
    return translate(
      ...
      'BarkOS also could not check {{reviewLabel}} status because this environment could not reach {{provider}}.',
      { reviewLabel, provider: providerName }
    )
  }
  if (refresh?.status === 'error' || isHardRefreshError(refresh)) {
    return translate(
      ...
      'BarkOS also could not confirm whether this branch already has a {{reviewLabel}}.',
      { reviewLabel }
    )
  }
  return undefined
...
export function transientRefreshState(
  ...
  if (refresh?.errorType === 'unknown') {
    return {
      ...
      description: translate(
        ...
        'The lookup failed, so BarkOS could not confirm whether this branch already has a {{reviewLabel}}.',
        { reviewLabel }
      )
    }
  }
  return {
    ...
    description: translate(
      ...
      'BarkOS could not confirm whether this branch already has a {{reviewLabel}}. Retry to check again.',
      { reviewLabel }
    )
  }
}
...
> = {
  ...
  gh_unavailable: {
    ...
    body: {
      ...
      fallback:
        'BarkOS could not run {{provider}} CLI in this environment. Set it up here, then retry.'
    }
  }
}
...
> = {
  disconnected: {
    ...
    body: {
      ...
      fallback:
        "This repository's execution host is disconnected, so BarkOS cannot refresh {{reviewLabel}} status."
    },
    recovery: ['retry']
  },
  bare: {
  ...
  archived: {
    ...
    body: {
      ...
      fallback: 'This repository is archived, so BarkOS is not refreshing {{reviewLabel}} status.'
    },
    recovery: []
  },
  'not-git': {
    ...
    body: {
      ...
      fallback: 'BarkOS could not treat this folder as a Git repository for {{reviewLabel}} status.'
    },
    recovery: []
  },
  remote: {
    ...
    body: {
      ...
      fallback:
        'BarkOS could not refresh {{reviewLabel}} status for this remote context. Retry after the host is available.'
    },
    recovery: ['retry']
  }
}
