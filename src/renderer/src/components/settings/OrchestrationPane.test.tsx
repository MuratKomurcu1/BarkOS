...
const INSTALL_COMMAND = 'barkos skills install --skill orchestration --global'
const UPDATE_COMMAND = 'barkos skills update --skill orchestration --global'
// Why: the Windows wrapper only guards `npx`-based commands against a missing
// npx; the BarkOS CLI ships with the app, so the command passes through bare.
  ...
  it('passes update commands to the main panel without an installed manual-copy path', async () => {
    ...
    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({
        ...
        // Why: an installed skill refreshes via the local update command; the
        // old Windows reinstall hack no longer applies to bundled installs.
