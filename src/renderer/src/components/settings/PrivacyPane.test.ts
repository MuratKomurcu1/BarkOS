...
//
//   1. BarkOS private builds keep inherited telemetry disabled.
...
describe('PrivacyPane — BarkOS private build', () => {
  beforeEach(() => {
  ...
  it.each([true, false, null])('keeps telemetry disabled when optedIn is %s', (optedIn) => {
    const markup = renderToStaticMarkup(
      React.createElement(PrivacyPane, {
        settings: buildSettings({ optedIn })
      })
    )
    expect(markup).toContain('aria-checked="false"')
    expect(markup.match(/<button[^>]*role="switch"[^>]*>/)?.[0]).toContain('disabled=""')
    expect(markup).toContain('BarkOS does not transmit usage analytics')
  })
})
