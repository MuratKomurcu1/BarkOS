...
describe('CliSkillRuntimeSetup runtime helpers', () => {
  ...
  it('preflights npx before Windows-host skill installs', () => {
    // Why: legacy npx-shaped commands still get the missing-npx guard.
    const npxCommand = 'npx skills add orchestration --global'
    ...
    ).toBe(`${windowsNpxPreflightPrefix}${windowsNpxGuidance}) else (${npxCommand})"`)
  })
  ...
  it('treats missing runtime as a preflighted Windows host fallback for npx installs', () => {
    const npxCommand = 'npx skills add orchestration --global'

    expect(buildSkillCommandForRuntime(npxCommand, undefined, 'win32')).toBe(
      `${windowsNpxPreflightPrefix}${windowsNpxGuidance}) else (${npxCommand})"`
    )
  })

  it('reinstalls Windows-host skill updates as a bundled install without the npx preflight', () => {
  ...
  it('skips the Windows preflight when the configured Windows shell is POSIX-family', () => {
    ...
    try {
      ...
      // cmd-family shells still preflight legacy npx commands.
      useAppStore.setState({
        settings: { ...getDefaultSettings('/tmp'), terminalWindowsShell: 'cmd.exe' }
      })
      expect(
        buildSkillCommandForRuntime('npx skills add orchestration --global', windowsHost, 'win32')
      ).toBe(
        `${windowsNpxPreflightPrefix}${windowsNpxGuidance}) else (npx skills add orchestration --global)"`
      )
    } finally {
  ...
  it('keeps the npx preflight in the PowerShell-forced setup terminal', () => {
    ...
    try {
      const copied = buildSkillCommandForRuntime(npxCommand, windowsHost, 'win32')
      // A Git Bash paste target must not receive cmd.exe switches.
      expect(copied).toBe(npxCommand)
