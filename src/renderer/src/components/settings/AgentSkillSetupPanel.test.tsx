...
const INSTALL_COMMAND =
  'npx skills add https://github.com/stablyai/orca --skill barkos-cli --global'
const UPDATE_COMMAND = 'npx skills update barkos-cli --global'
...
describe('AgentSkillSetupPanel', () => {
  ...
  it('surfaces skill freshness under hideHeader for guided setup hubs', () => {
    const html = renderPanel({
      ...
      hideHeader: true,
      description: null,
      freshnessSkillName: 'barkos-linear'
    })

    expect(html).toContain('data-testid="skill-freshness"')
    expect(html).toContain('barkos-linear')
  ...
  it('keeps update copy until the installed panel checks CLI prerequisites', () => {
    const html = renderPanel({
      ...
      preInstallNotice: 'Install the BarkOS CLI before running agent skill setup.'
    })
  ...
  it('refreshes first-install freshness only after the terminal re-check finishes', async () => {
    ...
    await renderInteractivePanel({ freshnessSkillName: 'barkos-cli', onRecheck })
    await clickButton('Install')
  ...
  it('keeps the command failure authoritative over presence discovery', async () => {
    await renderInteractivePanel({ freshnessSkillName: 'barkos-cli' })
    await clickButton('Install')

    await act(async () => {
      mocks.terminalProps.at(-1)?.onCommandFinished?.(1)
    })
    await rerenderInteractivePanel({ installed: true, freshnessSkillName: 'barkos-cli' })
