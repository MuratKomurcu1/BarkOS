...
describe('getFeatureWallSetupProgress', () => {
  ...
  it('does not mark the default-agent step done unless the saved agent is really detected', () => {
    const savedButNotDetected = getFeatureWallSetupProgress(
      makeInput({
        settings: { defaultTuiAgent: 'codex' } as never,
        detectedTuiAgents: ['claude']
      })
    )

    expect(savedButNotDetected.stepDone['default-agent']).toBe(false)

    const detected = getFeatureWallSetupProgress(
      makeInput({
        settings: { defaultTuiAgent: 'codex' } as never,
        detectedTuiAgents: ['claude', 'codex']
      })
    )

    expect(detected.stepDone['default-agent']).toBe(true)
  })
