...
describe('SkillFreshnessUpdateDialog', () => {
  ...
  it('shows the up-to-date state once every installation is current', async () => {
    ...
    expect(container?.textContent).toContain('All installed BarkOS skills are up to date.')
  ...
  it('names the skill in the stale-record remedy so the command is runnable as-is', async () => {
    ...
    expect(container?.textContent).toContain('barkos skills install --skill orchestration --global')
  })
  ...
  it('keeps the stale-record remedy when a project copy is listed beside it', async () => {
    ...
    expect(row?.textContent).toContain('barkos skills install --skill orchestration --global')
  ...
  it('stays coherent while an idle re-scan is in flight', async () => {
    ...
    expect(container?.textContent).toContain('Checking installed BarkOS skills…')
  ...
  it('shows incomplete plugin coverage without presenting a fabricated skill copy', async () => {
    ...
    expect(container?.textContent).toContain(
      'BarkOS could not finish checking plugin-managed skills.'
    )
    ...
    expect(container?.textContent).not.toContain('All installed BarkOS skills are up to date.')
