...
export function SkillInstallDialog({
  ...
  const resolveLink = useCallback(async (value: string): Promise<void> => {
    ...
    if (!shareId) {
      setError('Enter a BarkOS skill share link.')
      return
    }
    setBusy(true)
    setError(null)
  ...
  const install = async (discardLocal = false): Promise<void> => {
    ...
    try {
      ...
      if (operation.status !== 'ok') {
        setError(
          operation.status === 'reconnect-required'
            ? 'Reconnect your BarkOS account before installing.'
            : operation.message
        )
        return
      }
      ...
      setError('Installation failed before BarkOS could verify the requested version.')
    } finally {
