...
export function SkillInstallManagementDialog({
  ...
  const load = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      setError('BarkOS could not inspect managed installs on this machine.')
    } finally {
      if (generation === loadGeneration.current) {
        setBusy(false)
      }
      ...
      if (operation.status !== 'ok') {
        setError(
          operation.status === 'reconnect-required'
            ? 'Reconnect your BarkOS account to load version history.'
            : operation.message
        )
        return
      }
    ...
    try {
      ...
      if (bundleManifest) {
        ...
        if (operation.status !== 'ok') {
          setError(
            operation.status === 'reconnect-required'
              ? 'Reconnect your BarkOS account before changing versions.'
              : operation.message
          )
          return
        }
      ...
      setError('BarkOS could not verify the requested version.')
    } finally {
  ...
  const remove = async (discardLocal = false): Promise<void> => {
    ...
    try {
      ...
      setError('BarkOS could not safely remove this skill.')
    } finally {
      setBusy(false)
    }
  }
