...
async function inspectSkill(expectedDescription = 'A private skill'): Promise<void> {
  fireEvent.change(screen.getByLabelText('BarkOS skill link'), {
...
describe('SkillInstallDialog', () => {
  ...
  it('focuses the link first and programmatically names destination controls', async () => {
    ...
    expect(document.activeElement).toBe(screen.getByRole('textbox', { name: 'BarkOS skill link' }))
    ...
    await user.type(
      screen.getByRole('textbox', { name: 'BarkOS skill link' }),
