...
describe('LinuxPackageInstallRecoveryCard copy', () => {
  it('leads with the recovery copy and the three dedicated actions', () => {
    ...
    expect(
      screen.getByText(/a system terminal on the computer where BarkOS is installed/)
    ).toBeTruthy()
    expect(screen.getByText(/quit and reopen BarkOS to run the new version/)).toBeTruthy()
...
describe('LinuxPackageInstallRecoveryCard details', () => {
  ...
  it('scopes the trust note to the moment the command is built', () => {
    ...
    const detail = screen.getByText(
      /BarkOS checks the downloaded file against the release metadata/
    )
    expect(detail.textContent).toContain('at the moment it builds this command')
    expect(detail.textContent).toContain(
      'The system package itself is not signature-checked, and BarkOS cannot vouch for the file ' +
        'after that point.'
    )
  })
