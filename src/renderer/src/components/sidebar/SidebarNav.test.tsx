...
function setSidebarState({
  ...
  mocks.state = {
    ...
    openCompanyPage: mocks.openCompanyPage,
...
describe('SidebarNav', () => {
  ...
  it('opens Company from the sidebar', async () => {
    const container = await renderSidebarNav()

    await clickButton(getButtonByText(container, 'Company'))

    expect(mocks.openCompanyPage).toHaveBeenCalledOnce()
  })
  ...
  it('updates localized labels when the language changes after mount', async () => {
    ...
    expect(queryButtonByText(container, 'BarkOS Mobile')).not.toBeNull()
    ...
    expect(queryButtonByText(container, 'BarkOS 手机端')).not.toBeNull()
  })
  ...
  it('shows the inline hide control only once a device is paired', async () => {
    ...
    expect(queryButtonByText(beforePairing, 'BarkOS Mobile')).not.toBeNull()
  ...
  it('hides Mobile from its sidebar context menu', async () => {
    ...
    const mobileMenu = getButtonByText(container, 'BarkOS Mobile').closest(
      '[data-testid="context-menu"]'
    )
