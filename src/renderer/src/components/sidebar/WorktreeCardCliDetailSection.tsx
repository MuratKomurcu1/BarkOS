  ...
  return (
    <WorktreeCardDetailSection>
      <DetailHeader
        ...
        label={translate('auto.components.sidebar.WorktreeCardMeta.cliHeader', 'BarkOS CLI')}
      />
        ...
        <div className="text-[13px] font-semibold leading-snug text-foreground break-words">
          {provenance.callerTerminalHandle
            ? translate(
                ...
                'Created by an agent via `barkos worktree create`'
              )
            : translate(
                'auto.components.sidebar.WorktreeCardMeta.cliCreatedFromShell',
                'Created via `barkos worktree create`'
              )}
        </div>
        {agentLabel ? (
