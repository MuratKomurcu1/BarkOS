...
const BUILT_IN_WORKSPACE_STATUS_LABELS: Record<string, readonly string[]> = {
  todo: ['Todo', 'Yapılacak'],
  'in-progress': ['In progress', 'Devam ediyor'],
  'in-review': ['In review', 'İncelemede'],
  completed: ['Done', 'Completed', 'Tamamlandı']
}

const BUILT_IN_WORKSPACE_STATUS_TURKISH_LABELS: Record<string, string> = {
  todo: 'Yapılacak',
...
export function getWorkspaceStatusDisplayLabel(status: WorkspaceStatusDefinition): string {
  return BUILT_IN_WORKSPACE_STATUS_LABELS[status.id]?.includes(status.label)
    ? (BUILT_IN_WORKSPACE_STATUS_TURKISH_LABELS[status.id] ?? status.label)
    : status.label
}
