...
/**
 * Re-runs the setup-script prompt inspection when a shared `barkos.yaml` setup hook
...
export function useSetupScriptPromptRevalidation(input: {
  ...
  // Why: barkos.yaml is edited on disk or the hook runs in a terminal outside React
  ...
  useEffect(() => {
    ...
    // Why: a runtime reconnect can turn an unreadable barkos.yaml into a readable one,
