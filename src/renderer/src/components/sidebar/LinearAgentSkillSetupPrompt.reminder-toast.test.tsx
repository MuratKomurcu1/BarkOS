...
describe('LinearAgentSkillSetupPrompt reminder toast', () => {
  ...
  it('shows a warning toast on a later modal-only activation after a casual close', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      'BarkOS CLI and Linear skill are missing',
      expect.objectContaining({
        id: 'linear-agent-skill-setup-orca.linearTicketsSkill.setupDismissed.host',
        description:
          'Install the BarkOS CLI and the Linear skill to enable your agents to read and edit Linear tasks.',
        action: {
          label: 'Set up',
  ...
  it('does not repeat the Orca CLI in CLI-only reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      'BarkOS CLI is missing',
      expect.objectContaining({
        description: 'Install the BarkOS CLI to enable your agents to read and edit Linear tasks.'
      })
    )
  })
  ...
  it('keeps remote setup nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the BarkOS CLI and the Linear skill to enable your agents to read and edit Linear tasks. Remote agent environments may need their own setup.'
      })
    )
  })
  ...
  it('keeps WSL target nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the BarkOS CLI and the Linear skill to enable your agents to read and edit Linear tasks. This setup runs in the selected WSL agent runtime.'
      })
    )
  })
