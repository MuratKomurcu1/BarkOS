  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from BarkOS. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from BarkOS. Git worktrees will also be removed from git and disk; folder workspaces will only remove the BarkOS workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from BarkOS. The project folder on disk will not be deleted.'
