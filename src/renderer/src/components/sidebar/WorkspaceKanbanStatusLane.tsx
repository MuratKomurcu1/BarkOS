...
import { getWorkspaceStatusDisplayLabel, getWorkspaceStatusVisualMeta } from './workspace-status'
...
function WorkspaceKanbanStatusLane({
  ...
  const statusLabel = getWorkspaceStatusDisplayLabel(status)
  ...
  const createTooltip = canCreateWorktree
    ? `${statusLabel} durumunda yeni çalışma alanı`
    : 'Çalışma alanı oluşturmak için proje ekleyin'
  const createButton = (
    <Button
      type="button"
      variant="ghost"
