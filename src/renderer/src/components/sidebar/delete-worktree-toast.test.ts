...
describe('getDeleteWorktreeToastCopy', () => {
  ...
  it('uses terminal-teardown guidance when a PTY stop could not be proven', () => {
    expect(
      ...
      title: 'Failed to delete workspace feature/foo',
      description:
        'BarkOS could not confirm every terminal in this workspace has exited, so it stopped before deleting any files. Use Force Delete to remove it anyway.',
      isDestructive: false
    })
  })
  ...
  it('names the running terminals when verification proved they are still live', () => {
    expect(
      ...
      title: 'Failed to delete workspace feature/foo',
      description:
        'This workspace still has running terminals, so BarkOS stopped before deleting any files. Force Delete will kill them and discard any uncommitted work they hold.',
      isDestructive: false
    })
  })
  ...
  it('uses stale-row guidance when Git already removed the worktree directory', () => {
    expect(
      ...
      title: 'Failed to delete workspace feature/foo',
      description: 'Git already removed this workspace. Use Force Delete to clear it from BarkOS.',
      isDestructive: false
    })
  })
