...
import {
  ...
  getWorkspaceStatusDisplayLabel,
...
describe('workspace status labels', () => {
  it('shows built-in workflow labels in Turkish without changing custom labels', () => {
    expect(getWorkspaceStatusDisplayLabel({ id: 'todo', label: 'Todo' })).toBe('Yapılacak')
    expect(getWorkspaceStatusDisplayLabel({ id: 'in-progress', label: 'In progress' })).toBe(
      'Devam ediyor'
    )
    expect(getWorkspaceStatusDisplayLabel({ id: 'in-review', label: 'In review' })).toBe(
      'İncelemede'
    )
    expect(getWorkspaceStatusDisplayLabel({ id: 'completed', label: 'Done' })).toBe('Tamamlandı')
    expect(getWorkspaceStatusDisplayLabel({ id: 'todo', label: 'Özel sıra' })).toBe('Özel sıra')
  })
})
