...
export class WebRuntimeClient {
  ...
  async call(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!this.sendEncrypted({ id, deviceToken: this.pairing.deviceToken, method, params })) {
        ...
        reject(new Error('Remote BarkOS runtime is not connected.'))
      }
    })
  }
  ...
  private async subscribeOnCurrentConnection(
    ...
    if (!this.sendEncrypted({ id, deviceToken: this.pairing.deviceToken, method, params })) {
      ...
      throw new Error('Remote BarkOS runtime is not connected.')
    }
    return {
      unsubscribe: () => {
  ...
  close(options: { notifySubscriptions?: boolean } = {}): void {
    ...
    this.clearTimers()
    this.rejectAllPending('Remote BarkOS runtime connection closed.')
    this.rejectAllWaiters(new Error('Remote BarkOS runtime connection closed.'))
  ...
  private waitForConnected(timeoutMs = REQUEST_TIMEOUT_MS): Promise<void> {
    ...
    if (this.intentionallyClosed) {
      return Promise.reject(new Error('Remote BarkOS runtime connection closed.'))
    }
    return new Promise((resolve, reject) => {
      const timeout = window.setTimeout(() => {
        ...
        reject(
          new Error(
            withRemoteRuntimeTailscaleHint(
              'Timed out while connecting to the remote BarkOS runtime.',
              this.pairing.endpoint
            )
          )
        )
  ...
  private handleSocketClosed(closedWs: WebSocket): void {
    ...
    this.rejectAllPending('Remote BarkOS runtime connection interrupted.')
