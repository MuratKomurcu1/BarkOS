...
import { parseBarkosCompany, safeParseBarkosCompany } from '../../../shared/barkos/company'
import {
...
const BARKOS_COMPANY_STORAGE_KEY = 'barkos.web.company.v1'
const BARKOS_CONTROL_POLICY_STORAGE_KEY_PREFIX = 'barkos.web.control-policy.v1'
const BARKOS_DECISION_INBOX_STORAGE_KEY_PREFIX = 'barkos.web.decision-inbox.v1'
const BARKOS_MEMORY_VAULT_STORAGE_KEY_PREFIX = 'barkos.web.memory-vault.v1'
const BARKOS_PROVIDER_CAPACITY_STORAGE_KEY_PREFIX = 'barkos.web.provider-capacity.v1'
const BARKOS_WORKER_SESSION_STORAGE_KEY_PREFIX = 'barkos.web.worker-sessions.v1'
const BARKOS_WORK_LEDGER_STORAGE_KEY_PREFIX = 'barkos.web.work-ledger.v1'
const recoveredBarkosDecisionInboxGenerations = new Set<string>()
const recoveredBarkosProviderCapacityGenerations = new Set<string>()

function barkosWorkerSessionStorageKey(companyId: string): string {
  return `${BARKOS_WORKER_SESSION_STORAGE_KEY_PREFIX}.${companyId}`
}

function barkosControlPolicyStorageKey(companyId: string): string {
  return `${BARKOS_CONTROL_POLICY_STORAGE_KEY_PREFIX}.${companyId}`
}

function barkosDecisionInboxStorageKey(companyId: string): string {
  return `${BARKOS_DECISION_INBOX_STORAGE_KEY_PREFIX}.${companyId}`
}

function barkosMemoryVaultStorageKey(companyId: string): string {
  return `${BARKOS_MEMORY_VAULT_STORAGE_KEY_PREFIX}.${companyId}`
}

function barkosProviderCapacityStorageKey(companyId: string): string {
  return `${BARKOS_PROVIDER_CAPACITY_STORAGE_KEY_PREFIX}.${companyId}`
}

function barkosWorkLedgerStorageKey(companyId: string): string {
  return `${BARKOS_WORK_LEDGER_STORAGE_KEY_PREFIX}.${companyId}`
}
...
function createWebPreloadApi(): Partial<PreloadApi> {
  const webOrcaProfileAuthStatus = () =>
    Promise.resolve({
      ...
      setupMessage: 'BarkOS Cloud sign-in is not available in the browser fallback.'
    })
  ...
  return {
    barkosCompany: {
      load: async () => {
        ...
        const result = safeParseBarkosCompany(JSON.parse(serialized))
        if (!result.success) {
          throw new Error('Stored BarkOS company failed contract validation')
        }
        return result.data
      },
      save: async (company) => {
        const validated = parseBarkosCompany(company)
        window.localStorage.setItem(BARKOS_COMPANY_STORAGE_KEY, JSON.stringify(validated))
        return validated
      },
      archive: async () => {
        const serialized = window.localStorage.getItem(BARKOS_COMPANY_STORAGE_KEY)
        ...
        const company = parseBarkosCompany(JSON.parse(serialized))
        window.localStorage.removeItem(BARKOS_COMPANY_STORAGE_KEY)
        return company
      },
      exportCurrent: async () => {
        const serialized = window.localStorage.getItem(BARKOS_COMPANY_STORAGE_KEY)
        ...
        const company = parseBarkosCompany(JSON.parse(serialized))
        const memoryValue = window.localStorage.getItem(barkosMemoryVaultStorageKey(company.id))
        const memoryVault = memoryValue
          ? parseBarkosMemoryVaultForCompany(JSON.parse(memoryValue), company)
          : createEmptyBarkosMemoryVault(company.id, company.createdAt)
        const backup = createBarkosBackupBundle({ company, memoryVault })
        const fileName = `${company.name.replace(/[^a-z0-9_-]+/gi, '-').replace(/^-+|-+$/g, '') || 'company'}.barkos.backup.json`
      ...
      pickImport: async () => {
        throw new Error('BarkOS company import is available in the desktop app')
      },
      applyImport: async (value) => {
        const backup = parseBarkosBackupImport(value)
        window.localStorage.setItem(BARKOS_COMPANY_STORAGE_KEY, JSON.stringify(backup.company))
        window.localStorage.setItem(
          barkosMemoryVaultStorageKey(backup.company.id),
          JSON.stringify(backup.memoryVault)
        )
        return backup
      }
    },
    barkosDecisionInbox: {
      load: async () => {
        const companyValue = window.localStorage.getItem(BARKOS_COMPANY_STORAGE_KEY)
        if (!companyValue) {
          return null
        }
        const company = parseBarkosCompany(JSON.parse(companyValue))
        const serialized = window.localStorage.getItem(barkosDecisionInboxStorageKey(company.id))
        if (!serialized) {
          return null
        }
        const storageKey = barkosDecisionInboxStorageKey(company.id)
        ...
        if (
          ...
          value.companyCreatedAt !== company.createdAt
        ) {
        ...
        const inbox = parseBarkosDecisionInboxForCompany(value, company)
        const generationKey = `${company.id}:${company.createdAt}`
        if (recoveredBarkosDecisionInboxGenerations.has(generationKey)) {
          return inbox
        }
        ...
        recoveredBarkosDecisionInboxGenerations.add(generationKey)
        return recovered
      },
      save: async (inbox: BarkosDecisionInbox) => {
        const companyValue = window.localStorage.getItem(BARKOS_COMPANY_STORAGE_KEY)
        if (!companyValue) {
          throw new Error('barkos_company_not_found')
        }
        const company = parseBarkosCompany(JSON.parse(companyValue))
        const validated = parseBarkosDecisionInboxForCompany(inbox, company)
        const storageKey = barkosDecisionInboxStorageKey(company.id)
        const currentValue = window.localStorage.getItem(storageKey)
        if (currentValue) {
          const current = parseBarkosDecisionInboxForCompany(JSON.parse(currentValue), company)
          if (validated.revision !== current.revision + 1) {
            throw new Error(
              `BarkOS decision inbox revision ${validated.revision} does not follow stored revision ${current.revision}`
            )
          }
        }
      ...
      resolveSideEffect: async () => {
        throw new Error('BarkOS side-effect approval is unavailable on a paired web runtime')
      },
      preparePairedSideEffectApproval: async () => false
    },
    barkosMemoryVault: {
      load: async () => {
        const companyValue = window.localStorage.getItem(BARKOS_COMPANY_STORAGE_KEY)
        ...
        const company = parseBarkosCompany(JSON.parse(companyValue))
        const storageKey = barkosMemoryVaultStorageKey(company.id)
      ...
      save: async (vault: BarkosMemoryVault) => {
        const companyValue = window.localStorage.getItem(BARKOS_COMPANY_STORAGE_KEY)
        if (!companyValue) {
          throw new Error('barkos_company_not_found')
        }
        const company = parseBarkosCompany(JSON.parse(companyValue))
        const validated = parseBarkosMemoryVaultForCompany(vault, company)
        const storageKey = barkosMemoryVaultStorageKey(company.id)
        const currentValue = window.localStorage.getItem(storageKey)
        if (currentValue) {
          const current = parseBarkosMemoryVaultForCompany(JSON.parse(currentValue), company)
          if (validated.revision !== current.revision + 1) {
            throw new Error(
              `BarkOS memory vault revision ${validated.revision} does not follow stored revision ${current.revision}`
            )
          }
        }
        window.localStorage.setItem(storageKey, JSON.stringify(validated))
    ...
    barkosControlPolicy: {
      load: async () => {
        ...
        const company = parseBarkosCompany(JSON.parse(companyValue))
        const storageKey = barkosControlPolicyStorageKey(company.id)
        const serialized = window.localStorage.getItem(storageKey)
        if (!serialized) {
          return null
        }
      ...
      save: async (policy) => {
        ...
        const currentValue = window.localStorage.getItem(storageKey)
        if (currentValue) {
          const current = parseBarkosControlPolicyForCompany(JSON.parse(currentValue), company)
          if (validated.revision !== current.revision + 1) {
            throw new Error(
              `BarkOS control policy revision ${validated.revision} does not follow stored revision ${current.revision}`
            )
          }
        }
        window.localStorage.setItem(storageKey, JSON.stringify(validated))
    ...
    barkosProviderCapacity: {
      load: async () => {
        ...
        const company = parseBarkosCompany(JSON.parse(companyValue))
        const storageKey = barkosProviderCapacityStorageKey(company.id)
        const serialized = window.localStorage.getItem(storageKey)
        if (!serialized) {
          return null
        }
        ...
        const generationKey = `${company.id}:${company.createdAt}`
        if (recoveredBarkosProviderCapacityGenerations.has(generationKey)) {
          return ledger
        }
        const recovered = recoverInterruptedBarkosProviderFailovers({ ledger, company })
        if (recovered.changed) {
          window.localStorage.setItem(storageKey, JSON.stringify(recovered.ledger))
        }
        recoveredBarkosProviderCapacityGenerations.add(generationKey)
      ...
      save: async (ledger) => {
        ...
        const company = parseBarkosCompany(JSON.parse(companyValue))
        const validated = parseBarkosProviderCapacityLedgerForCompany(ledger, company)
        const storageKey = barkosProviderCapacityStorageKey(company.id)
        const currentValue = window.localStorage.getItem(storageKey)
        if (currentValue) {
          const current = parseBarkosProviderCapacityLedgerForCompany(
            JSON.parse(currentValue),
            company
          )
          if (validated.revision !== current.revision + 1) {
            throw new Error(
              `BarkOS provider capacity revision ${validated.revision} does not follow stored revision ${current.revision}`
            )
          }
        }
        window.localStorage.setItem(storageKey, JSON.stringify(validated))
    ...
    barkosUsageCost: {
      load: async () => null,
      sync: async () => {
        throw new Error('BarkOS provider usage accounting is available on the desktop host')
      }
    },
    barkosWorkerSessions: {
      load: async () => {
        ...
        const company = parseBarkosCompany(JSON.parse(companyValue))
        const storageKey = barkosWorkerSessionStorageKey(company.id)
        const serialized = window.localStorage.getItem(storageKey)
        if (!serialized) {
          return null
        }
        const reconciled = reconcileBarkosWorkerSessionSnapshot(JSON.parse(serialized), company)
        if (reconciled.changed) {
          window.localStorage.setItem(storageKey, JSON.stringify(reconciled.snapshot))
        }
        return reconciled.snapshot
      },
      record: async (binding) => {
        const companyValue = window.localStorage.getItem(BARKOS_COMPANY_STORAGE_KEY)
        if (!companyValue) {
          throw new Error('barkos_company_not_found')
        }
        ...
        const current = currentValue
          ? parseBarkosWorkerSessionSnapshotForCompany(JSON.parse(currentValue), company)
          : null
    ...
    barkosWorkLedger: {
      load: async () => {
        ...
        const company = parseBarkosCompany(JSON.parse(companyValue))
        const serialized = window.localStorage.getItem(barkosWorkLedgerStorageKey(company.id))
        if (!serialized) {
          return null
        }
        return parseBarkosWorkLedgerForCompany(JSON.parse(serialized), company)
      },
      save: async (ledger) => {
        const companyValue = window.localStorage.getItem(BARKOS_COMPANY_STORAGE_KEY)
        if (!companyValue) {
        ...
        const company = parseBarkosCompany(JSON.parse(companyValue))
        const validated = parseBarkosWorkLedgerForCompany(ledger, company)
        const storageKey = barkosWorkLedgerStorageKey(company.id)
        const currentValue = window.localStorage.getItem(storageKey)
        if (currentValue) {
          const current = parseBarkosWorkLedgerForCompany(JSON.parse(currentValue), company)
          if (validated.revision !== current.revision + 1) {
            throw new Error(
              `BarkOS work ledger revision ${validated.revision} does not follow stored revision ${current.revision}`
            )
          }
        }
        window.localStorage.setItem(storageKey, JSON.stringify(validated))
      ...
      pickScreenshot: async () => {
        throw new Error('BarkOS screenshot evidence selection is available in the desktop app')
      },
      runTest: async () => {
        throw new Error('BarkOS test execution is available in the desktop app')
      },
...
function createRuntimeEnvironmentsApi(): NonNullable<Partial<PreloadApi>['runtimeEnvironments']> {
  return {
    ...
    addFromPairingCode: async ({ name, pairingCode }) => {
      ...
      if (!offer) {
        throw new Error('Invalid BarkOS pairing code.')
      }
      ...
      try {
        ...
        return {
          ...
          message: translate(
            ...
            'Cannot reach BarkOS at {{endpoint}}.',
      ...
      try {
        ...
        return {
          ...
          message: translate(
            ...
            'BarkOS verified the host but could not save it. Check browser storage and try again.'
          )
        }
      }
      manuallyDisconnectedEnvironmentIds.clear()
...
function createCliApi(): NonNullable<Partial<PreloadApi>['cli']> {
  const status = {
    ...
    detail: 'CLI registration is managed on the BarkOS server, not in the web browser.'
  } as const
  return {
...
function createAgentHooksApi(): NonNullable<Partial<PreloadApi>['agentHooks']> {
  const status = (
    ...
    Promise.resolve({
      ...
      detail: 'Agent hook status is only available on the BarkOS server.'
    } as const)
  return {
  ...
  return {
    ...
    openSetup: (args) =>
      callRuntimeResult<ComputerUsePermissionSetupResult>(
        ...
        nextStep: 'Computer-use permissions are managed on the BarkOS server.'
      })),
    reset: () =>
      Promise.resolve({
        platform: getBrowserPlatform(),
...
function createAccountsApi(): never {
  ...
  return {
    ...
    prepareFailoverResume: () =>
      Promise.reject(new Error('Codex account failover resume is available on the desktop host')),
...
function resolveEnvironment(selector: string): StoredWebRuntimeEnvironment {
  ...
  throw new Error(`Unknown BarkOS runtime environment: ${selector}`)
}
...
function requireActiveEnvironment(): StoredWebRuntimeEnvironment {
  ...
  if (!activeEnvironment) {
    throw new Error('Pair this web client with a BarkOS server first.')
  }
...
function assertActiveEnvironment(environmentId: string): void {
  if (requireActiveEnvironment().id !== environmentId) {
    throw new Error('The paired BarkOS server changed while the request was in progress.')
  }
}
