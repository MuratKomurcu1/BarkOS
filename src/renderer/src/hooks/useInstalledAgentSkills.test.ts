...
describe('hasInstalledAgentSkill', () => {
  it('matches installed skills by summarized name', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'barkos-cli' })], 'barkos-cli')).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkill(
        [
          skill({
            name: 'BarkOS CLI',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\barkos-cli'
          })
        ],
        'barkos-cli'
      )
  ...
  it('ignores non-installed discovery entries', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'barkos-cli', installed: false })], 'barkos-cli')
    ).toBe(false)
  })
      ...
      hasInstalledAgentSkill(
        [
          skill({
            ...
            directoryPath: '/repo/.agents/skills/barkos-cli',
            skillFilePath: '/repo/.agents/skills/barkos-cli/SKILL.md'
          }),
          skill({
            id: 'skill-2',
            name: 'barkos-cli',
            ...
            directoryPath: '/Users/test/.codex/plugins/cache/vendor/barkos-cli',
            skillFilePath: '/Users/test/.codex/plugins/cache/vendor/barkos-cli/SKILL.md'
          })
        ],
        'barkos-cli',
  ...
  it('counts home skills when matching global installs', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'barkos-cli' })], 'barkos-cli', {
        sourceKinds: GLOBAL_AGENT_SKILL_SOURCE_KINDS
      })
    ).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkillNamed(
        [
          skill({
            name: 'Linear Tickets',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\barkos-linear'
          })
        ],
        ['barkos-linear', 'linear-tickets']
      )
  ...
  it('keeps aliases opt-in for unrelated single-name checks', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'linear-tickets' })], 'barkos-linear')).toBe(false)
  })
})
...
describe('discoverInstalledAgentSkills', () => {
  ...
  it('starts a fresh scan when a forced refresh arrives during a background scan', async () => {
    ...
    const freshResult = discoveryResult([skill({ name: 'barkos-cli' })])
  ...
  it('lets completed-scan broadcasts reuse the cached result', async () => {
    const discover = vi
      ...
      .mockResolvedValue(discoveryResult([skill({ name: 'barkos-linear' })]))
    vi.stubGlobal('window', {
      api: { skills: { discover } },
      dispatchEvent: vi.fn(),
      CustomEvent: class {}
    ...
    for (const pending of fromSubscribers) {
      await expect(pending).resolves.toMatchObject({
        skills: [expect.objectContaining({ name: 'barkos-linear' })]
      })
    }
    expect(discover).toHaveBeenCalledTimes(1)
  })
