...
describe('agent status provider failure', () => {
  it('stores the typed completion cause, preserves done repaints, and clears it on a new turn', () => {
    const store = createTestStore()
    const paneKey = 'tab-1:11111111-1111-4111-8111-111111111111'
    ...
    expect(store.getState().agentStatusByPaneKey[paneKey].providerFailure).toEqual({
    ...
    store.getState().setAgentStatus(paneKey, {
      state: 'working',
      prompt: 'Try the next task',
      agentType: 'codex'
    })
    expect(store.getState().agentStatusByPaneKey[paneKey].providerFailure).toBeUndefined()
  })
})
