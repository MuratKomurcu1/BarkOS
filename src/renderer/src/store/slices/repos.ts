...
async function assertProjectHostSetupRuntimeCapability(
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    PROJECT_HOST_SETUP_RUNTIME_CAPABILITY,
    'The selected BarkOS server does not support project host setup yet. Update BarkOS on the server and try again.',
    15_000
  )
}
...
async function assertProjectHostSetupMutationRuntimeCapabilities(
  ...
  await assertRuntimeEnvironmentCapability(
    target.environmentId,
    WORKSPACE_RUN_CONTEXT_RUNTIME_CAPABILITY,
    'The selected BarkOS server does not support explicit workspace run hosts yet. Update BarkOS on the server and try again.',
    15_000
  )
}
...
async function fetchRuntimeAddProjectPathStatus(args: {
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    translate(
      ...
      'Update BarkOS server to open non-Git folders on this runtime.'
    ),
    15_000
  )
  try {
    ...
    for (const projectId of targets.projectIds) {
      ...
      if (stillExists) {
        failedProjectRemovals.push({
          projectId,
          reason: 'Project remained in BarkOS after removeProject completed.'
        })
      } else {
