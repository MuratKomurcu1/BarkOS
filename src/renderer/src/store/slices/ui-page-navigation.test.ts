...
describe('createUISlice space navigation', () => {
  it('returns to the originating page after visiting Company', () => {
    const store = createUIStore()

    store.getState().openTaskPage()
    store.getState().openCompanyPage()

    expect(store.getState().activeView).toBe('company')
    expect(store.getState().previousViewBeforeCompany).toBe('tasks')

    store.getState().closeCompanyPage()
