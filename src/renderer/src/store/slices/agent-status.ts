    ...
    setAgentStatus: (paneKey, payload, terminalTitle, timing, routing, metadata) => {
      ...
      set((s) => {
        ...
        const entry: AgentStatusEntry = {
          ...
          // Why: done repaint/snapshot events may predate this optional field; retain the
          // exact structured cause only while the same completed turn remains authoritative.
          providerFailure:
            payload.providerFailure ??
            (existing?.state === 'done' &&
            payload.state === 'done' &&
            payload.prompt === existing.prompt
              ? existing.providerFailure
              : undefined),
        ...
        const doneRetentionFieldsChanged =
          ...
          (entry.prompt !== existing.prompt ||
            ...
            entry.providerFailure?.kind !== existing.providerFailure?.kind)
