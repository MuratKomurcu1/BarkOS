...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `BarkOS pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior BarkOS session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest BarkOS status hints:', ...statusHints] : []),
    '',
