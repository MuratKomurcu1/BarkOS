...
describe('emitBrowserCookieImportToast', () => {
  ...
  it('offers the in-app file import without recommending an exporter', () => {
    ...
    expect(message).toBe(
      "BarkOS cannot decrypt 3 of this browser's cookies because they use app-bound encryption. You can import cookies from a file using “From File…”."
    )
  ...
  it('shows separate host-specific Google guidance after success', () => {
    ...
    expect(warningToastMock).toHaveBeenCalledWith(
      'Google cookies were not imported. Open a browser in BarkOS on Remote Mac with this profile, then sign into Google.',
      { duration: 12000 }
    )
