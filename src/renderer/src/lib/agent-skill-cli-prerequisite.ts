...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, BarkOS may show a system prompt to register the BarkOS CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST_KEY =
  'auto.lib.agent.skill.cli.prerequisite.registrationToast'
export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'BarkOS needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION_KEY =
...
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the BarkOS CLI command.'
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the BarkOS CLI in PATH.'
          )
    )
    return null
  }
...
export async function showOrcaCliRegistrationPromptToast(delayMs = 700): Promise<void> {
  toast.message(
    translate(CLI_PREREQUISITE_REGISTRATION_TOAST_KEY, CLI_PREREQUISITE_REGISTRATION_TOAST),
    {
      description: translate(
        CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION_KEY,
