  ...
  it('keeps loading and missing states ahead of freshness', () => {
    expect(
      getAgentSkillNavInstallStatus({
        ...
        installed: true,
        loading: true,
        inventory: inventory([placement('barkos-linear', 'current')])
      })
    ).toBe('checking')
    expect(
      getAgentSkillNavInstallStatus({
...
describe('getLinearAgentSkillNavInstallStatus', () => {
  it('reports canonical installs under barkos-linear', () => {
    const skills = [skill('barkos-linear')]
    ...
    expect(
      getLinearAgentSkillNavInstallStatus({
        ...
        installed: true,
        loading: false,
        inventory: inventory([placement('barkos-linear', 'outdated')], ['barkos-linear'])
      })
  ...
  it('prefers canonical freshness when both names are installed', () => {
    expect(
      getLinearAgentSkillNavInstallStatus({
        skills: [skill('barkos-linear'), skill('linear-tickets')],
        installed: true,
        loading: false,
        inventory: inventory(
          [placement('barkos-linear', 'current'), placement('linear-tickets', 'outdated')],
          ['linear-tickets']
        )
      })
    ).toBe('up-to-date')
