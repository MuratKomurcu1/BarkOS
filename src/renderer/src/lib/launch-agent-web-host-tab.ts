...
export function launchAgentInWebHostTab(args: {
  ...
  pairedSideEffectApprovalVersion?: BarkosPairedSideEffectApprovalVersion
}): Promise<{ delivered: boolean; failureNotified: boolean; tabId: string | null }> {
  const {
    agent,
    worktreeId,
    environmentId,
  ...
  const launch = {
    ...
    onHostTabCreated: (hostTabId: string) => {
      const tabId = toWebTerminalSurfaceTabId(hostTabId)
      createdTabId = tabId
      onTabCreated?.(tabId)
    },
    ...(pairedSideEffectApprovalVersion !== undefined ? { pairedSideEffectApprovalVersion } : {})
  } as const
  ...
  const handleCreation = ({
  ...
  }): { delivered: boolean; failureNotified: boolean; tabId: string | null } => {
    ...
    if (outcome.status === 'failed') {
      ...
      return { delivered: false, failureNotified: true, tabId: createdTabId }
    }
    useAppStore.getState().setActiveTabType('terminal')
    if (hasPrompt && promptDelivered) {
      onPromptDelivered?.()
    }
    return { delivered: promptDelivered, failureNotified: false, tabId: createdTabId }
  }
