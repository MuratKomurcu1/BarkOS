...
describe('createWebRuntimeSessionTerminal', () => {
  ...
  it('creates paired web agents through host authority so activation is mirrored', async () => {
    ...
    const runtimeCall = vi
      ...
      .mockResolvedValueOnce({
        ...
        result: {
          ...
          runtimeProtocolVersion: 3,
          minCompatibleRuntimeClientVersion: 2,
          capabilities: ['agent-session.host-authority.v1', 'barkos.paired-side-effect-approval.v2']
        }
      })
      .mockResolvedValueOnce({
        id: 'create-terminal',
    ...
    await expect(
      createWebRuntimeSessionTerminal({
        ...
        command: "droid 'linked issue context'",
        cwd: '/repo/packages/app',
  ...
  it.each(['session.tabs.move', 'session.tabs.list'] as const)(
    ...
    async (failedMethod) => {
      ...
      expect(onHostTabCreated).toHaveBeenCalledTimes(1)
      expect(onHostTabCreated).toHaveBeenCalledWith('host-tab-created')
    }
  )
