...
import { barkosPairedApprovalCapabilityForVersion } from '../../../shared/barkos/paired-side-effect-approval'
import type {
...
type CreateWebRuntimeSessionTerminalArgs = {
  ...
  onHostTabCreated?: (hostTabId: string) => void
  pairedSideEffectApprovalVersion?: 1 | 2 | 3 | 4
}
...
async function createWebRuntimeSessionTerminalResult(
    ...
    return {
      outcome: {
        ...
        message: translate(
          ...
          'The workspace is not connected to a remote BarkOS host.'
        )
      }
    }
  }
  ...
  let createdTabPublished = false
  const publishCreatedTab = (): void => {
    if (!createdTabId || createdTabPublished) {
      return
    }
    createdTabPublished = true
    args.onHostTabCreated?.(createdTabId)
  }
  try {
    ...
    if (agent) {
                    ...
                    params: withAgentSessionCreateOperationId(
                      {
                        ...
                        ...(args.pairedSideEffectApprovalVersion !== undefined
                          ? {
                              sideEffectApproval: { version: args.pairedSideEffectApprovalVersion }
                            }
                          : {}),
                        presentation: 'background'
                      },
      ...
      const created = await runRemoteAgentSessionLaunch<{
        ...
        ...(args.agentSessionKind === 'resume' && agent === 'omp'
          ...
          : args.pairedSideEffectApprovalVersion !== undefined
            ? {
                hostAuthorityCapability: barkosPairedApprovalCapabilityForVersion(
                  args.pairedSideEffectApprovalVersion
                )
              }
            : {}),
        legacy: async () => {
