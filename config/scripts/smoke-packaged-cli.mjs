...
function readAppDirArg(argv) {
  ...
  if (process.platform === 'darwin') {
    return 'dist/mac-arm64/BarkOS.app'
  }
...
function getPackagedCliPath(appDir) {
  if (process.platform === 'darwin' || appDir.endsWith('.app')) {
    return join(appDir, 'Contents', 'Resources', 'bin', 'barkos')
  }
  if (process.platform === 'win32') {
    return join(appDir, 'resources', 'bin', 'barkos.exe')
  }
  return join(appDir, 'resources', 'bin', 'barkos')
}

const appDir = resolve(readAppDirArg(process.argv.slice(2)))
const tempRoot = await mkdtemp(join(tmpdir(), 'barkos-packaged-cli-smoke-'))
...
try {
  ...
  assert(list.topics.some((topic) => topic.name === 'barkos-cli'))
  assert.match((await run(['skills', 'get', 'barkos-cli'])).stdout, /name: barkos-cli/)
  ...
  const update = JSON.parse(
    (await run(['skills', 'update', '--skill', 'barkos-cli', '--dry-run', '--json'])).stdout
  )
...
// Why: on Windows the launcher above spawns the copied BarkOS.exe (and its crashpad/utility children)
