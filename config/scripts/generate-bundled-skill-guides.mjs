...
const GUIDE_ALIASES = {
  ...
  'linear-tickets': [],
  'barkos-cli': ['orca-cli'],
  'barkos-emulator': ['orca-emulator'],
  'barkos-emulator-android': ['orca-emulator-android'],
  'barkos-linear': ['orca-linear'],
  'barkos-per-workspace-env': ['orca-per-workspace-env'],
  orchestration: []
}

// Why: a stubbed topic ships a hybrid discovery stub as its installable projection while
// `barkos skills get <topic>` still serves the full version-matched guide from the binary.
...
async function buildArtifacts(repoRoot = REPO_ROOT) {
  ...
  return [
    {
      path: path.join(repoRoot, 'src', 'shared', 'bundled-skill-guides.ts'),
