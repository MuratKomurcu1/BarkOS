...
export function getRequiredReleaseAssetNames(tag) {
  ...
  return [
    ...
    `barkos_${version}_amd64.deb`,
    `barkos_${version}_arm64.deb`,
    `barkos-${version}.x86_64.rpm`,
    `barkos-${version}.aarch64.rpm`,
    'barkos-windows-setup.exe',
    'barkos-windows-setup.exe.blockmap',
    `BarkOS-${version}-mac.zip`,
    `BarkOS-${version}-mac.zip.blockmap`,
    `BarkOS-${version}-arm64-mac.zip`,
    `BarkOS-${version}-arm64-mac.zip.blockmap`,
    'barkos-macos-x64.dmg',
    'barkos-macos-x64.dmg.blockmap',
    'barkos-macos-arm64.dmg',
    'barkos-macos-arm64.dmg.blockmap'
  ]
}
...
async function main() {
  ...
  const repo = process.env.GITHUB_REPOSITORY || 'MuratKomurcu1/BarkOS'
