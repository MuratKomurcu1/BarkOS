  ...
  it('includes both mac updater ZIP names for the tag version', () => {
    expect(getRequiredReleaseAssetNames('v1.4.27')).toEqual(
      expect.arrayContaining([
        'BarkOS-1.4.27-mac.zip',
        'BarkOS-1.4.27-mac.zip.blockmap',
        'BarkOS-1.4.27-arm64-mac.zip',
        'BarkOS-1.4.27-arm64-mac.zip.blockmap'
      ])
    )
  })
  ...
  it('includes x64 and arm64 Linux assets', () => {
    expect(getRequiredReleaseAssetNames('v1.4.27')).toEqual(
      expect.arrayContaining([
        ...
        'barkos-linux.AppImage',
        'barkos-linux-arm64.AppImage',
        'barkos_1.4.27_amd64.deb',
        'barkos_1.4.27_arm64.deb',
        'barkos-1.4.27.x86_64.rpm',
        'barkos-1.4.27.aarch64.rpm'
      ])
    )
  })
})
  ...
  it('extracts relative and absolute manifest asset names', () => {
    expect(
      extractManifestAssetNames(
        [
          'files:',
          '  - url: BarkOS-1.4.27-arm64-mac.zip',
          '  - url: https://example.com/downloads/barkos-windows-setup.exe',
          'path: barkos-linux.AppImage'
        ].join('\n')
      )
    ).toEqual(['BarkOS-1.4.27-arm64-mac.zip', 'barkos-windows-setup.exe', 'barkos-linux.AppImage'])
  })
})
...
describe('verifyRequiredReleaseAssets', () => {
  it('fails when a manifest-referenced asset has not been uploaded', async () => {
    ...
    const assets = required.filter((name) => name !== 'BarkOS-1.4.27-arm64-mac.zip')
      ...
      .mockResolvedValueOnce(
        jsonResponse(
          [
            ...
            '  - url: BarkOS-1.4.27-arm64-mac.zip',
            '    sha512: test',
            'path: BarkOS-1.4.27-arm64-mac.zip'
          ].join('\n')
        )
      )
    ...
    ).rejects.toThrow('Missing: BarkOS-1.4.27-arm64-mac.zip')
  ...
  it('checks assets referenced by the Linux arm64 updater manifest', async () => {
      ...
      .mockResolvedValueOnce(
        jsonResponse(
          [
            'version: 1.4.27',
            'files:',
            '  - url: barkos-linux-arm64.AppImage.blockmap',
            'path: barkos-linux-arm64.AppImage'
          ].join('\n')
        )
      )
      .mockResolvedValue(jsonResponse('version: 1.4.27\n'))
    ...
    await expect(
      verifyRequiredReleaseAssets({ repo: 'stablyai/orca', tag, token: 'token' })
    ).rejects.toThrow('Missing: barkos-linux-arm64.AppImage.blockmap')
