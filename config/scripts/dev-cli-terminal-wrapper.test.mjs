...
describe('dev CLI terminal wrappers', () => {
  it('writes profile-scoped Windows wrappers for worker terminals', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'barkos-dev-terminal-wrapper-'))
    ...
    const wrapper = readFileSync(path.join(userDataPath, 'cli', 'bin', 'barkos-dev.cmd'), 'utf8')
    ...
    expect(readFileSync(path.join(root, 'out', 'bin', 'barkos-dev.cmd'), 'utf8')).toBe(wrapper)
  ...
  it('escapes literal percent signs in every Windows batch path', () => {
    const root = path.join(
      mkdtempSync(path.join(tmpdir(), 'barkos-dev-terminal-wrapper-')),
      '%repo%'
    )
    const userDataPath = path.join(root, '%profile%')
  ...
  it('writes executable-style POSIX wrappers with the same profile identity', () => {
    ...
    const wrapper = readFileSync(path.join(userDataPath, 'cli', 'bin', 'barkos-dev'), 'utf8')
    ...
    expect(readFileSync(path.join(root, 'out', 'bin', 'barkos-dev'), 'utf8')).toBe(wrapper)
