#!/usr/bin/env node
// Builds the BarkOS notification-status helper binary.
//
...
const bundleId = readArg('--bundle-id') ?? 'com.barkos.desktop'
...
const workDir = path.join(tmpdir(), `barkos-notification-status-${process.pid}`)
...
try {
  ...
  const builtBinaries = triples.map((triple) => {
    const output = path.join(workDir, `barkos-notification-status-${triple}`)
    execFileSync(
      'swiftc',
      [
        '-O',
...
<dict>
  ...
  <string>barkos-notification-status</string>
</dict>
</plist>
`
}
