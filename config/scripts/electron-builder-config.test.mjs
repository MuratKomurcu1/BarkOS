...
describe('electron-builder config', () => {
  ...
  it('owns only BarkOS desktop install identities', () => {
    ...
    expect(electronBuilderConfig.protocols).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({ schemes: expect.arrayContaining(['orca']) })
      ])
    )
  ...
  // resources/bin/barkos.cmd (beside the native resources/bin/barkos.exe). If the
  // source tree is also packed into app.asar it gets extracted by
  // asarUnpack:['resources/**'] to app.asar.unpacked/resources/win32/bin/barkos.cmd,
  // a duplicate with no adjacent barkos.exe that fails to launch (#7351).
  ...
  it('keeps legacy Orca launchers out of BarkOS packages', () => {
    expect(electronBuilderConfig.files).toEqual(
      expect.arrayContaining([
  ...
  it('ships the mac notification-status helper in Contents/MacOS, not Resources', () => {
    ...
    expect(electronBuilderConfig.mac.extraResources).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ to: 'barkos-notification-status' })])
    )
  })
  ...
  it('ships the mac keyboard-layout helper in Contents/MacOS, not Resources', () => {
    ...
    expect(electronBuilderConfig.mac.extraResources).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ to: 'barkos-keyboard-layout' })])
    )
  })
  ...
  it('matches the Linux desktop entry to Electron window class', () => {
    expect(electronBuilderConfig.linux.desktop.entry.StartupWMClass).toBe('barkos')
  })

  it('uses BarkOS package identities for Linux artifacts', () => {
    expect(electronBuilderConfig.linux.target).toEqual(['AppImage', 'deb'])
    expect(electronBuilderConfig.appImage.artifactName).toBe('barkos-linux.${ext}')
    expect(electronBuilderConfig.deb.artifactName).toBe('barkos_${version}_${arch}.${ext}')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      try {
        ...
        const launcherPath = join(resourcesDir, 'bin', 'barkos')
