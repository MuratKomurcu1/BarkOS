...
case "$entrypoint_kind" in
  ...
  launcher)
    ...
    entrypoint=("$app_root/resources/bin/barkos")
    ;;
...
if [[ "$signal_target_kind" == serving-electron ]]; then
  signal_target_pid=$(awk '/\/barkos .* --serve / {print $1; exit}' <<<"$tree_snapshot")
...
owned_residue=$(ps -eo pid=,ppid=,stat=,args= | awk -v state="$state_dir" \
  '($0 ~ state || $0 ~ /\/artifacts\/root\/barkos/ || $0 ~ /[X]vfb :99 /) && $0 !~ /awk -v state=/ {print}' || true)
