...
const localBuildCompatibility = require('../src/shared/local-build-compatibility-contract.json')

// Why: dev-channel builds must carry the *release* identity — same bundle id,
// Developer ID signature, and notarization ticket — or Squirrel.Mac refuses to
// swap them over an installed BarkOS and macOS treats each build as a new app.
...
const localMacBuildArch = process.env.ORCA_LOCAL_BUILD_ARCH
if (localMacBuildArch && localMacBuildArch !== 'x64' && localMacBuildArch !== 'arm64') {
  throw new Error(`Unsupported local macOS build architecture: ${localMacBuildArch}`)
}
const macBuildArchitectures = localMacBuildArch ? [localMacBuildArch] : ['x64', 'arm64']
...
// main's hourlies.
const appId = localBuildCompatibility.appId
...
module.exports = {
  ...
  files: [
    ...
    // Retain source compatibility in the repository without shipping obsolete
    // pre-BarkOS asset filenames in user installations.
    '!resources/app-icons/orca-{blue,watercolor}.png',
    '!resources/tray/orca-menu-barTemplate{,@2x}.png',
    '!resources/icon-source/**',
    '!resources/logo.svg',
    // Why: the Windows CLI shim ships via extraResources to resources/bin/barkos.cmd
    // (beside the native resources/bin/barkos.exe). Packing the source tree into
    // app.asar too lets asarUnpack:['resources/**'] extract a second copy at
    // app.asar.unpacked/resources/win32/bin/barkos.cmd with no adjacent barkos.exe,
    // which fails to launch the CLI (#7351).
    '!resources/win32{,/**/*}',
    // BarkOS launchers ship only through extraResources; exclude legacy Orca
    // source launchers so they cannot appear in app.asar.unpacked.
  ...
  afterPack: async (context) => {
    ...
    if (context.electronPlatformName === 'darwin') {
      await signMacComputerUseHelper(
        join(resourcesDir, 'BarkOS Computer Use.app'),
        context.packager
      )
      await signMacStandaloneHelper(
        join(resourcesDir, '..', 'MacOS', 'barkos-notification-status'),
        'barkos-notification-status',
        context.packager
      )
      await signMacStandaloneHelper(
        join(resourcesDir, '..', 'MacOS', 'barkos-keyboard-layout'),
        'barkos-keyboard-layout',
        context.packager
      )
    }
  },
  win: {
    ...
    extraResources: [
      ...
      {
        from: 'resources/win32/bin/barkos.cmd',
        to: 'bin/barkos.cmd'
      },
      {
        from: 'native/windows-cli-launcher/.build/barkos.exe',
        to: 'bin/barkos.exe'
      },
      {
  ...
  nsis: {
    artifactName: 'barkos-windows-setup.${ext}',
  ...
  mac: {
    ...
    extendInfo: {
      NSAppleEventsUsageDescription:
        'BarkOS allows terminal-launched developer tools to automate local apps when you request it.',
      NSBluetoothAlwaysUsageDescription:
        'BarkOS allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSBluetoothPeripheralUsageDescription:
        'BarkOS allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSCameraUsageDescription: "Application requests access to the device's camera.",
      NSLocationUsageDescription:
        'BarkOS allows terminal-launched developer tools to access location when you request it.',
      NSLocalNetworkUsageDescription:
        'BarkOS allows terminal-launched developer tools to discover and connect to local development servers when you request it.',
      NSMicrophoneUsageDescription: "Application requests access to the device's microphone.",
      NSAudioCaptureUsageDescription:
        'BarkOS allows terminal-launched developer tools to capture desktop audio when you request it.',
    ...
    extraResources: [
      ...
      {
        from: 'resources/darwin/bin/barkos',
        to: 'bin/barkos'
      },
      {
      ...
      {
        from: 'native/computer-use-macos/.build/release/BarkOS Computer Use.app',
        to: 'BarkOS Computer Use.app'
      },
      featureWallResources
    ],
    ...
    extraFiles: [
      {
        from: 'native/notification-status-macos/.build/release/barkos-notification-status',
        to: 'MacOS/barkos-notification-status'
      },
      {
        from: 'native/keyboard-layout-macos/.build/release/barkos-keyboard-layout',
        to: 'MacOS/barkos-keyboard-layout'
      }
    ],
    target: [
      {
        target: 'dmg',
        arch: macBuildArchitectures
      },
      {
        target: 'zip',
        arch: macBuildArchitectures
  ...
  linux: {
    ...
    desktop: {
      entry: {
        // Why: Electron reports WM_CLASS=barkos for the visible Linux window;
        // GNOME docks need the same value to group it with barkos.desktop.
    ...
    extraResources: [
      ...
      {
        from: 'resources/linux/bin/barkos',
        to: 'bin/barkos'
      },
      {
  ...
  appImage: {
    artifactName: isLinuxArm64Release ? 'barkos-linux-arm64.${ext}' : 'barkos-linux.${ext}'
  },
  deb: {
    packageName: 'barkos',
    ...
    // Why: symlink the bundled CLI onto PATH at install time so `barkos serve`
  ...
  rpm: {
    packageName: 'barkos',
    artifactName: 'barkos-${version}.${arch}.${ext}',
...
function chmodUnixCliLaunchers(resourcesDir, electronPlatformName) {
  ...
  for (const launcherName of ['barkos']) {
...
async function signMacComputerUseHelper(helperAppPath, packager) {
  if (!existsSync(helperAppPath)) {
    if (isMacRelease) {
      throw new Error(`Missing BarkOS Computer Use helper app at ${helperAppPath}`)
    }
    return
  }
  ...
  const identity =
    ...
    (isMacRelease ? null : '-')
  if (!identity) {
    throw new Error('Missing signing identity for BarkOS Computer Use helper app')
  }
  // Why: TCC grants attach to this nested app's code identity. Sign it before
  // the outer BarkOS.app is sealed so production builds preserve that identity.
