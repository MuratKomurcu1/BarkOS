#!/bin/bash
# Why: register the bundled `barkos` CLI on PATH at package-install time.
...
for dir in /opt/BarkOS /opt/barkos; do
  ...
  shim="$dir/resources/bin/barkos"
  if [ -x "$shim" ]; then
    # Only manage our own symlink; never clobber an unrelated /usr/bin/barkos.
