---
name: barkos-per-workspace-env
description: >-
  Set up, review, debug, or validate BarkOS per-workspace environment recipes —
  ...
  per-workspace environments, fix an `environmentRecipes` entry in `barkos.yaml`, scaffold
  provider lifecycle scripts, or resolve an `barkos vm recipe doctor` failure.
---
...
BarkOS is a **thin wrapper**: you guide, detect, and scaffold; you never own the user's cloud account,
...
**The one branch that shapes everything — connection mode:** **BarkOS-server** (`create` runs `barkos serve`
in the env and emits a `pairingCode`; §7c/§7f) vs **SSH** (`create` runs no server and emits a
`connection.type:"ssh"` block BarkOS dials into; §7g/§7h). Settle this first — it changes the `create`
output shape and half the templates.

Keep BarkOS's checkout behavior unchanged by default: omit `checkoutMode`, emit schema version 1, and
let BarkOS create a linked worktree. Only use `checkoutMode: provisioned-root` when the user explicitly
...
**Quick-start (happy path):** interview the user (connection mode BarkOS-server vs SSH, provider, agent CLI,
git auth — §1.2) + read the provider's CLI docs → scaffold `scripts/orca-vm/` from §7 → run the
base-snapshot script, then the auth script (you invoke these by hand; not via `barkos.yaml`) → wire
`environmentRecipes` in `barkos.yaml` → `barkos vm recipe doctor <id> --json` (free) → then the `--provision`
...
a long time, or need the user at the keyboard. Never create a BarkOS workspace or commit unless asked.
...
2. **Interview the user up front** — gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** how BarkOS attaches to the environment — an **BarkOS server** (the VM runs
     `barkos serve` and BarkOS pairs over its pairing URL; worked example §7f) or **SSH** (BarkOS connects to
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, … For non-obvious providers, also
     ...
     (host/port/user/key or proxy command) or only a provider-mediated interactive shell; BarkOS SSH mode
     needs the former.
...
7. **Wire the recipe** so `barkos.yaml` points create/suspend/resume/destroy at the scripts (§8). The
   workspace composer reads `environmentRecipes` from the project's primary checkout of `barkos.yaml`, **not** from
   a feature branch or worktree. So a recipe added only on a branch won't appear as a "Run on" option
   until that `barkos.yaml` change is committed and merged to the project's primary branch. Tell the user
...
8. **Dry-run doctor** — `barkos vm recipe doctor <recipe-id> --repo-path <repo> --json` (free, static; §9).
   Fix every failure before going live.
9. **[CHECKPOINT] Live self-test** — get the user's OK once, then run
   `barkos vm recipe doctor <recipe-id> --provision --json` as a loop: it runs create → validates →
...
- **Connection mode** (BarkOS server vs SSH) confirmed with the user — see §1 step 2; it shapes the recipe.
...
bash`, `set -euo pipefail`, quoted paths. **Windows:** a bare `.sh` won't run — scaffold `.ps1`/`.cmd`
  or require WSL/Git-Bash and point `barkos.yaml` at the right launcher.
...
Worked Vercel commands for this phase are in §7f. You run this script by hand (not via `barkos.yaml`),
...
# 3. remote exec: start barkos serve in the background and read the recipe JSON it writes (see below)
...
**The exact `barkos serve` invocation and its output (verified — do not improvise the flags).** Inside the
VM, run:
...
**Binary name:** in a VM built from source (the Phase-2 flow), run it as `pnpm exec barkos-dev serve …`
from the repo root — `barkos-dev` is the in-repo entrypoint and is what the §7f example uses. Plain
`barkos serve …` is the same command when the built CLI is installed on the VM's PATH. The flags/output
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # BarkOS passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in barkos.yaml)
```
...
# 3. (remote) start barkos serve in the background, writing recipe JSON to a file; poll until it parses
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  ...
  -- bash -lc 'set -euo pipefail; cd "$ORCA_PROJECT_ROOT"; rm -f /tmp/orca-recipe.json /tmp/orca-serve.log; \
    nohup pnpm exec barkos-dev serve --port "$ORCA_PORT" --project-root "$ORCA_PROJECT_ROOT" \
...
`userData.resourceId` from stdin (§7d). This is the **BarkOS-server** connection mode (the recipe emits a
...
- **`create` does NOT run `barkos serve` and does NOT emit a `pairingCode`.** BarkOS itself connects to the
  host over its SSH relay, brings up the git + filesystem providers, and imports the repo. The script's
  only job is to make the host ready and **print SSH connection details** BarkOS will dial.
- The result uses a `connection` block with `type: "ssh"` and a `target`, **not** the flat
  `pairingCode`/`projectRoot` shape. Exact shape (BarkOS rejects anything else):
...
- `relayGracePeriodSeconds` (optional): how long BarkOS keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO barkos serve here)
...
# 2. print the SSH connection block (NO pairingCode, NO barkos serve). host/port/username tell BarkOS's
...
`destroy: none` and omit suspend/resume. (BarkOS still disconnects/reconnects its own SSH relay on
...
`connection.type:"ssh"` block above instead of starting `barkos serve`.
...
require WSL/Git-Bash (and point `barkos.yaml` at e.g. `bash ./scripts/orca-vm/<name>.sh` via a `.cmd`
...
# BarkOS-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
...
**BarkOS-server mode** — boot the env, start `barkos serve` in it, and print serve's result:
...
Here `pairingCode` (from `barkos serve --recipe-json`) and `projectRoot` are required; `schemaVersion` (`1`)
and `userData` are optional.

**SSH mode** — do **not** run `barkos serve`; print the `connection.type:"ssh"` block instead (full shape +
...
Start BarkOS remotely with `barkos serve --port "$PORT" --project-root "$ABS_ROOT" --pairing-address
...
`barkos vm recipe doctor <recipe-id> --repo-path <repo> --json` validates **static wiring only** — it does
...
`barkos vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` actually runs the recipe end
...
- Don't make BarkOS own provider lifecycle beyond invoking the configured scripts.
- Don't commit or create a BarkOS workspace unless asked.
