...
name: computer-use
description: >-
  Use BarkOS's computer-use CLI to inspect and operate local desktop app windows
  ...
  accessibility actions. Also use for browser windows, webviews, BarkOS app UI,
...
Use this skill for desktop UI through `barkos computer`. When the requested target is a website or web app, operate the desktop browser app/window that contains the page.
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `ORCA computer ... --app Gmail` unchanged because `barkos computer` app selectors refer to desktop apps, not website names.
...
Confirm BarkOS status unless already checked, then run `ORCA computer capabilities --json`. For website or web-app targets such as Gmail, identify the desktop browser app/window that contains the page, then get that target app state with `ORCA computer get-app-state --app <app> --json`.
