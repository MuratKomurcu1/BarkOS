---
name: barkos-emulator
description: >
  Control a mobile (iOS) emulator / simulator stream from inside BarkOS using the `barkos` CLI.
  Use for taps, gestures, typing, hardware buttons, camera injection, permissions, accessibility tree, and more — all while seeing the live view in BarkOS's emulator pane.
  Prefer this over raw `npx serve-sim` or direct simctl when running agents inside BarkOS (the orca surface handles device scoping, helper lifecycle, and worktree context).
  Complements the barkos-cli skill for terminals, worktrees, and the built-in browser.
license: Apache-2.0
---

# BarkOS Emulator (serve-sim powered)

Drive an Apple Simulator (iOS / iPad / Watch) **from within BarkOS** using `ORCA emulator ...` commands (or `ORCA emulator exec` for raw power). This wraps the excellent [serve-sim](https://github.com/EvanBacon/serve-sim) open-source tool so agents get a consistent BarkOS-native CLI surface, automatic helper management, and seamless integration with BarkOS's live emulator pane (the visual "preview" surface).

The underlying serve-sim helper captures the real simulator framebuffer (via private SimulatorKit / IOSurface for low-latency 60fps H.264 or MJPEG) and exposes a WebSocket control channel. BarkOS's bridge owns the helper processes and per-worktree "active emulator" state so unqualified commands "just work" on whatever device/pane is current for the worktree.

## CLI executable

Choose the BarkOS executable once: use the `ORCA_CLI_COMMAND` environment value when set;
otherwise use `barkos-dev` in a dev session exposing `ORCA_DEV_REPO_ROOT`, `barkos` on
Linux outside a BarkOS-managed terminal, and `barkos` everywhere else. Never try bare
`barkos` first on unmanaged Linux because it normally resolves to the GNOME screen reader.
...
- The user/agent wants to **tap, swipe, drag, pinch, or press hardware buttons** on a running iOS simulator while seeing the live result in BarkOS.
...
- You are inside a BarkOS worktree/terminal and want the emulator to be **workspace-scoped** (like browser tabs) with explicit targeting when needed.
- The agent should use BarkOS's preview pane instead of external Simulator.app or raw serve-sim URLs.

**When NOT to use**

- Android emulators → use the `barkos-emulator-android` skill (same `ORCA emulator` namespace, cross-platform via adb/emulator).
...
## Prerequisites (enforced / surfaced by BarkOS)

- macOS host (with Xcode Command Line Tools: `xcrun --version`).
- A booted simulator (`xcrun simctl list devices booted` or let BarkOS/attach help boot one).
- Node available (for the serve-sim bits; BarkOS bundles the CLI surface).
- macOS 14+ recommended for full camera injection features.

BarkOS will give clear errors if these are missing (e.g. "emulator commands require macOS + Xcode tools").
...
│ BarkOS EmulatorBridge│ ───────────────► │ serve-sim-bin   │ ────────────► │ iOS Simulator│
│ (main process)     │ (or exec serve-sim) (per-device)   │               └──────────────┘
└────────────────────┘                  └─────────────────┘
          ▲
          │ (state + lifecycle)
┌────────────────────┐
│ BarkOS CLI (agents)  │  e.g. ORCA emulator tap 0.5 0.7
│ barkos-emulator skill│
└────────────────────┘
```
...
Agents use the BarkOS executable chosen above (on PATH in BarkOS terminals) and never have to manage PIDs, state files in /tmp, or raw WS URLs themselves.

**For `pnpm dev` testing:** run `pnpm build:cli` first (rebuilds the CLI + ensures the `barkos-dev` shim points at _this_ worktree). Then inside the dev app use `barkos-dev emulator ...` (or the direct `./config/scripts/barkos-dev.mjs emulator ...` from the repo root). The orchestration preambles and dev launchers automatically select the dev command name so the CLI reaches your in-memory EmulatorBridge / runtime. Plain `barkos` reaches a packaged install instead.
...
| Goal                     | Command                                                             | Notes                                                                                                                                                                                                               |
| ------------------------ | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| List available / running | `ORCA emulator list [--worktree <sel>]`                             | Shows BarkOS-managed + raw serve-sim streams. Use output for explicit --device/--emulator.                                                                                                                          |
...
| Stop                     | `ORCA emulator kill [--device <id>]`                                | Or let pane close / BarkOS quit clean up.                                                                                                                                                                           |

Most support `--worktree <selector>` and explicit `--device <udid|name>` or `--emulator <id>` (from list) for targeting.

## Critical gotchas (teach agents)

- **Prefer `tap` over `gesture` for single taps** (same as raw serve-sim). Separate gesture begin/end can be interpreted as long-press due to WS overhead. The BarkOS wrapper uses the reliable quick sequence.
...
- Stale helpers / state are cleaned by BarkOS on quit, but agents should `kill` when done.
- Private APIs under the hood (SimulatorKit etc.) — version sensitive (Xcode updates can affect).

## Targeting devices & worktrees

- Default: current worktree's active emulator (resolved from shell cwd or BarkOS context).
...
- BarkOS-generated emulator id (for stability, like browserPageId): use `--emulator <id>` returned by list (recommended for scripts that persist ids).

`--worktree all` only for listing.

## Integration with the live pane (UI)

- Opening the emulator pane in BarkOS (or `attach`) makes that stream the "active" one for the worktree → CLI commands target it automatically.
...
Or let BarkOS quit / close the pane.

Orphans are cleaned by BarkOS (like agent-browser sessions).
...
Confirm `ORCA status --json` and `ORCA emulator list --json`, then drive the emulator while the live view is visible in BarkOS.

See also: barkos-cli skill (terminals, worktrees, built-in browser), computer-use for desktop outside the simulator.

This skill is the BarkOS-native replacement for raw serve-sim when you want the visual + control integrated in the IDE.
