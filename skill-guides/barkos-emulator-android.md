---
name: barkos-emulator-android
description: >
  Control an Android emulator / device from inside BarkOS using the `barkos` CLI.
  ...
  (Windows, Linux, macOS). Complements the barkos-emulator (iOS) and barkos-cli skills.
license: Apache-2.0
---

# BarkOS Emulator — Android (adb / emulator powered)

Drive an Android emulator or adb-connected device **from within BarkOS** using
...
Windows, Linux, and macOS — unlike the iOS backend (`barkos-emulator`), which is
...
- iOS simulators → use the `barkos-emulator` skill (macOS only).
...
## Prerequisites (surfaced by BarkOS)

- **Android Studio / Android SDK** installed, with `ANDROID_HOME` (or
  `ANDROID_SDK_ROOT`) set. BarkOS also checks the per-OS default location
...
BarkOS returns a clear message when the SDK is missing
...
```text
┌────────────────────────┐
│ BarkOS CLI (agents)      │  e.g. ORCA emulator tap 0.5 0.7 --device emulator-5554
└───────────┬────────────┘
            │ RPC
            ▼
...
BarkOS owns backend routing and the per-worktree active-device registry. The
Android backend converts BarkOS's normalized 0–1 coordinates to device pixels and
...
(top-left origin) — never pixels; BarkOS converts using the live screen size.
...
- **All coordinates are normalized 0..1** (top-left origin), never pixels — BarkOS
...
See also: `barkos-emulator` (iOS, macOS-only), `barkos-cli` (terminals, worktrees,
