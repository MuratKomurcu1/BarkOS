...
describe('HostProtocolGate', () => {
  ...
  it('replaces the host UI with the block screen when mobile is too old', async () => {
    ...
    expect(output).toContain("BarkOS Mobil'i güncelleyin")
    expect(output).toContain('özel dağıtımından')
    expect(output).not.toContain('App Store')
    expect(output).not.toContain('HostContent')
  })

  it('keeps Android updates inside the private BarkOS distribution', async () => {
    vi.spyOn(console, 'warn').mockImplementation(() => {})
    ...
    expect(output).toContain("BarkOS Mobil'i güncelleyin")
    expect(output).toContain('özel dağıtımından')
    expect(output).not.toContain('GitHub Releases')
    expect(output).not.toContain('App Store')
    expect(output).not.toContain('HostContent')
  ...
  it('replaces the host UI with the block screen when desktop is too old', async () => {
    ...
    renderer = await renderGate()
    const output = renderedText(renderer)
    expect(output).toContain("Bilgisayarınızdaki BarkOS'u güncelleyin")
    expect(output).not.toContain('GitHub Releases')
    expect(output).not.toContain('HostContent')
  })
