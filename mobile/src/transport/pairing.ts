...
export function extractPairingCodeFromUrl(url: string): string | null {
  ...
  const match = /^(?:barkos|orca):\/\/([^/?#]*)([^?#]*)?/i.exec(trimmed)
  if (!match) {
    return null
  }
...
// Why: BarkOS links are canonical; legacy Orca links remain readable for imported pair records.
