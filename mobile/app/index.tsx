...
export default function HomeScreen() {
  ...
  const renderTaskHomeCard = () => (
    ...
    >
      ...
      <View style={styles.taskHomeMain}>
        <Text style={styles.taskHomeTitle}>Görevler</Text>
        ...
        >
          {primaryTaskProviders.map((provider) => (
            <Pressable
              ...
              accessibilityLabel={`${TASK_PROVIDER_LABELS[provider]} görevlerini aç`}
              hitSlop={8}
              style={({ pressed }) => [
  ...
  async function handleRemove() {
    ...
    try {
      ...
      Alert.alert('Bilgisayar kaldırılamadı', 'Lütfen yeniden deneyin.')
    }
  }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {hostCatalog.length === 0 ? (
        ...
        >
          <View style={styles.emptyHero}>
            <Text style={styles.emptyTitle}>Bilgisayarınızı bağlayın</Text>
            <Text style={styles.emptyBody}>
              Ajanları izlemek, terminallere geçmek ve işleri telefondan yönetmek için
              bilgisayarınızdaki BarkOS ile eşleştirin.
            </Text>
            <Pressable style={styles.primaryButton} onPress={() => router.push('/pair-scan')}>
              <QrCode size={17} color={colors.bgBase} />
              <Text style={styles.primaryButtonText}>Bilgisayarı eşleştir</Text>
            </Pressable>
          </View>

          <View style={styles.stepsSection}>
            <Text style={styles.sectionHeading}>Nasıl çalışır?</Text>
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <View>
              <View style={styles.hero}>
                <Text style={styles.heroTitle}>Tekrar hoş geldiniz</Text>
              </View>
              ...
              {stats && (
                <View style={styles.statsRow}>
                  <View style={styles.statCard}>
                    ...
                    <Text style={styles.statLabel}>Başlatılan ajan</Text>
                  </View>
                  <View style={styles.statCard}>
                    <Text style={styles.statValue}>{formatDuration(stats.totalAgentTimeMs)}</Text>
                    <Text style={styles.statLabel}>Ajan süresi</Text>
                  </View>
                  <View style={styles.statCard}>
                    <Text style={styles.statValue}>{stats.totalPRsCreated.toLocaleString()}</Text>
                    <Text style={styles.statLabel}>Oluşturulan inceleme</Text>
                  </View>
                </View>
              )}

              <Text style={styles.sectionHeading}>Bilgisayarlar</Text>
            </View>
          }
                ...
                onPress={() => {
                  if (item.credentialStatus === 'missing') {
                    ...
                    void loadHostCatalog()
                      .then(setHostCatalog)
                      .catch(() =>
                        Alert.alert('Eşleştirme denetlenemedi', 'Lütfen yeniden deneyin.')
                      )
                  } else {
          ...
          ListFooterComponent={
            <View>
              ...
              {resumeCard ? (
                <>
                  <Text style={[styles.sectionHeading, styles.sectionHeadingTightTop]}>Sürdür</Text>
                  <Pressable
              ...
              <Text style={[styles.sectionHeading, styles.sectionHeadingTightTop]}>Görevler</Text>
              {renderTaskHomeCard()}
                            ...
                            <View key={provider} style={styles.accountsRow}>
                              ...
                              <View style={styles.accountsInfo}>
                                <Text style={styles.accountsEmail} numberOfLines={1}>
                                  {active?.email ?? 'Sistem varsayılanı'}
                                </Text>
      ...
      <ConfirmModal
        ...
        message={`"${confirmRemove?.name}" kaldırılsın mı? Daha sonra yeniden eşleştirebilirsiniz.`}
        confirmLabel="Kaldır"
        destructive
...
const ONBOARDING_STEPS = [
  {
    title: "Masaüstü BarkOS'u açın",
    desc: 'Ayarlar → Mobil bölümüne gidip eşleştirme QR kodu oluşturun.'
  },
  {
    title: 'Kodu tarayın',
    desc: 'Tarayıcıyı açmak için yukarıdaki düğmeye dokunup ekrandaki QR koduna yöneltin.'
  },
  {
    title: 'Bağlantı hazır',
    desc: 'Bilgisayarınız burada görünür. Her şey uçtan uca şifrelenir.'
  }
]
