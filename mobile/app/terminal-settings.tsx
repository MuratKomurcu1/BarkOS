...
const TEXT_SIZE_OPTIONS: (PickerOption<TextSizeValue> & { scale: number })[] = [
  { value: 'smallest', label: 'En küçük (%50)', scale: 0.5 },
  { value: 'smaller', label: 'Küçük (%75)', scale: 0.75 },
  { value: 'default', label: 'Varsayılan (%100)', scale: 1 },
  { value: 'large', label: 'Büyük (%125)', scale: 1.25 },
  { value: 'larger', label: 'Daha büyük (%150)', scale: 1.5 },
  { value: 'largest', label: 'En büyük (%200)', scale: 2 }
]
...
const AUTO_RESTORE_FIT_OPTIONS: (PickerOption<RestoreValue> & { ms: number | null })[] = [
  { value: 'indefinite', label: 'Telefon boyutunda tut (varsayılan)', ms: null },
  { value: '60s', label: '1 dakika sonra', ms: 60_000 },
  { value: '5m', label: '5 dakika sonra', ms: 5 * 60_000 },
  { value: '30m', label: '30 dakika sonra', ms: 30 * 60_000 }
]
...
function autoRestoreSummary(ms: number | null | undefined): string {
  ...
  const exact = AUTO_RESTORE_FIT_OPTIONS.find((o) => o.ms === ms)
  return exact ? exact.label : `${Math.round(ms / 1000)} saniye sonra`
}
...
export default function TerminalSettingsScreen() {
  ...
  return (
    <GestureHandlerRootView style={[styles.container, { paddingTop: insets.top + spacing.sm }]}>
      ...
      >
        <Text style={styles.groupHeading}>UYGULAMADAN AYRILDIĞINIZDA</Text>
        <Text style={styles.groupDescription}>
          Telefonda terminal kullanırken BarkOS ekranınıza sığması için terminali küçültür.
          Uygulamadan ayrıldığınızda terminalin telefon boyutunda kalmasını veya yeniden masaüstü
          boyutuna dönmesini buradan belirleyin.
        </Text>

        {hosts.length === 0 ? (
          <View style={[styles.section, styles.sectionTopGap]}>
            <Text style={styles.emptyText}>
              Henüz eşleştirilmiş bilgisayar yok. Terminal davranışını yönetmek için bir bilgisayar
              eşleştirin.
            </Text>
          </View>
        ) : (
        ...
        <Text style={[styles.groupHeading, styles.inputGroupGap]}>METİN BOYUTU</Text>
        <Text style={styles.groupDescription}>
          Terminal metnini ölçeklendirin. Bu ayar yalnızca bu cihazdaki görünümü değiştirir;
          masaüstü terminalini etkilemez.
        </Text>
        <View style={[styles.section, styles.sectionTopGap]}>
          <Pressable
            style={({ pressed }) => [styles.row, pressed && styles.rowPressed]}
          ...
          >
            ...
            <View style={styles.rowContent}>
              <Text style={styles.rowLabel}>Metin boyutu</Text>
        ...
        <Text style={[styles.groupHeading, styles.inputGroupGap]}>KLAVYE GİRİŞİ</Text>
        <Text style={styles.groupDescription}>
          Terminal komut çubuğunda telefon tipi otomatik tamamlama, düzeltme ve yazım önerilerini
          etkinleştirin. Komutların veya dosya yollarının değişmemesi için varsayılan olarak
          kapalıdır.
        </Text>
        <View style={[styles.section, styles.sectionTopGap]}>
          <View style={styles.row}>
            <View style={styles.rowContent}>
              <Text style={styles.rowLabel}>Otomatik tamamlama ve düzeltme</Text>
              <Text style={styles.rowSublabel}>{autocompleteEnabled ? 'Açık' : 'Kapalı'}</Text>
            </View>
            <Switch
      ...
      <PickerModal<RestoreValue>
        ...
        title={pickerHost ? `${pickerHost.name} boyutunu geri yükle` : ''}
