  ...
  const chooseSessionView = useCallback(
    async (view: MobileSessionView) => {
      ...
      try {
        ...
        setError('Seçiminiz kaydedilemedi. Yeniden deneyin.')
        setBusyChoice(null)
        choiceInFlightRef.current = false
      }
    },
  ...
  const chooseNotifications = useCallback(
    async (choice: NotificationOnboardingChoice) => {
      ...
      try {
        ...
        setError('Bildirim ayarları güncellenemedi. Yeniden deneyin.')
        setBusyChoice(null)
        choiceInFlightRef.current = false
      }
    },
      ...
      <View style={styles.brandRow}>
        ...
        {steps.length > 1 ? (
          <View
            ...
            accessibilityValue={{ text: `${steps.length} adımın ${activeIndex + 1}. adımı` }}
