...
const DICTATION_MODES = [
  { value: 'toggle', label: 'Aç/kapat' },
  { value: 'hold', label: 'Basılı tut' }
] as const
...
export default function VoiceSettingsScreen(): React.JSX.Element {
  ...
  const refresh = useCallback(async (): Promise<boolean | undefined> => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Ses ayarları yüklenemedi')
      return undefined
    } finally {
      setLoading(false)
    }
  ...
  const handleUseModel = useCallback(
    async (model: MobileSpeechModel) => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : 'Model seçilemedi')
      } finally {
        setBusyAction(null)
      }
    },
  ...
  const selectedModelLabel = selectedModel?.label ?? 'Seçilmedi'
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + spacing.sm }]}>
      <View style={styles.topRow}>
        ...
        <Text style={styles.heading}>Ses</Text>
      </View>

      {!client ? (
        <View style={[styles.section, styles.sectionTopGap]}>
          <Text style={styles.emptyText}>
            Ses ayarlarını yönetmek için bir bilgisayara bağlanın.
          </Text>
        </View>
      ) : loading && setup === null ? (
        ...
        <View style={[styles.section, styles.sectionTopGap]}>
          <Text style={styles.errorText}>{error ?? 'Ses ayarları yüklenemedi.'}</Text>
        </View>
      ) : (
        <ScrollView
        ...
        >
          <Text style={styles.groupHeading}>DİKTE</Text>
          <View style={[styles.section, styles.sectionTopGap]}>
            <View style={styles.row}>
              <View style={styles.rowContent}>
                <Text style={styles.rowLabel}>Sesli dikteyi etkinleştir</Text>
                <Text style={styles.rowSublabel}>
                  Masaüstünüzde odaklanmış herhangi bir panele metin dikte edin.
                </Text>
              </View>
              <Switch
                value={enabled}
            ...
            >
              <View style={styles.rowContent}>
                <Text style={styles.rowLabel}>Dikte modu</Text>
                <Text style={styles.rowSublabel}>
                  Aç/kapat: başlatmak ve durdurmak için bir kez basın. Basılı tut: basarken dikte
                  edin.
                </Text>
              </View>
          ...
          <Text style={[styles.groupHeading, styles.inputGroupGap]}>KONUŞMA MODELİ</Text>
          <View style={[styles.section, styles.sectionTopGap]}>
            <Pressable
              style={({ pressed }) => [
                styles.row,
            ...
            >
              <View style={styles.rowContent}>
                <Text style={styles.rowLabel}>Konuşma modeli</Text>
      ...
      <BottomDrawer visible={modelDrawerOpen} onClose={() => setModelDrawerOpen(false)}>
        <Text style={styles.drawerTitle}>Konuşma modeli</Text>
        {setup ? (
          <VoiceModelList
            setup={setup}
            disabled={false}
