  ...
  useEffect(() => {
    ...
    const hydrateTaskState = async (): Promise<void> => {
      ...
      if (!status.capabilities?.includes(MOBILE_TASKS_CAPABILITY)) {
        ...
        setMergeMethodProjectRow(null)
        resetWorkspaceCreateState()
        setError("Mobil görevleri kullanmak için masaüstü BarkOS'u güncelleyin.")
        setTaskStateHydrated(false)
        return
      }
    ...
    async (project: GitHubProjectRef, options: { viewNumber?: number } = {}): Promise<void> => {
      ...
      try {
        ...
        if (explicitView && explicitView.layout !== 'TABLE_LAYOUT') {
          throw new Error('BarkOS bu GitHub Projesi düzenini henüz desteklemiyor.')
        }
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          // Why: desktop prompts before running repo-owned barkos.yaml hooks. Mobile
  ...
  const createWorkspaceFromProjectRow = useCallback(
    async (row: GitHubProjectRow): Promise<void> => {
      ...
      if (!kind || !row.content.number || !row.content.url) {
        setError("Çalışma alanı oluşturmadan önce proje öğesinin deposunu BarkOS'a ekleyin.")
        return
      }
      if (!repo) {
        const slug = splitRepositorySlug(row.content.repository)
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {!tasksSupported ? (
        tasksUnsupported ? (
          <View style={styles.centered}>
            <Text style={styles.emptyText}>Masaüstü BarkOS'u güncelleyin</Text>
            <Text style={styles.centeredHint}>
              Mobil görev görünümü daha yeni bir masaüstü çalışma zamanı gerektiriyor.
            </Text>
          </View>
        ) : (
          <View style={styles.centered}>
      ...
      <PickerModal
        ...
        onSelect={(viewId) => {
          ...
          if (view && view.layout !== 'TABLE_LAYOUT') {
            setGithubProjectError('BarkOS bu GitHub Projesi düzenini henüz desteklemiyor.')
            return
          }
        ...
        {orcaYamlTrustPrompt ? (
          <View>
            <View style={styles.sheetHeader}>
              ...
              <Text style={styles.sheetSubtitle}>
                This repository's barkos.yaml runs on your machine before the workspace starts. Only
      ...
      >
        {projectRepoNotInOrca ? (
          <View>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>Depo BarkOS'ta yok</Text>
              <Text style={styles.sheetSubtitle}>
                {projectRepoNotInOrca.owner}/{projectRepoNotInOrca.repo} BarkOS'a eklenmemiş. Depoyu
                masaüstü uygulamasından ekleyip mobil Görevler sayfasını yenileyin.
              </Text>
            </View>
              ...
              projectRowItem.content.state !== 'MERGED' ? (
                <>
                  ...
                  >
                    ...
                    <Text style={styles.actionText}>Pull request'i birleştir</Text>
                  </Pressable>
                  {!projectRowHostedRepo ? (
                    <Text style={styles.emptyInlineText}>
                      Birleştirme için bu deponun BarkOS'a eklenmesi gerekir.
                    </Text>
                  ) : null}
                </>
              ) : null}
