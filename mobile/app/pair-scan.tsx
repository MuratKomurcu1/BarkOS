...
export default function PairScanScreen() {
  ...
  const handleBarCodeScanned = useCallback(
    ({ data }: { data: string }) => {
      ...
      if (!offer) {
        setStatus('error')
        setErrorMessage('Geçerli bir BarkOS QR kodu değil')
  ...
  const handlePasteSubmit = useCallback((input: string) => {
    ...
    if (!offer) {
      setStatus('error')
      setErrorMessage(
        'Geçerli bir eşleştirme kodu değil; bilgisayarınızdan kopyalayıp yeniden yapıştırın'
      )
      processingRef.current = false
      return
    }
  ...
  async function testAndSave(offer: PairingOffer) {
    ...
    try {
      ...
      setErrorMessage(
        timedOut
          ? `${PAIRING_OVERALL_TIMEOUT_MS / 1000} saniye içinde bağlanılamadı; ayrıntı için aşağıdaki günlüğe bakın`
          : `Eşleştirme başarısız: ${err instanceof Error ? err.message : String(err)}`
      )
      processingRef.current = false
    }
  }
      ...
      <View ref={setPairScanRootRef} style={[styles.container, containerPadding]}>
        ...
        <View style={styles.centered}>
          <Text style={styles.title}>
            {canAskAgain ? 'Bilgisayarla eşleştirin' : 'Kamera erişimi kapalı'}
          </Text>
          <Text style={styles.subtitle}>
            {canAskAgain
              ? 'Masaüstü BarkOS uygulamasındaki QR kodunu tarayın veya eşleştirme kodunu yapıştırın.'
              : 'Ayarlardan kamera erişimini açın veya eşleştirme kodunu yapıştırın.'}
          </Text>
          <Pressable
          ...
          >
            ...
            <Text style={styles.primaryButtonText}>{canAskAgain ? 'Devam et' : 'Ayarları aç'}</Text>
          </Pressable>
          <Pressable
          ...
          >
            ...
            <Text style={styles.pasteButtonText}>Bunun yerine kodu yapıştır</Text>
          </Pressable>
        </View>
        <TextInputModal
          visible={pasteVisible}
          title="Eşleştirme kodunu yapıştır"
          message="Bilgisayarınızdaki QR kodunun altında görünen kodu kopyalayın."
          placeholder="barkos://pair?code=... veya kodu yapıştırın"
  ...
  return (
    <View ref={setPairScanRootRef} style={[styles.container, containerPadding]}>
      ...
      <View style={styles.steps}>
        <Step number={1} text="Bilgisayarınızda BarkOS'u açın" />
        <Step number={2} text="Ayarlar → Mobil bölümüne gidin" />
        <Step number={3} text="QR kodunu tarayın" />
      </View>
      ...
      {status === 'scanning' && (
        <>
          ...
          >
            <ClipboardIcon size={16} color={colors.textSecondary} />
            <Text style={styles.pasteButtonText}>Ya da eşleştirme kodunu yapıştır</Text>
          </Pressable>
        </>
      )}
      ...
      {status === 'connecting' && (
        <View style={styles.centered}>
          ...
          <View style={styles.logSlot}>
            <ConnectionLog entries={logs} title="Eşleştirme günlüğü" />
          </View>
        </View>
      )}
      ...
      {status === 'error' && (
        <View style={styles.centered}>
          ...
          <View style={styles.errorActions}>
            ...
            >
              <Text style={styles.secondaryButtonText}>Bunun yerine kodu yapıştır</Text>
            </Pressable>
          </View>
        </View>
      )}
