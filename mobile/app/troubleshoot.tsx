  ...
  const runDiagnostics = useCallback(async () => {
    ...
    try {
      ...
      results.push(
        hosts.length > 0
          ? {
              label: 'Eşleştirilmiş bilgisayarlar',
              status: 'pass',
              detail: `${hosts.length} eşleşme`
            }
          : {
              label: 'Eşleştirilmiş bilgisayarlar',
              status: 'fail',
              detail: 'Yok — eşleştirmek için QR tarayın'
            }
      )
    } catch {
      results.push({
        label: 'Eşleştirilmiş bilgisayarlar',
        status: 'warn',
        detail: 'Bilgisayar verisi okunamadı'
      })
    }
    ...
    try {
      ...
      results.push(
        resp.ok
          ? { label: 'İnternet', status: 'pass', detail: 'Bağlı' }
          : { label: 'İnternet', status: 'warn', detail: 'Beklenmeyen yanıt' }
      )
    } catch {
      if (!isCurrentRun()) {
        return
      }
      results.push({ label: 'İnternet', status: 'fail', detail: 'Bağlantı yok' })
    } finally {
    ...
    try {
      ...
      for (const host of hosts) {
        ...
        results.push({
          ...
          detail: reachable
            ? `${formatEndpoint(host.endpoint)} üzerinden erişilebilir`
            : unreachableHostDetail(host.endpoint)
        })
        setChecks([...results])
      }
    } catch {
      results.push({ label: 'Bilgisayarlar', status: 'warn', detail: 'Denetlenemedi' })
    }
  ...
  return (
    ...
    >
      <View style={styles.topRow}>
        ...
        <Text style={styles.heading}>Sorun giderme</Text>
      </View>
      ...
      >
        ...
        >
          ...
          <Text style={styles.diagnosticButtonLabel}>Bağlantı günlüğünü göster</Text>
        </Pressable>
        ...
        <Text style={styles.sectionHeading}>Yaygın sorunlar</Text>
